#include "data_tamer/sinks/mcap_read_sink.hpp"
...
{
template <size_t N>
void SerializeIntoBuffer(SpanBytes &buffer, boost::container::small_vector<uint8_t, N> const &value)
{
...
MCAPReadSink::MCAPReadSink(const std::string &filepath, bool do_compression)
    : filepath_(filepath), compression_(do_compression), original_filepath_(filepath)
{
    openFile(filepath_);
}

void DataTamer::MCAPReadSink::openFile(std::string const &filepath)
{
...
MCAPReadSink::~MCAPReadSink()
{
    stopThread();
    std::scoped_lock lk(mutex_);
}

void MCAPReadSink::addChannel(std::string const &channel_name, Schema const &schema)
{
    std::scoped_lock lk(mutex_);
...
bool MCAPReadSink::storeSnapshot(const Snapshot &snapshot)
{
    std::scoped_lock lk(mutex_);
...
void MCAPReadSink::setMaxTimeBeforeReset(std::chrono::seconds reset_time)
{
    reset_time_ = reset_time;
}

void MCAPReadSink::setCreateNewFileOnReset(bool create_file_on_reset)
{
    create_file_on_reset_ = create_file_on_reset;
}

void MCAPReadSink::stopRecording()
{
    std::scoped_lock lk(mutex_);
...
void MCAPReadSink::restartRecording(const std::string &filepath, bool do_compression)
{
    restartRecordingImpl(filepath, do_compression, true);
}

void MCAPReadSink::restartRecordingImpl(const std::string &filepath, bool do_compression, bool new_file)
{
    std::scoped_lock lk(mutex_);
    if (new_file)
    {
