...
inline ValuePtr::ValuePtr(const T *pointer, CustomSerializer::Ptr type_info)
    : v_ptr_(pointer), type_(GetBasicType<T>()), type_index_(typeid(T)), memory_size_(sizeof(T)), is_vector_(false)
{
    if (type_info)
    {
...
inline ValuePtr::ValuePtr(const Container<T, TArgs...> *vect)
    : v_ptr_(vect), type_(GetBasicType<T>()), type_index_(typeid(Container<T, TArgs...>)), memory_size_(sizeof(T)),
      is_vector_(true)
{
...
inline ValuePtr::ValuePtr(const std::array<T, N> *array)
    : v_ptr_(array), type_(GetBasicType<T>()), type_index_(typeid(std::array<T, N>)), is_vector_(true), array_size_(N)
{
...
{
    return type_ == other.type_ && type_index_ == other.type_index_ && is_vector_ == other.is_vector_ &&
