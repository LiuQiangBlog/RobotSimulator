...
public:
    ...
    void serialize(const void *src_instance, SerializeMe::SpanBytes &dst_buffer) const override;
...
public:
    template <typename T>
    CustomSerializer::Ptr addType(const std::string &type_name, bool skip_if_present = false);
...
template <typename T>
inline CustomSerializerT<T>::CustomSerializerT(std::string type_name) : _name(std::move(type_name))
{
    static_assert(!SerializeMe::container_info<T>().is_container, "Don't pass containers a template type");
...
template <typename T>
inline void CustomSerializerT<T>::serialize(const void *src_instance, SerializeMe::SpanBytes &dst_buffer) const
{
    const auto *obj = static_cast<const T *>(src_instance);
...
{
    static_assert(!IsNumericType<T>(), "You don't need to create a serializer for a numerical type.");
    static_assert(!SerializeMe::container_info<T>().is_container, "Don't pass containers as template type");
...
template <typename T>
inline CustomSerializer::Ptr TypesRegistry::addType(const std::string &type_name, bool skip_if_present)
{
    static_assert(!IsNumericType<T>(), "You don't need to create a serializer for a numerical type.");
    static_assert(!SerializeMe::container_info<T>().is_container, "Don't pass containers as template type");
