...
static int formatString_u(char *buffer, size_t buffer_size, const char *formatString, unsigned int value)
{
    ...
    for (i = 0; formatString[i] != '\0' && written < buffer_size - 1; i++)
    {
        ...
        if (formatString[i] == 'u')
        {
            if (written + valueSize >= buffer_size)
                         ...
                         const BMK_advancedParams_t *adv)
{
    ...
    CHECK_Z(ZSTD_CCtx_setParameter(ctx, ZSTD_c_targetCBlockSize, (int)adv->targetCBlockSize));
...
/* `addArgs` is the context */
static size_t
local_defaultDecompress(const void *srcBuffer, size_t srcSize, void *dstBuffer, size_t dstCapacity, void *addArgs)
{
...
#define NB_MARKS 4
        ...
        while (!(compressionCompleted && decompressionCompleted))
        {
            if (!compressionCompleted)
            {
...

                {
                    ...
                    OUTPUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (x%5.*f), %6.*f MB/s \r", marks[markNb], displayName,
                                (unsigned)srcSize, (unsigned)cSize, ratioDigits, ratio,
            ...
            if (!decompressionCompleted)
            {
                ...
                {
                    int const ratioDigits = 1 + (ratio < 100.) + (ratio < 10.);
                    OUTPUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (x%5.*f), %6.*f MB/s, %6.1f MB/s\r", marks[markNb],
                                displayName, (unsigned)srcSize, (unsigned)cSize, ratioDigits, ratio,
                                benchResult.cSpeed < (10 * MB_UNIT) ? 2 : 1, (double)benchResult.cSpeed / MB_UNIT,
        ...
        if (displayLevel == 1)
        { /* hidden display mode -q, used by python speed benchmark */
            ...
            if (adv->additionalParam)
            {
                OUTPUT("-%-3i%11i (%5.3f) %6.2f MB/s %6.1f MB/s  %s (param=%d)\n", cLevel, (int)cSize, ratio, cSpeed,
                       dSpeed, displayName, adv->additionalParam);
            }
            else
            {
...

{
    ...
    int const allocationincomplete = !srcPtrs || !srcSizes || !cPtrs || !cSizes || !cCapacities || !resPtrs ||
                                     !resSizes || !timeStateCompress || !timeStateDecompress || !cctx || !dctx ||
                                     !compressedBuffer || !resultBuffer;

    if (!allocationincomplete && !dstParamsError)
    {
        outcome = BMK_benchMemAdvancedNoAlloc(
            srcPtrs, srcSizes, cPtrs, cCapacities, cSizes, resPtrs, resSizes, &resultBuffer, compressedBuffer,
            maxCompressedSize, timeStateCompress, timeStateDecompress, srcBuffer, srcSize, fileSizes, nbFiles, cLevel,
            comprParams, dictBuffer, dictBufferSize, cctx, dctx, displayLevel, displayName, adv);
    }
                                ...
                                const char *displayName)
{
    ...
    return BMK_benchMemAdvanced(srcBuffer, srcSize, NULL, 0, fileSizes, nbFiles, cLevel, comprParams, dictBuffer,
                                dictBufferSize, displayLevel, displayName, &adv);
}

/* @return: 0 on success, !0 if error */
static int BMK_benchCLevels(const void *srcBuffer,
                            size_t benchedSize,
                            const size_t *fileSizes,
                            unsigned nbFiles,
                            int startCLevel,
                            ...
                            BMK_advancedParams_t const *const adv)
{
    ...
    for (level = startCLevel; level <= endCLevel; level++)
    {
        BMK_benchOutcome_t res =
            BMK_benchMemAdvanced(srcBuffer, benchedSize, NULL, 0, fileSizes, nbFiles, level, comprParams, dictBuffer,
                                 dictBufferSize, displayLevel, displayName, adv);
        if (!BMK_isSuccessful_benchOutcome(res))
            return 1;
    }
    return 0;
}
                      ...
                      const BMK_advancedParams_t *adv)
{
    ...
    else
    {
        ...
        formatString_u(nameBuff, sizeof(nameBuff), "Synthetic %u%%", (unsigned)(compressibility * 100));
    }

    /* Bench */
    {
        int res = BMK_benchCLevels(srcBuffer, benchedSize, &benchedSize, 1, startingCLevel, endCLevel,
                                   compressionParams, NULL, 0, /* dictionary */
                         ...
                         int displayLevel)
{
    ...
    for (n = 0; n < nbFiles; n++)
    {
        ...
        U64 fileSize = UTIL_getFileSize(filename); /* last file may be shortened */
...

        {
            ...
            if (f == NULL)
            {
                RETURN_ERROR_INT(10, "cannot open file %s", filename);
            }
            OUTPUTLEVEL(2, "Loading %s...       \r", filename);
            {
                ...
                {
                    fclose(f);
                    RETURN_ERROR_INT(11, "invalid read %s", filename);
                }
                pos += readSize;
            }
                           ...
                           const BMK_advancedParams_t *adv)
{
    ...
    if (dictFileName != NULL)
    {
        ...
        {
            free(fileSizes);
            DISPLAYLEVEL(1, "not enough memory for dictionary (%u bytes)", (unsigned)dictBufferSize);
            return 19;
        }
    ...
    /* Bench */
    {
        ...
        formatString_u(mfName, sizeof(mfName), " %u files", nbFiles);
        {
            ...
            res = BMK_benchCLevels(srcBuffer, benchedSize, fileSizes, nbFiles, startCLevel, endCLevel,
                                   compressionParams, dictBuffer, dictBufferSize, displayLevel, displayName, adv);
        }
    }
...
{
    BMK_advancedParams_t const adv = BMK_initAdvancedParams();
    return BMK_benchFilesAdvanced(fileNamesTable, nbFiles, dictFileName, cLevel, cLevel, compressionParams,
                                  displayLevel, &adv);
}
