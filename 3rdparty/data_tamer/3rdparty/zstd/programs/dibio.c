 ...
 */
static int DiB_loadFiles(void *buffer,
                         ...
                         int displayLevel)
{
    ...
    while (nbSamplesLoaded < sstSize && fileIndex < nbFiles)
    {
        ...
        fileDataLoaded =
            targetChunkSize > 0 ? (size_t)MIN(fileSize, (S64)targetChunkSize) : (size_t)MIN(fileSize, SAMPLESIZE_MAX);
                       ...
                       unsigned memLimit)
{
    ...
    if ((S64)loadedSize < fs.totalSizeToLoad)
        DISPLAYLEVEL(1, "Training samples set too large (%u MB); training on %u MB only...\n",
                     (unsigned)(fs.totalSizeToLoad / (1 MB)), (unsigned)(loadedSize / (1 MB)));

    /* Load input buffer */
    nbSamplesLoaded = DiB_loadFiles(srcBuffer, &loadedSize, sampleSizes, fs.nbSamples, fileNamesTable, nbFiles,
                                    chunkSize, displayLevel);

    {
        ...
        if (params)
        {
            ...
            dictSize =
                ZDICT_trainFromBuffer_legacy(dictBuffer, maxDictSize, srcBuffer, sampleSizes, nbSamplesLoaded, *params);
        }
        else if (coverParams)
        {
            if (optimize)
            {
                dictSize = ZDICT_optimizeTrainFromBuffer_cover(dictBuffer, maxDictSize, srcBuffer, sampleSizes,
                                                               nbSamplesLoaded, coverParams);
            ...
            {
                dictSize = ZDICT_trainFromBuffer_cover(dictBuffer, maxDictSize, srcBuffer, sampleSizes, nbSamplesLoaded,
                                                       *coverParams);
            }
        }
        else if (fastCoverParams != NULL)
        {
            if (optimize)
            {
                dictSize = ZDICT_optimizeTrainFromBuffer_fastCover(dictBuffer, maxDictSize, srcBuffer, sampleSizes,
                                                                   nbSamplesLoaded, fastCoverParams);
                if (!ZDICT_isError(dictSize))
                {
            ...
            {
                dictSize = ZDICT_trainFromBuffer_fastCover(dictBuffer, maxDictSize, srcBuffer, sampleSizes,
                                                           nbSamplesLoaded, *fastCoverParams);
            }
        }
        else
