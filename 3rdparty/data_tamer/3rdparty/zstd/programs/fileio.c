...
static FILE *FIO_openDstFile(FIO_ctx_t *fCtx,
 ...
 */
static size_t
FIO_setDictBufferMalloc(FIO_Dict_t *dict, const char *fileName, FIO_prefs_t *const prefs, stat_t *dictFileStat)
{
    FILE *fileHandle;
    U64 fileSize;
    ...
    *bufferPtr = NULL;
    ...
    fileSize = UTIL_getFileSizeStat(dictFileStat);
    {
        ...
        {
            EXM_THROW(34, "Dictionary file %s is too large (> %u bytes)", fileName,
...
static size_t
FIO_setDictBufferMMap(FIO_Dict_t *dict, const char *fileName, FIO_prefs_t *const prefs, stat_t *dictFileStat)
{
    int fileHandle;
    U64 fileSize;
    void **bufferPtr = &dict->dictBuffer;
...
FIO_setDictBufferMMap(FIO_Dict_t *dict, const char *fileName, FIO_prefs_t *const prefs, stat_t *dictFileStat)
{
    ...
    *bufferPtr = MapViewOfFile(mapping, FILE_MAP_READ, 0, 0,
                               (DWORD)fileSize); /* we can only cast to DWORD here because dictSize <= 2GB */
    if (*bufferPtr == NULL)
...
static void FIO_initDict(FIO_Dict_t *dict,
                         const char *fileName,
                         FIO_prefs_t *const prefs,
                         stat_t *dictFileStat,
 ...
 * This function handles logic when processing multiple files with -o or -c, displaying the appropriate
 * warnings/prompts. Returns 1 if the console should abort, 0 if console should proceed.
 *
 ...
 */
static int
FIO_multiFilesConcatWarning(const FIO_ctx_t *fCtx, FIO_prefs_t *prefs, const char *outFileName, int displayLevelCutoff)
{
    if (fCtx->hasStdoutOutput)
    {
        if (prefs->removeSrcFile)
            /* this should not happen ; hard fail, to protect user's data
             * note: this should rather be an assert(), but we want to be certain that user's data will not be wiped out
             * in case it nonetheless happen */
                                    ...
                                    ZSTD_compressionParameters comprParams)
{
    ...
    CHECK(ZSTD_CCtx_setParameter(
        ress.cctx, ZSTD_c_contentSizeFlag,
        prefs->contentSize)); /* always enable content size when available (note: supposed to be default) */
                                              ...
                                              U64 *readsize)
{
    ...
    {
        int const ret =
            deflateInit2(&strm, compressionLevel, Z_DEFLATED, 15 /* maxWindowLogSize */ + 16 /* gzip only */, 8,
    ...
    while (1)
    {
        ...
        else
        {
            DISPLAYUPDATE_PROGRESS("\rRead : %u / %u MB ==> %.2f%% ", (unsigned)(inFileSize >> 20),
                                   (unsigned)(srcFileSize >> 20), (double)outFileSize / (double)inFileSize * 100);
        }
    }
                                               ...
                                               U64 *readsize)
{
...

    {
        ...
        if (LZ4F_isError(headerSize))
            EXM_THROW(38, "zstd: %s: lz4 end of file generation failed : %s", srcFileName,
                                                ...
                                                U64 *readsize)
{
               ...
               || (directive == ZSTD_e_end && stillToFlush != 0))
        {
            ...
            if (prefs->adaptiveMode && UTIL_clockSpanMicro(lastAdaptTime) > adaptEveryMicro)
            {
                ...
                { /* only possible if nbWorkers >= 1 */
                    ...
                    if ((zfp.consumed ==
                         previous_zfp_update.consumed) /* no data compressed : no data available, or no more buffer to
                                                          compress to, OR compression is really slow (compression of a
                                                          single block is slower than update rate)*/
                    ...
                    if ((newlyProduced >
                         (newlyFlushed * 9 / 8)) /* compression produces more data than output can flush (though
                    ...
                    {
                        DISPLAYLEVEL(6,
                                     "compression faster than flush (%llu > %llu), and flushed was never slowed down "
                                     "by lack of production => slow down \n",
                ...
                if (zfp.currentJobID > lastJobID)
                {
                    ...
                    { /* warm up period, to fill all workers */
                        if (inputBlocked <= 0)
                        {
                        ...
                        else if (speedChange == noChange)
                        {
                            ...
                            DISPLAYLEVEL(
                                ...
                                (unsigned)newlyIngested, (unsigned)newlyConsumed, (unsigned)newlyFlushed,
                                (unsigned)newlyProduced);
                            if ((inputBlocked > inputPresented / 8) /* input is waiting often, because input buffers is
                                                                       full : compression or output too slow */
            ...
            if (SHOULD_DISPLAY_PROGRESS() && READY_FOR_UPDATE())
            {
                ZSTD_frameProgression const zfp = ZSTD_getFrameProgression(ress.cctx);
                double const cShare =
                    (double)zfp.produced / (double)(zfp.consumed + !zfp.consumed /*avoid div0*/) * 100;
                ...
                if (g_display_prefs.displayLevel >= 3)
                {
                    ...
                    DISPLAY_PROGRESS("(L%i) Buffered:%5.*f%s - Consumed:%5.*f%s - Compressed:%5.*f%s => %.2f%% ",
                                     compressionLevel, buffered_hrs.precision, buffered_hrs.value, buffered_hrs.suffix,
                                     consumed_hrs.precision, consumed_hrs.value, consumed_hrs.suffix,
                                     produced_hrs.precision, produced_hrs.value, produced_hrs.suffix, cShare);
                }
                else
                {
                    ...
                    if (fCtx->nbFilesTotal > 1)
                    {
                        ...
                        if (srcFileNameSize > 18)
                        {
                            ...
                            DISPLAY_PROGRESS("Compress: %u/%u files. Current: ...%s ", fCtx->currFileIdx + 1,
                                             fCtx->nbFilesTotal, truncatedSrcFileName);
                        }
                        else
                        {
                            DISPLAY_PROGRESS("Compress: %u/%u files. Current: %*s ", fCtx->currFileIdx + 1,
                                             fCtx->nbFilesTotal, (int)(18 - srcFileNameSize), srcFileName);
                        }
                    }
    ...
    {
        EXM_THROW(27, "Read error : Incomplete read : %llu / %llu B", (unsigned long long)*readsize,
                                         ...
                                         int compressionLevel)
{
    ...
    case FIO_zstdCompression:
        compressedfilesize =
            FIO_compressZstdFrame(fCtx, prefs, &ress, srcFileName, fileSize, compressionLevel, &readsize);
        break;
...
#ifdef ZSTD_LZMACOMPRESS
        compressedfilesize = FIO_compressLzmaFrame(&ress, srcFileName, fileSize, compressionLevel, &readsize,
    ...
    if (FIO_shouldDisplayFileSummary(fCtx))
    {
        ...
        if (readsize == 0)
        {
            DISPLAY_SUMMARY("%-20s :  (%6.*f%s => %6.*f%s, %s) \n", srcFileName, hr_isize.precision, hr_isize.value,
                            hr_isize.suffix, hr_osize.precision, hr_osize.value, hr_osize.suffix, dstFileName);
        }
        else
        {
            DISPLAY_SUMMARY("%-20s :%6.2f%%   (%6.*f%s => %6.*f%s, %s) \n", srcFileName,
                            (double)compressedfilesize / (double)readsize * 100, hr_isize.precision, hr_isize.value,
                            hr_isize.suffix, hr_osize.precision, hr_osize.value, hr_osize.suffix, dstFileName);
        }
    }
                                        ...
                                        int compressionLevel)
{
    ...
    if (AIO_WritePool_getFile(ress.writeCtx) == NULL)
    {
        ...
        if (strcmp(srcFileName, stdinmark) && strcmp(dstFileName, stdoutmark) && UTIL_isRegularFileStat(srcFileStat))
        {
            transferStat = 1;
    ...
    if (closeDstFile)
    {
        ...
        {
            FIO_removeFile(
                dstFileName); /* remove compression artefact; note don't do anything special if remove() fails */
        }
    }
                                        ...
                                        int compressionLevel)
{
    ...
    if (strcmp(srcFileName, stdinmark))
    {
        if (UTIL_stat(srcFileName, &srcFileStat))
        {
            ...
            if (ress.dictFileName != NULL &&
                UTIL_isSameFileStat(srcFileName, ress.dictFileName, &srcFileStat, &ress.dictFileStat))
            {
    ...
    result = FIO_compressFilename_dstFile(fCtx, prefs, ress, dstFileName, srcFileName, &srcFileStat, compressionLevel);
...
{
    static const char *formatOptions[5] = {ZSTD_EXTENSION, GZ_EXTENSION, XZ_EXTENSION, LZMA_EXTENSION, LZ4_EXTENSION};
...
{
    cRess_t ress =
        FIO_createCResources(prefs, dictFileName, UTIL_getFileSize(srcFileName), compressionLevel, comprParams);
                                  ...
                                  ZSTD_compressionParameters comprParams)
{
    ...
    { /* output into a single destination (stdout typically) */
        ...
        else
        {
            ...
            {
                status = FIO_compressFilename_srcFile(fCtx, prefs, ress, outFileName,
                                                      inFileNamesTable[fCtx->currFileIdx], compressionLevel);
                if (!status)
    ...
    if (FIO_shouldDisplayMultipleFileSummary(fCtx))
    {
        ...
        if (fCtx->totalBytesInput == 0)
        {
            DISPLAY_SUMMARY("%3d files compressed : (%6.*f%4s => %6.*f%4s)\n", fCtx->nbFilesProcessed,
                            hr_isize.precision, hr_isize.value, hr_isize.suffix, hr_osize.precision, hr_osize.value,
                            hr_osize.suffix);
        }
        else
        {
            DISPLAY_SUMMARY("%3d files compressed : %.2f%% (%6.*f%4s => %6.*f%4s)\n", fCtx->nbFilesProcessed,
                            (double)fCtx->totalBytesOutput / ((double)fCtx->totalBytesInput) * 100, hr_isize.precision,
                            hr_isize.value, hr_isize.suffix, hr_osize.precision, hr_osize.value, hr_osize.suffix);
        }
    }
...
static unsigned long long FIO_decompressZstdFrame(FIO_ctx_t *const fCtx,
                                                  dRess_t *ress,
                                                  ...
                                                  U64 alreadyDecoded) /* for multi-frames streams */
{
    ...
    while (1)
    {
        ...
        if (fCtx->nbFilesTotal > 1)
        {
            DISPLAYUPDATE_PROGRESS("\rDecompress: %2u/%2u files. Current: %s : %.*f%s...    ", fCtx->currFileIdx + 1,
                                   fCtx->nbFilesTotal, srcFName20, hrs.precision, hrs.value, hrs.suffix);
        }
        else
        {
            DISPLAYUPDATE_PROGRESS("\r%-20.20s : %.*f%s...     ", srcFName20, hrs.precision, hrs.value, hrs.suffix);
        }
...
static unsigned long long FIO_decompressLzmaFrame(dRess_t *ress, const char *srcFileName, int plain_lzma)
{
    unsigned long long outFileSize = 0;
    lzma_stream strm = LZMA_STREAM_INIT;
    lzma_action action = LZMA_RUN;
...
static unsigned long long FIO_decompressLz4Frame(dRess_t *ress, const char *srcFileName)
{
    ...
    for (; nextToLoad;)
    {
        ...
        if (!ress->readCtx->srcBufferLoaded)
            break; /* reached end of file */

        while ((pos < ress->readCtx->srcBufferLoaded) || fullBufferDecoded)
        { /* still to read, or still to flush */
            /* Decode Input (at least partially) */
            size_t remaining = ress->readCtx->srcBufferLoaded - pos;
            decodedBytes = writeJob->bufferSize;
            nextToLoad = LZ4F_decompress(dCtx, writeJob->buffer, &decodedBytes,
                                         (char *)(ress->readCtx->srcBuffer) + pos, &remaining, NULL);
                                ...
                                const char *srcFileName)
{
    ...
    for (;;)
    {
        ...
        {
            if (readSomething == 0)
            { /* srcFile is empty (which is invalid) */
        ...
        { /* not enough input to check magic number */
            if (passThrough)
            {
...
#else
            DISPLAYLEVEL(
                1, "zstd: %s: xz/lzma file cannot be uncompressed (zstd compiled without HAVE_LZMA) -- ignored \n",
                srcFileName);
            return 1;
#endif
        }
    ...
    for (;;)
    {
        ...
        }
        {
            ...
            if (magicNumber == ZSTD_MAGICNUMBER)
            {
                ...
                ERROR_IF(ZSTD_getFrameHeader(&header, headerBuffer, numBytesRead) != 0, info_frame_error,
                         "Error: could not decode frame header");
                if (info->dictID != 0 && info->dictID != header.dictID)
                {
                    DISPLAY("WARNING: File contains multiple frames with different dictionary IDs. Showing dictID 0 "
                            "instead");
                    info->dictID = 0;
                }
                else
                ...
                /* move to the end of the frame header */
                {
                    ...
                    ERROR_IF(fseek(srcFile, ((long)headerSize) - ((long)numBytesRead), SEEK_CUR) != 0, info_frame_error,
                ...
                /* skip all blocks in the frame */
                {
                    ...
                    do
                    {
                        ...
                        ERROR_IF(fread(blockHeaderBuffer, 1, 3, srcFile) != 3, info_frame_error,
                                 "Error while reading block header");
                        {
                            ...
                            ERROR_IF(fseek(srcFile, blockSize, SEEK_CUR) != 0, info_frame_error,
                ...
                /* check if checksum is used */
                {
                    ...
                    {
                        info->usesCheck = 1;
                        ERROR_IF(fread(info->checksum, 1, 4, srcFile) != 4, info_frame_error,
            ...
            else if ((magicNumber & ZSTD_MAGIC_SKIPPABLE_MASK) == ZSTD_MAGIC_SKIPPABLE_START)
            {
                ...
                ERROR_IF(LONG_SEEK(srcFile, seek, SEEK_CUR) != 0, info_frame_error,
...
static void displayInfo(const char *inFileName, const fileInfo_t *info, int displayLevel)
{
    ...
    double const ratio =
        (info->compressedSize == 0) ? 0 : ((double)info->decompressedSize) / (double)info->compressedSize;
    ...
    if (displayLevel <= 2)
    {
        if (!info->decompUnavailable)
        {
            DISPLAYOUT("%6d  %5d  %6.*f%4s  %8.*f%4s  %5.3f  %5s  %s\n",
                       ...
                       decompressed_hrs.precision, decompressed_hrs.value, decompressed_hrs.suffix, ratio, checkString,
                       inFileName);
        }
        else
        {
            DISPLAYOUT("%6d  %5d  %6.*f%4s                       %5s  %s\n",
                       info->numSkippableFrames + info->numActualFrames, info->numSkippableFrames,
                       compressed_hrs.precision, compressed_hrs.value, compressed_hrs.suffix, checkString, inFileName);
        }
    }
    else
    {
        ...
        DISPLAYOUT("Compressed Size: %.*f%s (%llu B)\n", compressed_hrs.precision, compressed_hrs.value,
                   compressed_hrs.suffix, (unsigned long long)info->compressedSize);
        if (!info->decompUnavailable)
        {
            DISPLAYOUT("Decompressed Size: %.*f%s (%llu B)\n", decompressed_hrs.precision, decompressed_hrs.value,
                       decompressed_hrs.suffix, (unsigned long long)info->decompressedSize);
            DISPLAYOUT("Ratio: %.4f\n", ratio);
        }

        if (info->usesCheck && info->numActualFrames == 1)
        {
            DISPLAYOUT("Check: %s %02x%02x%02x%02x\n", checkString, info->checksum[3], info->checksum[2],
...
int FIO_listMultipleFiles(unsigned numFiles, const char **filenameTable, int displayLevel)
{
    /* ensure no specified input is stdin (needs fseek() capability) */
    {
        ...
        {
            ERROR_IF(!strcmp(filenameTable[u], stdinmark), 1,
    ...
    }
    {
        ...
        { /* display total */
            ...
            double const ratio =
                (total.compressedSize == 0) ? 0 : ((double)total.decompressedSize) / (double)total.compressedSize;
            const char *const checkString = (total.usesCheck ? "XXH64" : "");
            DISPLAYOUT("----------------------------------------------------------------- \n");
            if (total.decompUnavailable)
            {
                DISPLAYOUT("%6d  %5d  %6.*f%4s                       %5s  %u files\n",
                           total.numSkippableFrames + total.numActualFrames, total.numSkippableFrames,
                           compressed_hrs.precision, compressed_hrs.value, compressed_hrs.suffix, checkString,
            ...
            else
            {
                DISPLAYOUT("%6d  %5d  %6.*f%4s  %8.*f%4s  %5.3f  %5s  %u files\n",
                           total.numSkippableFrames + total.numActualFrames, total.numSkippableFrames,
                           compressed_hrs.precision, compressed_hrs.value, compressed_hrs.suffix,
                           decompressed_hrs.precision, decompressed_hrs.value, decompressed_hrs.suffix, ratio,
