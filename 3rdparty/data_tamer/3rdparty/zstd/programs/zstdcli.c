...
static void usageAdvanced(const char *programName)
{
    ...
    DISPLAYOUT("  --single-thread               Share a single thread for I/O and compression (slightly different than "
               "`-T1`).\n");
    ...
    DISPLAYOUT("  --size-hint=#                 Optimize compression parameters for streaming input of approximately "
               "size #.\n");
...
#endif
    {
        ...
        if (exeNameMatch(programName, ZSTD_CAT) || exeNameMatch(programName, ZSTD_ZCAT) ||
...
static unsigned parseFastCoverParameters(const char *stringPtr, ZDICT_fastCover_params_t *params)
{
    ...
    if (stringPtr[0] != 0)
        return 0;
    DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\nshrink=%u\n", params->k, params->d,
                 params->f, params->steps, (unsigned)(params->splitPoint * 100), params->accel,
...
static unsigned parseAdaptParameters(const char *stringPtr, int *adaptMinPtr, int *adaptMaxPtr)
{
    for (;;)
    {
        ...
        {
            *adaptMinPtr = readIntFromChar(&stringPtr);
            if (stringPtr[0] == ',')
            {
                stringPtr++;
                continue;
        ...
        {
            *adaptMaxPtr = readIntFromChar(&stringPtr);
            if (stringPtr[0] == ',')
            {
                stringPtr++;
                continue;
...
static void printActualCParams(const char *filename,
                               ...
                               const ZSTD_compressionParameters *cParams)
{
    ...
    DISPLAY("--zstd=wlog=%d,clog=%d,hlog=%d,slog=%d,mml=%d,tlen=%d,strat=%d\n", actualCParams.windowLog,
            actualCParams.chainLog, actualCParams.hashLog, actualCParams.searchLog, actualCParams.minMatch,
            actualCParams.targetLength, actualCParams.strategy);
}
...
{
    int argNb, followLinks = 0, allowBlockDevices = 0, forceStdin = 0, forceStdout = 0, hasStdout = 0, ldmFlag = 0,
               main_pause = 0, adapt = 0, adaptMin = MINCLEVEL, adaptMax = MAXCLEVEL, rsyncable = 0,
               nextArgumentsAreFiles = 0, operationResult = 0, separateFiles = 0, setRealTimePrio = 0, singleThread = 0,
               defaultLogicalCores = 0, showDefaultCParams = 0, ultra = 0, contentSize = 1, removeSrcFile = 0;
    ...
    {
        DISPLAYLEVEL(1, "zstd: allocation error \n");
        exit(1);
    }
    ...
    for (argNb = 1; argNb < argCount; argNb++)
    {
        ...
        if (argument[0] == '-')
        {
            ...
            if (argument[1] == '-')
            {
                ...
                if (!strcmp(argument, "--asyncio"))
                {
                    FIO_setAsyncIOFlag(prefs, 1);
                    continue;
                }
                if (!strcmp(argument, "--no-asyncio"))
                {
                ...
                if (!strcmp(argument, "--show-default-cparams"))
                {
                ...
                if (!strcmp(argument, "--no-row-match-finder"))
                {
                    useRowMatchFinder = ZSTD_ps_disable;
                    continue;
                }
                if (!strcmp(argument, "--row-match-finder"))
                {
                ...
                if (!strcmp(argument, "--mmap-dict"))
                {
                    mmapDict = ZSTD_ps_enable;
                    continue;
                }
                if (!strcmp(argument, "--no-mmap-dict"))
                {
                ...
                if (!strcmp(argument, "--fake-stdin-is-console"))
                {
                    UTIL_fakeStdinIsConsole();
                    continue;
                }
                if (!strcmp(argument, "--fake-stdout-is-console"))
                {
                    UTIL_fakeStdoutIsConsole();
                    continue;
                }
                if (!strcmp(argument, "--fake-stderr-is-console"))
                {
                    UTIL_fakeStderrIsConsole();
                    continue;
                }
                if (!strcmp(argument, "--trace-file-stat"))
                {
                    UTIL_traceFileStat();
                    continue;
                }
                ...
                if (!strcmp(argument, "--max"))
                {
                    ...
                    ultra = 1;
                    ldmFlag = 1;
                    setMaxCompression(&compressionParams);
                    continue;
                }
    ...
    if (operation == zom_bench)
    {
        ...
        if (benchParams.mode == BMK_decodeOnly)
            cLevel = cLevelLast = 0;
        ...
        if (filenames->tableSize > 0)
        {
            if (separateFiles)
            {
                ...
                {
                    operationResult =
                        BMK_benchFilesAdvanced(&filenames->fileNames[i], 1, dictFileName, cLevel, cLevelLast,
            ...
            {
                operationResult =
                    BMK_benchFilesAdvanced(filenames->fileNames, (unsigned)filenames->tableSize, dictFileName, cLevel,
                                           cLevelLast, &compressionParams, g_displayLevel, &benchParams);
            }
        }
        else
        {
            operationResult = BMK_syntheticTest(compressibility, cLevel, cLevelLast, &compressionParams, g_displayLevel,
                                                &benchParams);
        }
    ...
    if (operation == zom_train)
    {
        ...
        if (dict == cover)
        {
            ...
            operationResult =
                DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames, (int)filenames->tableSize, blockSize,
        ...
        else if (dict == fastCover)
        {
            ...
            operationResult =
                DiB_trainFromFiles(outFileName, maxDictSize, filenames->fileNames, (int)filenames->tableSize, blockSize,
                                   NULL, NULL, &fastCoverParams, optimize, memLimit);
        }
        else
        {
    ...
    if (!forceStdin && (UTIL_searchFileNamesTable(filenames, stdinmark) != -1) && UTIL_isConsole(stdin))
    {
        DISPLAYLEVEL(1, "stdin is a console, aborting\n");
        CLEAN_RETURN(1);
    }
    if ((!outFileName || !strcmp(outFileName, stdoutmark)) && UTIL_isConsole(stdout) &&
        (UTIL_searchFileNamesTable(filenames, stdinmark) != -1) && !forceStdout && operation != zom_decompress)
    {
    ...
    if (operation == zom_compress)
    {
        ...
        if ((filenames->tableSize == 1) && outFileName)
            operationResult = FIO_compressFilename(fCtx, prefs, outFileName, filenames->fileNames[0], dictFileName,
                                                   cLevel, compressionParams);
        else
            operationResult =
                FIO_compressMultipleFilenames(fCtx, prefs, filenames->fileNames, outMirroredDirName, outDirName,
        ...
        (void)streamSrcSize;
        (void)srcSizeHint;
        (void)ZSTD_strategyMap;
        (void)useRowMatchFinder;
        (void)cType;
    ...
    { /* decompression or test */
        ...
        {
            operationResult = FIO_decompressMultipleFilenames(fCtx, prefs, filenames->fileNames, outMirroredDirName,
                                                              outDirName, outFileName, dictFileName);
        }
#else
