...
static unsigned AIO_fwriteSparse(FILE *file,
                                 const void *buffer,
                                 size_t bufferSize,
    ...
    while (ptrT < bufferTEnd)
    {
        ...
        { /* not all 0s */
            ...
            if (fwrite(ptrT + nb0T, sizeof(size_t), nbNon0ST, file) != nbNon0ST)
                EXM_THROW(93, "Write error : cannot write block : %s", strerror(errno));
        }
        ptrT += seg0SizeT;
    }
        ...
        if (bufferSize & maskT)
        {
            ...
            if (restPtr != restEnd)
            {
                ...
                if (fwrite(restPtr, 1, restSize, file) != restSize)
                    EXM_THROW(95, "Write error : cannot write end of decoded block : %s", strerror(errno));
                storedSkips = 0;
            }
        }
    }
