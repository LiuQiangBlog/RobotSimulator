...
static unsigned g_displayLevel = DEFAULT_DISPLAY_LEVEL; /* 0 : no display;   1: errors;   2 : + result + interaction +
...
static void BMK_setNotificationLevel(unsigned level)
{
    g_displayLevel = level;
}

static void BMK_setAdditionalParam(int additionalParam)
{
        ...
        while (!cCompleted | !dCompleted)
        {
            ...
            { /* still some time to do compression tests */
                ...
                if (compressor == BMK_ZSTD)
                {
                    ...
                    ZSTD_CDict *const cdict = ZSTD_createCDict_advanced(dictBuffer, dictBufferSize, ZSTD_dlm_byRef,
                                                                        ZSTD_dct_auto, zparams.cParams, cmem);
                    if (cdict == NULL)
                    ...
                    do
                    {
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                        {
                            if (dictBufferSize)
                            {
                                rSize = ZSTD_compress_usingCDict(ctx, blockTable[blockNb].cPtr,
                                                                 blockTable[blockNb].cRoom, blockTable[blockNb].srcPtr,
                                                                 blockTable[blockNb].srcSize, cdict);
                            }
                            else
                            {
                                rSize =
                                    ZSTD_compressCCtx(ctx, blockTable[blockNb].cPtr, blockTable[blockNb].cRoom,
                ...
                else if (compressor == BMK_ZSTD_STREAM)
                {
                    ...
                    if (!cctxParams)
                        EXM_THROW(1, "ZSTD_createCCtxParams() allocation failure");
                    if (zbc == NULL)
                    ...
                    do
                    {
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                        {
                            ...
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_CCtx_reset() failed : %s", ZSTD_getErrorName(rSize));
                            rSize = ZSTD_CCtx_setPledgedSrcSize(zbc, blockTable[blockNb].srcSize);
                            if (ZSTD_isError(rSize))
                                EXM_THROW(1, "ZSTD_CCtx_setPledgedSrcSize() failed : %s", ZSTD_getErrorName(rSize));
                         ...
                         compressor == BMK_ZLIB_REUSE)
                {
                    ...
                    do
                    {
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                        {
                            ...
                            if (ret != Z_STREAM_END)
                                EXM_THROW(1, "deflate failure ret=%d srcSize=%d", ret,
            ...
            DISPLAYLEVEL(2, "%2s-%-17.17s :%10u ->%10u (%5.3f),%6.1f MB/s\r", marks[markNb], displayName,
                         (unsigned)srcSize, (unsigned)cSize, ratio, (double)srcSize / (double)fastestC);
            ...
            if (!dCompleted)
            {
                ...
                if (compressor == BMK_ZSTD)
                {
                    ...
                    do
                    {
                        ...
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                        {
                            size_t const regenSize = ZSTD_decompress_usingDDict(
                                dctx, blockTable[blockNb].resPtr, blockTable[blockNb].srcSize, blockTable[blockNb].cPtr,
                                blockTable[blockNb].cSize, ddict);
                ...
                else if (compressor == BMK_ZSTD_STREAM)
                {
                    ...
                    if (ZSTD_isError(rSize))
                        EXM_THROW(1, "ZSTD_DCtx_reset() failed : %s", ZSTD_getErrorName(rSize));
                    rSize = ZSTD_DCtx_loadDictionary(zbd, dictBuffer, dictBufferSize);
                    if (ZSTD_isError(rSize))
                        EXM_THROW(1, "ZSTD_DCtx_loadDictionary() failed : %s", ZSTD_getErrorName(rSize));
                    do
                    {
                        U32 blockNb;
                        for (blockNb = 0; blockNb < nbBlocks; blockNb++)
            ...
            markNb = (markNb + 1) % NB_MARKS;
            DISPLAYLEVEL(2, "%2s-%-17.17s :%10u ->%10u (%5.3f),%6.1f MB/s ,%6.1f MB/s\r", marks[markNb], displayName,
                         (unsigned)srcSize, (unsigned)cSize, ratio, (double)srcSize / (double)fastestC,
                            ...
                            size_t dictBufferSize)
{
    ...
    {
        BMK_benchMem(srcBuffer, benchedSize, displayName, l, fileSizes, nbFiles, dictBuffer, dictBufferSize,
                     BMK_ZSTD_STREAM);
    }
...
static void
BMK_benchFileTable(const char **fileNamesTable, unsigned nbFiles, const char *dictFileName, int cLevel, int cLevelLast)
{
    void *srcBuffer;
    size_t benchedSize;
    ...
    snprintf(mfName, sizeof(mfName), " %u files", nbFiles);
    {
        ...
        BMK_benchCLevel(srcBuffer, benchedSize, displayName, cLevel, cLevelLast, fileSizes, nbFiles, dictBuffer,
                        dictBufferSize);
    }
...
static int
BMK_benchFiles(const char **fileNamesTable, unsigned nbFiles, const char *dictFileName, int cLevel, int cLevelLast)
{
...
int main(int argCount, char **argv)
{
    ...
    {
        size_t pos;
        for (pos = strlen(programName); pos > 0; pos--)
        {
            if (programName[pos] == '/')
            {
                pos++;
