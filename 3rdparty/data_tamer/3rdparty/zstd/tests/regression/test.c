...
static struct option long_options[] = {
    {"cache", required_argument, NULL, 'c'}, {"output", required_argument, NULL, 'o'},
    {"zstd", required_argument, NULL, 'z'},  {"config", required_argument, NULL, 128},
    {"data", required_argument, NULL, 129},  {"method", required_argument, NULL, 130},
    {"diff", required_argument, NULL, 'd'},  {"help", no_argument, NULL, 'h'},
};
...
static void print_help(void)
{
    ...
    for (size_t i = 0; i < nargs; ++i)
    {
        if (long_options[i].val < 128)
        {
            ...
            fprintf(stderr, "--%s / -%c \t- %s %s\n", long_options[i].name, long_options[i].val, long_extras[i].help,
...
void tprint_names(FILE *results, char const *data_name, char const *config_name, char const *method_name)
{
    ...
    tprintf(results, "%s, %*s%s, %*s%s, %*s", data_name, data_padding, "", config_name, config_padding, "", method_name,
            method_padding, "");
}
    ...
    for (size_t method = 0; methods[method] != NULL; ++method)
    {
        ...
        for (size_t datum = 0; data[datum] != NULL; ++datum)
        {
            ...
            for (size_t config = 0; configs[config] != NULL; ++config)
            {
                ...
                result_t const result = methods[method]->compress(state, configs[config]);
                if (result_is_skip(result))
                    continue;
                tprint_names(results, data[datum]->name, configs[config]->name, methods[method]->name);
...
static int diff_results(char const *actual_file, char const *expected_file)
{
    ...
    if (ret != 0)
    {
        fprintf(stderr, "actual results '%s' does not match expected results '%s'\n", actual_file, expected_file);
    }
    else
    {
