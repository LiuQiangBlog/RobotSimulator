...
#define CHECK_RET_Z(f)                                                             \
    {                                                                              \
        size_t const err = f;                                                      \
        CHECK_RET(err, ZSTD_isError(err), "%s : %s ", #f, ZSTD_getErrorName(err)); \
    }
...
static buffer_t FUZ_createDictionary(const void *src, size_t srcSize, size_t blockSize, size_t requestedDictSize)
{
    ...
    }
    {
        size_t const dictSize =
            ZDICT_trainFromBuffer(dict.start, requestedDictSize, src, blockSizes, (unsigned)nbBlocks);
        free(blockSizes);
...
static size_t
SEQ_roundTrip(ZSTD_CCtx *cctx, ZSTD_DCtx *dctx, XXH64_state_t *xxh, void *data, size_t size, ZSTD_EndDirective endOp)
{
...
static size_t SEQ_generateRoundTrip(ZSTD_CCtx *cctx,
                                    ZSTD_DCtx *dctx,
                                    XXH64_state_t *xxh,
                                    SEQ_stream *seq,
                                    SEQ_gen_type type,
...
static int basicUnitTests(U32 seed, double compressibility, int bigTests)
{
    ...
    DISPLAYLEVEL(3, "test%3i : estimate CStream size : ", testNb++);
    {
        ...
        size_t const cdictSize =
            ZSTD_estimateCDictSize_advanced(dictSize, cParams, ZSTD_dlm_byCopy); /* uses ZSTD_initCStream_usingDict() */
    ...
    DISPLAYLEVEL(3, "test%3i : estimate CStream size using CCtxParams : ", testNb++);
    {
        ...
        CHECK_Z(cctxSize);
        if (cstreamSize <= cctxSize + 2 * ZSTD_BLOCKSIZE_MAX)
            goto _output_error;
    ...
    DISPLAYLEVEL(3, "test%3i : early decompression error if first bytes are incorrect : ", testNb++);
    {
        ...
        if (!ZSTD_isError(remaining))
            goto _output_error; /* should have errored out immediately (note: this does not test the exact error code)
                                 */
    }
    DISPLAYLEVEL(3, "OK \n");
    ...
    if (outBuff.pos != CNBufferSize)
        goto _output_error; /* should regenerate the same amount */
    if (inBuff.pos != cSize)
        DISPLAYLEVEL(4, "inBuff.pos != cSize : should have real all input ! \n");
    if (inBuff.pos != cSize)
        goto _output_error; /* should have read the entire frame */
    DISPLAYLEVEL(3, "OK \n");
    ...
    { /* skippable frame */
        ...
        if (!ZSTD_isError(r))
            DISPLAYLEVEL(4, "ZSTD_decompressStream should have triggered a no_forward_progress error \n");
        if (!ZSTD_isError(r))
            goto _output_error; /* should have triggered no_forward_progress error */
    }
    DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : maxBlockSize = 2KB : ", testNb++);
    {
        ...
        CHECK(streamingSize < singlePassSize + (1 << 18) + 3 * ZSTD_BLOCKSIZE_MAX,
              "Streaming doesn't use the right amount of memory");
        CHECK(streamingSize != streaming2KSize + 3 * (ZSTD_BLOCKSIZE_MAX - 2048),
              "ZSTD_d_blockSizeMax didn't save the right amount of memory");
        DISPLAYLEVEL(3, "| %u | %u | %u | ", (unsigned)singlePassSize, (unsigned)streaming2KSize,
                     (unsigned)streamingSize);
    ...
    /* Compression with ZSTD_c_stable{In,Out}Buffer */
    {
        ...
        /* @cctxSize1 : sizeof_CCtx when doing single-shot compression (no streaming) */
        {
            ...
            cSize3 = ZSTD_compress_advanced(cctx1, compressedBuffer, compressedBufferSize, CNBuffer, CNBufferSize, NULL,
                                            0, params);
            CHECK_Z(cSize3);
        ...
        DISPLAYLEVEL(
            3, "test%3i : ZSTD_compressStream2() with ZSTD_c_stableInBuffer and ZSTD_c_stableOutBuffer : ", testNb++);
        ...
        DISPLAYLEVEL(
            3, "test%3i : ZSTD_compressStream2() ZSTD_c_stableInBuffer and ZSTD_c_stableOutBuffer allocated size : ",
            testNb++);
        {
        ...
        /* stableSrc + streaming */
        DISPLAYLEVEL(3,
                     "test%3i : ZSTD_c_stableInBuffer compatibility with compressStream, flushStream and endStream : ",
                     testNb++);
        ...
        CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_stableInBuffer, 1));
        {
            ...
            CHECK(ZSTD_endStream(cctx, &outBuf) != 0, "compression should be successful and fully flushed");
            {
                ...
                CHECK_Z(decSize);
                CHECK(decSize != inputSize, "regenerated %u bytes, instead of %u", (unsigned)decSize,
                      (unsigned)inputSize);
        ...
        /* stableSrc + streaming */
        DISPLAYLEVEL(
            3, "test%3i : ZSTD_c_stableInBuffer compatibility with compressStream2, using different end directives : ",
            testNb++);
        CHECK_Z(ZSTD_initCStream(cctx, 1));
        CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_stableInBuffer, 1));
        {
            ...
            inBuf.size = nonZeroStartPos + inputSize;
            CHECK_Z(ZSTD_compressStream2(cctx, &outBuf, &inBuf, ZSTD_e_continue));
            CHECK(ZSTD_compressStream2(cctx, &outBuf, &inBuf, ZSTD_e_end) != 0,
                  "compression should be successful and fully flushed");
            {
                const void *const realSrcStart = (const char *)inBuf.src + nonZeroStartPos;
                void *const verifBuf = (char *)outBuf.dst + outBuf.pos;
                const size_t decSize = ZSTD_decompress(verifBuf, inputSize, outBuf.dst, outBuf.pos);
        ...
        {
            size_t const cctxSize = ZSTD_sizeof_CCtx(cctx);
            DISPLAYLEVEL(4, "cctxSize1=%u; cctxSize=%u; cctxSize2=%u : ", (unsigned)cctxSize1, (unsigned)cctxSize,
                         (unsigned)cctxSize2);
    ...
    DISPLAYLEVEL(3, "test%3i : ZSTD_decompressStream() with dictionary : ", testNb++);
    {
        ...
        CHECK_Z(ZSTD_DCtx_loadDictionary(dctx, dictionary.start, dictionary.filled));
        {
            ZSTD_outBuffer out = {decodedBuffer, decodedBufferSize, 0};
            ZSTD_inBuffer in = {compressedBuffer, cSize, 0};
            if (ZSTD_decompressStream(dctx, &out, &in) != 0)
                goto _output_error;
            if (in.pos != in.size)
                goto _output_error;
        }
    ...
    if (ZSTD_endStream(zc, &outBuff) != 0)
        goto _output_error;
    cSize = outBuff.pos;
    if (ZSTD_findDecompressedSize(compressedBuffer, cSize) != ZSTD_CONTENTSIZE_UNKNOWN)
        goto _output_error;
    DISPLAYLEVEL(3, "OK \n");
    ...
    inBuff.pos = 0;
    {
        ...
        if (compressResult != 0)
            goto _output_error; /* compression must be completed in a single round */
    }
    if (inBuff.pos != inBuff.size)
        goto _output_error; /* entire input should be consumed */
    {
    ...
    /* Complex multithreading + dictionary test */
    {
        ...
        DISPLAYLEVEL(3, "test%3i : compress %u bytes with multiple threads + dictionary : ", testNb++,
                     (unsigned)srcSize);
    ...
    }
    {
        ZSTD_compressionParameters const cParams = ZSTD_getCParams(
            1, 4 KB,
            dictionary
                .filled); /* intentionally lies on estimatedSrcSize, to push cdict into targeting a small window size */
        ZSTD_CDict *const cdict = ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef,
    ...
    DISPLAYLEVEL(3, "test%3i : check dictionary FSE tables can represent every code : ", testNb++);
    {
        ...
        cdict = ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef, ZSTD_dct_fullDict,
    ...
    DISPLAYLEVEL(3, "test%3i : Test offset == windowSize : ", testNb++);
    {
        ...
        for (windowLog = ZSTD_WINDOWLOG_MIN; windowLog <= kMaxWindowLog; ++windowLog)
        {
            ...
            cSize = ZSTD_compressSequences(zc, compressedBuffer, compressedBufferSize, sequences, kNbSequences, src,
                                           srcSize);
            CHECK_Z(cSize);
    ...
    { /* This test has a window size of 1024 bytes and consists of 3 blocks:
          ...
          3. a compressed block with no literals and 3 sequence commands:
              ...
              litlength = 0, offset = 24, match length = 3 (this one creates an overlength write of length
         2*WILDCOPY_OVERLENGTH - 3) litlength = 0, offset = 1021, match length = 3 (this one will try to read from
         overwritten data if the buffer is too small) */

        const char *testCase = "\x28\xB5\x2F\xFD\x04\x00\x4C\x00\x00\x10\x61\x61\x01\x00\x00\x2A"
    ...
    { /* This test consists of 3 blocks. Each block has one sequence.
          ...
          The order of values are ll, ml, of.
            - First block  : (10, 7, 13) (rle, rle, rle)
               - size of sequences section: 6 bytes (1 byte for nbSeq, 1 byte for encoding mode, 3 bytes for rle, 1 byte
         bitstream)
            - Second block : (10, 7, 1) (repeat, repeat, rle)
               - size of sequences section: 4 bytes (1 byte for nbSeq, 1 byte for encoding mode, 1 bytes for rle, 1 byte
         bitstream)
        ...
        unsigned char compressed[] = {0x28, 0xb5, 0x2f, 0xfd, 0x24, 0x3c, 0x35, 0x01, 0x00, 0xf0, 0x85, 0x08, 0xc2,
                                      0xc4, 0x70, 0xcf, 0xd7, 0xc0, 0x96, 0x7e, 0x4c, 0x6b, 0xa9, 0x8b, 0xbc, 0xc5,
                                      0xb6, 0xd9, 0x7f, 0x4c, 0xf1, 0x05, 0xa6, 0x54, 0xef, 0xac, 0x69, 0x94, 0x89,
                                      0x1c, 0x03, 0x44, 0x0a, 0x07, 0x00, 0xb4, 0x04, 0x80, 0x40, 0x0a, 0xa4};
        unsigned int compressedSize = 51;
        unsigned char decompressed[] = {0x85, 0x08, 0xc2, 0xc4, 0x70, 0xcf, 0xd7, 0xc0, 0x96, 0x7e, 0x85, 0x08,
        ...
        CHECK(ZSTD_decompressStream(zds, &outBuff, &inBuff) != 0, "Decompress did not reach the end of frame");
        ...
        CHECK(memcmp(outBuff.dst, decompressed, decompressedSize) != 0, "Decompressed data does not match");
    ...
    DISPLAYLEVEL(
        3, "test%3i : dictionary + uncompressible block + reusing tables checks offset table validity: ", testNb++);
    {
        ZSTD_CDict *const cdict =
            ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef, ZSTD_dct_fullDict,
                                      ZSTD_getCParams(3, 0, dictionary.filled), ZSTD_defaultCMem);
    ...
    DISPLAYLEVEL(3, "test%3i : Block-Level External Sequence Producer API: ", testNb++);
    {
        ...
        for (enableFallback = 0; enableFallback <= 1; enableFallback++)
        {
            ...
            EMF_testCase const testCases[] = {EMF_ONE_BIG_SEQ,      EMF_LOTS_OF_SEQS,   EMF_ZERO_SEQS,
                                              EMF_BIG_ERROR,        EMF_SMALL_ERROR,    EMF_INVALID_OFFSET,
                                              EMF_INVALID_MATCHLEN, EMF_INVALID_LITLEN, EMF_INVALID_LAST_LITS};

            ZSTD_ErrorCode const errorCodes[] = {ZSTD_error_no_error,
                                                 ZSTD_error_no_error,
            ...
            for (testCaseId = 0; testCaseId < numTestCases; testCaseId++)
            {
                ...
                int const testWithSequenceValidation = (testCases[testCaseId] == EMF_INVALID_OFFSET);
                ...
                else
                {
                    ...
                    CHECK(ZSTD_getErrorCode(res) != errorCodes[testCaseId], "EMF: Wrong error code: %s",
        ...
        CHECK_Z(ZSTD_CCtx_reset(zc, ZSTD_reset_session_and_parameters));
        {
            ...
            res = ZSTD_compress2(zc, dstBuf, dstBufSize, CNBuffer, CNBufferSize);
            CHECK(!ZSTD_isError(res), "EMF: Should have raised an error!");
            CHECK(ZSTD_getErrorCode(res) != ZSTD_error_parameter_combination_unsupported, "EMF: Wrong error code: %s",
                  ZSTD_getErrorName(res));
        }
    ...
    DISPLAYLEVEL(3, "test%3i : Testing sequence validation: ", testNb++);
    {
        ...
        /* Test minMatch >= 4, matchLength < 4 */
        {
            ...
            cSize = ZSTD_compressSequences(cctx, dst, dstSize, sequences, kNbSequences, src, srcSize);

            CHECK(!ZSTD_isError(cSize), "Should throw an error"); /* maxNbSeq is too small and an assert will fail */
            CHECK(ZSTD_getErrorCode(cSize) != ZSTD_error_externalSequences_invalid, "Wrong error code: %s",
                  ZSTD_getErrorName(cSize)); /* fails sequence validation */
    ...
    DISPLAYLEVEL(3, "test%3i : Testing large offset with small window size: ", testNb++);
    {
        ...
        /* Test large offset, small window size*/
        {
            ...
            cdict = ZSTD_createCDict_advanced(dictBuffer, largeDictSize, ZSTD_dlm_byRef, ZSTD_dct_rawContent, cParams,
                                              ZSTD_defaultCMem);
            ddict = ZSTD_createDDict_advanced(dictBuffer, largeDictSize, ZSTD_dlm_byRef, ZSTD_dct_rawContent,
                                              ZSTD_defaultCMem);
    ...
    DISPLAYLEVEL(3, "test%3i : Testing external sequence producer with static CCtx (one-shot): ", testNb++);
    {
        ...
        seqProdState = EMF_BIG_ERROR;
        {
            ...
            CHECK(ZSTD_getErrorCode(cResult) != ZSTD_error_sequenceProducer_failed, "EMF: Wrong error code: %s",
                  ZSTD_getErrorName(cResult));
        }
    ...
    for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime); testNb++)
    {
        ...
        else
        {
            ...
            U32 const cLevelCandidate =
                (FUZ_rand(&lseed) % ((unsigned)ZSTD_maxCLevel() - (MAX(testLog, dictLog) / 3))) + 1;
...
static size_t
setCCtxParameter(ZSTD_CCtx *zc, ZSTD_CCtx_params *cctxParams, ZSTD_cParameter param, unsigned value, int useOpaqueAPI)
{
    if (useOpaqueAPI)
    {
...
static int fuzzerTests_newAPI(U32 seed, int nbTests, int startTest, double compressibility, int bigTests)
{
    U32 const maxSrcLog = bigTests ? 24 : 22;
    static const U32 maxSampleLog = 19;
    size_t const srcBufferSize = (size_t)1 << maxSrcLog;
    ...
    for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime); testNb++)
    {
        ...
        else
        {
            U32 const testLog = FUZ_rand(&lseed) % maxSrcLog;
            U32 const dictLog = FUZ_rand(&lseed) % maxSrcLog;
            U32 const cLevelCandidate = (FUZ_rand(&lseed) % (ZSTD_maxCLevel() - (MAX(testLog, dictLog) / 2))) + 1;
            ...
            pledgedSrcSize = (FUZ_rand(&lseed) & 3) ? ZSTD_CONTENTSIZE_UNKNOWN : maxTestSize;
            {
                ...
                cParams.targetLength =
                    (U32)((cParams.targetLength + 1) * (0.5 + ((double)(FUZ_rand(&lseed) & 127) / 128)));
                ...
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_searchLog, cParams.searchLog, opaqueAPI));
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_minMatch, cParams.minMatch, opaqueAPI));
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_targetLength, cParams.targetLength, opaqueAPI));

                /* mess with long distance matching parameters */
                if (bigTests)
                {
                    if (FUZ_rand(&lseed) & 1)
                        CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_enableLongDistanceMatching,
                                                 FUZ_randomClampedLength(&lseed, ZSTD_ps_auto, ZSTD_ps_disable),
                                                 opaqueAPI));
                    if (FUZ_rand(&lseed) & 3)
                        CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_ldmHashLog,
                                                 FUZ_randomClampedLength(&lseed, ZSTD_HASHLOG_MIN, 23), opaqueAPI));
                    if (FUZ_rand(&lseed) & 3)
                        CHECK_Z(setCCtxParameter(
                            zc, cctxParams, ZSTD_c_ldmMinMatch,
                            FUZ_randomClampedLength(&lseed, ZSTD_LDM_MINMATCH_MIN, ZSTD_LDM_MINMATCH_MAX), opaqueAPI));
                    if (FUZ_rand(&lseed) & 3)
                        CHECK_Z(setCCtxParameter(
                            zc, cctxParams, ZSTD_c_ldmBucketSizeLog,
                            FUZ_randomClampedLength(&lseed, ZSTD_LDM_BUCKETSIZELOG_MIN, ZSTD_LDM_BUCKETSIZELOG_MAX),
                            opaqueAPI));
                    if (FUZ_rand(&lseed) & 3)
                        CHECK_Z(setCCtxParameter(
                            zc, cctxParams, ZSTD_c_ldmHashRateLog,
                            FUZ_randomClampedLength(&lseed, ZSTD_LDM_HASHRATELOG_MIN, ZSTD_LDM_HASHRATELOG_MAX),
                            opaqueAPI));
                    if (FUZ_rand(&lseed) & 3)
                        CHECK_Z(setCCtxParameter(
                            zc, cctxParams, ZSTD_c_srcSizeHint,
                            FUZ_randomClampedLength(&lseed, ZSTD_SRCSIZEHINT_MIN, ZSTD_SRCSIZEHINT_MAX), opaqueAPI));
                }
                ...
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_dictIDFlag, FUZ_rand(&lseed) & 1, opaqueAPI));
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_contentSizeFlag, FUZ_rand(&lseed) & 1, opaqueAPI));
                if (FUZ_rand(&lseed) & 1)
                {
                ...
                if (bigTests || (FUZ_rand(&lseed) & 0xF) == 0xF)
                {
                    ...
                    U32 const nbThreadsAdjusted =
                        (windowLogMalus < nbThreadsCandidate) ? nbThreadsCandidate - windowLogMalus : 1;
                    ...
                    if (nbThreads > 1)
                    {
                        ...
                        CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_jobSize, (U32)FUZ_rLogLength(&lseed, jobLog),
                                                 opaqueAPI));
                    }
                }
                ...
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_forceMaxWindow, FUZ_rand(&lseed) & 1, opaqueAPI));
                if (FUZ_rand(&lseed) & 1)
                    CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_deterministicRefPrefix, FUZ_rand(&lseed) & 1,
                                             opaqueAPI));
                ...
                else
                {
                    ...
                    for (totalTestSize = 0; (totalTestSize < maxTestSize);)
                    {
                        ...
                        do
                        {
                            ...
                            DISPLAYLEVEL(6, "t%u: compress consumed %u bytes (total : %u) ; flush: %u (total : %u) \n",
                                         testNb, (unsigned)inBuff.pos, (unsigned)(totalTestSize + inBuff.pos),
                    ...
                    /* final frame epilogue */
                    {
                        ...
                        while (remainingToFlush)
                        {
                            ...
                            CHECK(ZSTD_isError(remainingToFlush), "ZSTD_compressStream2 w/ ZSTD_e_end error : %s",
        ...
        }
        {
            ...
            for (totalGenSize = 0; decompressionResult;)
            {
                ...
                CHECK(ZSTD_isError(decompressionResult), "decompression error : %s",
                      ZSTD_getErrorName(decompressionResult));
                CHECK(inBuff.pos > cSize, "ZSTD_decompressStream consumes too much input : %u > %u ",
                      (unsigned)inBuff.pos, (unsigned)cSize);
            }
