...
static void writeFrameHeader(U32 *seed, frame_t *frame, dictInfo info)
{
    ...
    if (contentSizeFlag)
    {
        ...
        int minFcsCode = (fh.contentSize >= 256) + (fh.contentSize >= 65536 + 256) + (fh.contentSize > 0xFFFFFFFFU);
...

    {
         ...
         * singleSegment: 1-bit flag describing if data must be regenerated within a single continuous memory segment.
         * (bit 5) contentChecksumFlag: 1-bit flag that is set if frame includes checksum at the end -- set to 1 below
         * (bit 2) dictBits: 2-bit flag describing how many bytes Dictionary_ID uses -- set to 3 (bits 1-0) For more
         * information: https://github.com/facebook/zstd/blob/dev/doc/zstd_compression_format.md#frame_header
         */
        int const dictBits = info.useDict ? 3 : 0;
        BYTE const frameHeaderDescriptor = (BYTE)((fcsCode << 6) | (singleSegment << 5) | (1 << 2) | dictBits);
...
static size_t writeHufHeader(U32 *seed, HUF_CElt *hufTable, void *dst, size_t dstSize, const void *src, size_t srcSize)
{
...
static size_t writeLiteralsBlockCompressed(U32 *seed, frame_t *frame, size_t contentSize)
{
    ...
    do
    {
        ...
        if ((RAND(seed) & 3) || !frame->stats.hufInit)
        {
            do
            {
                ...
                else
                {
                    ...
                    RAND_bufferMaxSymb(seed, frame->stats.hufDist, DISTSIZE, 15);
                }
                RAND_bufferDist(seed, frame->stats.hufDist, LITERAL_BUFFER, litSize);
                ...
                hufHeaderSize =
                    writeHufHeader(seed, frame->stats.hufTable, op, opend - op, frame->stats.hufDist, DISTSIZE);
        ...
        do
        {
            compressedSize = sizeFormat == 0 ? HUF_compress1X_usingCTable(op, opend - op, LITERAL_BUFFER, litSize,
                                                                          frame->stats.hufTable, /* flags */ 0)
                                             : HUF_compress4X_usingCTable(op, opend - op, LITERAL_BUFFER, litSize,
                                                                          frame->stats.hufTable, /* flags */ 0);
    ...
    case 1:
    {
        U32 const header = hType | (sizeFormat << 2) | ((U32)litSize << 4) | ((U32)compressedSize << 14);
        MEM_writeLE24(ostart, header);
        break;
    }
    case 2:
    {
        U32 const header = hType | (sizeFormat << 2) | ((U32)litSize << 4) | ((U32)compressedSize << 18);
        MEM_writeLE32(ostart, header);
        break;
    }
    case 3:
    {
        U32 const header = hType | (sizeFormat << 2) | ((U32)litSize << 4) | ((U32)compressedSize << 22);
        MEM_writeLE32(ostart, header);
...
static U32 generateSequences(U32 *seed,
                             frame_t *frame,
                             SeqStore_t *seqStore,
                             size_t contentSize,
                             size_t literalsSize,
                             dictInfo info)
{
    ...
    for (i = 0; i < numSequences; i++)
    {
         ...
         * ensure nice numbers */
        U32 matchLen = MIN_SEQ_LEN + ROUND(RAND_exp(seed, (double)excessMatch / (double)(numSequences - i)));
        U32 literalLen =
            (RAND(seed) & 7) ? ROUND(RAND_exp(seed, (double)literalsSize / (double)(numSequences - i))) : 0;
        ...
        do
        {
            if (RAND(seed) & 7)
            {
                ...
                offset =
                    (RAND(seed) % MIN(frame->header.windowSize, (size_t)((BYTE *)srcPtr - (BYTE *)frame->srcStart))) +
                    1;
                    ...
                    dataDecompressed < frame->header.windowSize)
                {
                    ...
                    /* including i+1 != numSequences because the last sequences has to adhere to predetermined
                     * contentSize */
            ...
            else
            {
                ...
                else
                {
                    ...
                    offset = randomRepIndex == 2 ? frame->stats.rep[0] - 1 : frame->stats.rep[randomRepIndex + 1];
        ...
        DISPLAYLEVEL(6, "      LL: %5u OF: %5u ML: %5u", (unsigned)literalLen, (unsigned)offset, (unsigned)matchLen);
        DISPLAYLEVEL(7, " srcPos: %8u seqNb: %3u", (unsigned)((BYTE *)srcPtr - (BYTE *)frame->srcStart), (unsigned)i);
        DISPLAYLEVEL(6, "\n");
        if (OFFBASE_IS_REPCODE(offBase))
        { /* expects sumtype numeric representation of ZSTD_storeSeq() */
            DISPLAYLEVEL(7, "        repeat offset: %d\n", (int)repIndex);
        }
        /* use libzstd sequence handling */
        ZSTD_storeSeq(seqStore, literalLen, literals, literals + literalLen, offBase, matchLen);
...
static size_t writeSequences(U32 *seed, frame_t *frame, SeqStore_t *seqStorePtr, size_t nbSeq)
{
    ...
    /* CTable for Literal Lengths */
    {
        ...
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, llCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
        assert(!HIST_isError(mostFrequent));
        if (frame->stats.fseInit && !(RAND(seed) & 3) &&
            isSymbolSubset(llCodeTable, nbSeq, frame->stats.litlengthSymbolSet, 35))
        {
        ...
        else if (!(RAND(seed) & 3))
        {
            ...
            CHECKERR(FSE_buildCTable_wksp(CTable_LitLength, LL_defaultNorm, MaxLL, LL_defaultNormLog, scratchBuffer,
    ...
    /* see Literal Lengths for descriptions of mode choices */
    {
        unsigned max = MaxOff;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, ofCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
        assert(!HIST_isError(mostFrequent));
        if (frame->stats.fseInit && !(RAND(seed) & 3) &&
            isSymbolSubset(ofCodeTable, nbSeq, frame->stats.offsetSymbolSet, 28))
        {
            Offtype = set_repeat;
        }
        else if (mostFrequent == nbSeq)
    ...
    /* see Literal Lengths for descriptions of mode choices */
    {
        unsigned max = MaxML;
        size_t const mostFrequent =
            HIST_countFast_wksp(count, &max, mlCodeTable, nbSeq, WKSP, sizeof(WKSP)); /* cannot fail */
        assert(!HIST_isError(mostFrequent));
        if (frame->stats.fseInit && !(RAND(seed) & 3) &&
            isSymbolSubset(mlCodeTable, nbSeq, frame->stats.matchlengthSymbolSet, 52))
        {
            MLtype = set_repeat;
        }
        else if (mostFrequent == nbSeq)
    ...
    *seqHead = (BYTE)((LLtype << 6) + (Offtype << 4) + (MLtype << 2));
    ...
    /* Encoding Sequences */
    {
        ...
        RETURN_ERROR_IF(ERR_isError(BIT_initCStream(&blockStream, op, oend - op)), dstSize_tooSmall,
...
static size_t writeSequencesBlock(U32 *seed, frame_t *frame, size_t contentSize, size_t literalsSize, dictInfo info)
{
    SeqStore_t seqStore;
    size_t numSequences;
...
static void writeBlock(U32 *seed, frame_t *frame, size_t contentSize, int lastBlock, dictInfo info)
{
    int force_block_type = opts.blockType != NULL;
    ...
    else
    {
        ...
        if (compressedSize >= contentSize && !force_block_type)
        { /* compressed block must be strictly smaller than uncompressed one */
            blockType = 0;
            memcpy(op, frame->src, contentSize);
...
static U32 generateCompressedBlock(U32 seed, frame_t *frame, dictInfo info)
{
    ...
    while (!blockWritten)
    {
        ...
        { /* compressed size must be strictly smaller than decompressed size :
...
static int genRandomDict(U32 dictID, U32 seed, size_t dictSize, BYTE *fullDict)
{
...

    {
        ...
        dictWriteSize = ZDICT_finalizeDictionary(fullDict, dictSize, dictContent, dictContentSize, samples, sampleSizes,
                                                 numSamples, zdictParams);
...
static size_t testDecodeSimple(frame_t *fr)
{
    ...
    size_t const ret = ZSTD_decompress(DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr->dataStart,
...
static size_t testDecodeWithDict(U32 seed, genType_e genType)
{
...

    {
        ...
        if (genType == gt_frame)
        {
            ...
            ret = ZSTD_decompress_usingDict(dctx, DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr.dataStart,
        ...
        else
        {
            ...
            ret = ZSTD_decompressBlock_deprecated(dctx, DECOMPRESSED_BUFFER, MAX_DECOMPRESSED_SIZE, fr.dataStart,
                                                  (BYTE *)fr.data - (BYTE *)fr.dataStart);
        }
        ZSTD_freeDCtx(dctx);
...
static int runTestMode(U32 seed, unsigned numFiles, unsigned const testDurationS, genType_e genType)
{
    unsigned fnum;
    ...
    for (fnum = 0; fnum < numFiles || UTIL_clockSpanMicro(startClock) < maxClockSpan; fnum++)
    {
        ...
        {
            int const ret = (genType == gt_frame) ? runFrameTest(&seed) : runBlockTest(&seed);
            if (ret)
                return ret;
        }
    }
...
static int generateFile(U32 seed, const char *const path, const char *const origPath, genType_e genType)
{
    frame_t fr;
...
static int
generateCorpus(U32 seed, unsigned numFiles, const char *const path, const char *const origPath, genType_e genType)
{
...
static int generateCorpusWithDict(U32 seed,
                                  unsigned numFiles,
        ...
        if (argument[0] == '-')
        {
            ...
            while (*argument != 0)
            {
                ...
                case '-':
                    ...
                    {
                        U32 value = readU32FromChar(&argument);
                        g_maxDecompressedSizeLog = MIN(MAX_DECOMPRESSED_SIZE_LOG, value);
                    }
