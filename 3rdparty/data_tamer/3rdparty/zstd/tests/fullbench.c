...
#define WELCOME_MESSAGE                                                                                          \
    "*** %s %s %i-bits, by %s (%s) ***\n", PROGRAM_DESCRIPTION, ZSTD_versionString(), (int)(sizeof(void *) * 8), \
        AUTHOR, __DATE__
...
static size_t local_ZSTD_decompress(const void *src, size_t srcSize, void *dst, size_t dstSize, void *unused)
{
    (void)unused;
...
static size_t local_ZSTD_decompressDCtx(const void *src, size_t srcSize, void *dst, size_t dstSize, void *unused)
{
    (void)unused;
    ...
    RETURN_ERROR_IF(srcSize < MIN_CBLOCK_SIZE, corruption_detected, "");
    {
        ...
        if (litEncType == set_compressed)
        {
                            ...
                            "srcSize >= MIN_CBLOCK_SIZE == 3; here we need up to 5 for case 3");
            {
                ...
                return HUF_readDTableX1_wksp(dctx->entropy.hufTable, istart + lhSize, litCSize, dctx->workspace,
                                             sizeof(dctx->workspace), flags);
#else
                return HUF_readDTableX2_wksp(dctx->entropy.hufTable, istart + lhSize, litCSize, dctx->workspace,
                                             sizeof(dctx->workspace), flags);
#endif
            }
        }
...
static size_t local_ZSTD_decodeSeqHeaders(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *unused)
{
    int nbSeq;
    (void)unused;
    (void)dst;
...
static size_t
local_ZSTD_compressStream_freshCCtx(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    if (g_cstream != NULL)
...
static size_t
local_ZSTD_compressStream2_end(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    ZSTD_outBuffer buffOut;
    ZSTD_inBuffer buffIn;
    (void)payload;
...
static size_t
local_ZSTD_compressStream2_continue(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    ZSTD_outBuffer buffOut;
    ZSTD_inBuffer buffIn;
    (void)payload;
...
static size_t
local_ZSTD_compress_generic_T2_end(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    (void)payload;
...
static size_t
local_ZSTD_compress_generic_T2_continue(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    ZSTD_outBuffer buffOut;
    ZSTD_inBuffer buffIn;
    (void)payload;
...
static size_t local_ZSTD_decompressStream(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *unused)
{
    ZSTD_outBuffer buffOut;
    ZSTD_inBuffer buffIn;
    (void)unused;
...
static size_t
local_ZSTD_compressContinue_extDict(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
...
static size_t
local_ZSTD_decompressContinue(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *unused)
{
...
static size_t local_compressSequences(const void *input, size_t inputSize, void *dst, size_t dstCapacity, void *payload)
{
    const char *ip = input;
    ...
    assert(8 + nbSeqs * sizeof(ZSTD_Sequence) + srcSize == inputSize);
    (void)inputSize;
    (void)payload;
...
static size_t
local_compressSequencesAndLiterals(const void *input, size_t inputSize, void *dst, size_t dstCapacity, void *payload)
{
    const char *ip = input;
...
static size_t local_convertSequences(const void *input, size_t inputSize, void *dst, size_t dstCapacity, void *payload)
{
    const char *ip = input;
    ...
    ZSTD_CCtx_setParameter(g_zcc, ZSTD_c_repcodeResolution, ZSTD_ps_enable);
#endif
    assert(8 + nbSeqs * sizeof(ZSTD_Sequence) == inputSize);
    (void)inputSize;
    (void)dst;
    (void)dstCapacity;
    (void)payload;
...
static size_t check_compressedSequences(const void *compressed, size_t cSize, const void *orig, size_t origSize)
{
    size_t decSize;
    int diff;
    ...
    {
        free(decompressed);
        DISPLAY("ZSTD_decompress failed (%u) ", (unsigned)decSize);
        return 1;
    }
...
static size_t local_get1BlockSummary(const void *input, size_t inputSize, void *dst, size_t dstCapacity, void *payload)
{
    const char *ip = input;
    size_t const blockSize = MEM_read32(ip);
    size_t const nbSeqs = MEM_read32(ip += 4);
 ...
 *********************************************************/
static int
benchMem(unsigned scenarioID, const void *origSrc, size_t origSrcSize, int cLevel, ZSTD_compressionParameters cparams)
{
    ...
    if (scenarioID >= NB_SCENARIOS)
        return 0; /* scenario doesn't exist */
    ...
    if (prepBuffSize == 0)
        goto _cleanOut; /* failed preparation */

    /* warming up dstBuff */
    {
        size_t i;
        for (i = 0; i < dstCapacity; i++)
            ((BYTE *)dst)[i] = (BYTE)i;
    }
    ...
    /* benchmark loop */
    {
        ...
        bp.dstBuffers = (void *const *)
            avoidStrictAliasingPtr; /* circumvent strict aliasing warning on gcc-8,
        ...
        for (;;)
        {
...

            {
                ...
                DISPLAY("\r%2u#%-31.31s:%8.1f MB/s  (%8u) ", scenarioID, benchName,
 ...
 */
static int
benchSample(U32 scenarioID, size_t benchedSize, double compressibility, int cLevel, ZSTD_compressionParameters cparams)
{
    /* Allocation */
