...
int LLVMFuzzerTestOneInput(const uint8_t *src, size_t size)
{
    ...
    // Test one-shot decompression
    {
        ...
        const size_t standardRet = ZSTD_decompressDCtx(dctx, standardDst, dstSize, standardSrc, standardSize);

        FUZZ_ZASSERT(ZSTD_DCtx_reset(dctx, ZSTD_reset_session_and_parameters));
        FUZZ_ZASSERT(ZSTD_DCtx_setParameter(dctx, ZSTD_d_format, ZSTD_f_zstd1_magicless));
        const size_t magiclessRet = ZSTD_decompressDCtx(dctx, magiclessDst, dstSize, magiclessSrc, magiclessSize);

        // Standard accepts => magicless should accept
        if (!ZSTD_isError(standardRet))
            FUZZ_ZASSERT(magiclessRet);
        ...
        if (!ZSTD_isError(magiclessRet))
            FUZZ_ZASSERT(standardRet);
        ...
        if (!ZSTD_isError(standardRet) && !ZSTD_isError(magiclessRet))
        {
            ...
            if (standardRet > 0)
            {
                FUZZ_ASSERT(memcmp(standardDst, magiclessDst, standardRet) == 0);
            }
        }
    }
    ...
    // Test streaming decompression
    {
        ...
        // Standard accepts => magicless should accept
        if (standardRet == 0)
            FUZZ_ASSERT(magiclessRet == 0);

        // Magicless accepts => standard should accept
        // NOTE: this is nice-to-have, please disable this check if it is difficult to satisfy.
        if (magiclessRet == 0)
            FUZZ_ASSERT(standardRet == 0);
        ...
        if (standardRet == 0 && magiclessRet == 0)
        {
            ...
            {
                FUZZ_ASSERT(memcmp(standardOut.dst, magiclessOut.dst, standardOut.pos) == 0);
            }
        }
    }
