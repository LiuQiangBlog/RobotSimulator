...
static unsigned produceParamValue(unsigned min, unsigned max, FUZZ_dataProducer_t *producer)
{
    return FUZZ_dataProducer_uint32Range(producer, min, max);
}

static void setRand(ZSTD_CCtx *cctx, ZSTD_cParameter param, unsigned min, unsigned max, FUZZ_dataProducer_t *producer)
{
...
ZSTD_compressionParameters FUZZ_randomCParams(size_t srcSize, FUZZ_dataProducer_t *producer)
{
    ...
    cParams.minMatch = FUZZ_dataProducer_uint32Range(producer, ZSTD_MINMATCH_MIN, ZSTD_MINMATCH_MAX);
...
static void setSequenceProducerParams(ZSTD_CCtx *cctx, FUZZ_dataProducer_t *producer)
{
    ...
    ZSTD_registerSequenceProducer(cctx, FUZZ_seqProdState, FUZZ_thirdPartySeqProd);
#else
...
void FUZZ_setRandomParameters(ZSTD_CCtx *cctx, size_t srcSize, FUZZ_dataProducer_t *producer)
{
    ...
    setRand(cctx, ZSTD_c_ldmMinMatch, ZSTD_LDM_MINMATCH_MIN, ZSTD_LDM_MINMATCH_MAX, producer);
    setRand(cctx, ZSTD_c_ldmBucketSizeLog, 0, ZSTD_LDM_BUCKETSIZELOG_MAX, producer);
    setRand(cctx, ZSTD_c_ldmHashRateLog, ZSTD_LDM_HASHRATELOG_MIN, ZSTD_LDM_HASHRATELOG_MAX, producer);
    /* Set misc parameters */
...
FUZZ_dict_t FUZZ_train(void const *src, size_t srcSize, FUZZ_dataProducer_t *producer)
{
    ...
    dict.size = ZDICT_trainFromBuffer_fastCover(dict.buff, dictSize, samples, samplesSizes, nbSamples, params);
