...
#define CHECK(fn)                                                \
    {                                                            \
        if (!(fn))                                               \
        {                                                        \
            DISPLAYLEVEL(1, "Error : test (%s) failed \n", #fn); \
            exit(1);                                             \
        }                                                        \
    }
...
#define CHECK_VAR(var, fn)                                                  \
    ...
    if (ZSTD_isError(var))                                                  \
    {                                                                       \
        DISPLAYLEVEL(1, "%s : fails : %s \n", #fn, ZSTD_getErrorName(var)); \
        exit(1);                                                            \
    }
...
static int FUZ_mallocTests_internal(unsigned seed,
...
static void FUZ_decodeSequences(BYTE *dst,
                                ZSTD_Sequence *seqs,
                                size_t seqsSize,
                                BYTE *src,
                                size_t size,
...
static void FUZ_transferLiterals(void *dst,
                                 size_t dstCapacity,
                                 const void *src,
                                 size_t srcSize,
                                 const ZSTD_Sequence *seqs,
                                 size_t nbSeqs)
{
    size_t n;
...
static void *threadPoolTests_compressionJob(void *payload)
{
    ...
    size_t cSize;
    if (ZSTD_isError(ZSTD_CCtx_refThreadPool(args->cctx, args->pool)))
        args->err = 1;
    cSize = ZSTD_compress2(args->cctx, args->compressedBuffer, args->compressedBufferSize, args->CNBuffer,
                           args->CNBuffSize);
    if (ZSTD_isError(cSize))
        args->err = 1;
    if (ZSTD_isError(ZSTD_decompress(args->decodedBuffer, args->CNBuffSize, args->compressedBuffer, cSize)))
        args->err = 1;
    return payload;
}
...
static int threadPoolTests(void)
{
    ...
    DISPLAYLEVEL(3, "thread pool test : threadPool simultaneous usage: ");
    {
        ...
        ZSTD_pthread_t t1;
        ZSTD_pthread_t t2;
        threadPoolTests_compressionJob_payload p1 = {
            cctx1, pool, CNBuffer, CNBuffSize, compressedBuffer, compressedBufferSize, decodedBuffer, 0 /* err */};
        threadPoolTests_compressionJob_payload p2 = {
            cctx2, pool, CNBuffer, CNBuffSize, compressedBuffer2, compressedBufferSize, decodedBuffer2, 0 /* err */};
...
static void test_blockSplitter_incompressibleExpansionProtection(unsigned testNb, unsigned seed)
{
    DISPLAYLEVEL(3, "test%3i : Check block splitter doesn't oversplit incompressible data (seed %u): ", testNb, seed);
    {
        ...
        DISPLAYLEVEL(4, "compressed size: cSizeWithSplit %u == %u cSizeNoSplit : ", (unsigned)cSizeWithSplit,
                     (unsigned)cSizeNoSplit);
...
static int basicUnitTests(U32 const seed, double compressibility)
{
    ...
    DISPLAYLEVEL(3, "test%3u : default compression level : ", testNb++);
    {
        ...
        if (defaultCLevel != ZSTD_CLEVEL_DEFAULT)
            goto _output_error;
        DISPLAYLEVEL(3, "%i (OK) \n", defaultCLevel);
    }
    ...
    DISPLAYLEVEL(3, "test%3i : decompress %u bytes with Huffman assembly disabled : ", testNb++, (unsigned)CNBuffSize);
    {
        ...
        if (r != CNBuffSize || memcmp(decodedBuffer, CNBuffer, CNBuffSize))
            goto _output_error;
        ZSTD_freeDCtx(dctx);
    }
    DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : decompress with null dict : ", testNb++);
    {
        ...
        {
            size_t const r =
                ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, NULL, 0);
            if (r != CNBuffSize)
                goto _output_error;
        }
        ZSTD_freeDCtx(dctx);
    ...
    { /* copy the compressed buffer and corrupt the checksum */
        ...
        if (!ZSTD_isError(r))
            goto _output_error;
        if (ZSTD_getErrorCode(r) != ZSTD_error_checksum_wrong)
            goto _output_error;
        ...
        if (!ZSTD_isError(r))
            goto _output_error; /* wrong checksum size should still throw error */
    ...
    { /* ensure frame content size is missing */
        ...
        if (ret != 0 || zfh.frameContentSize != ZSTD_CONTENTSIZE_UNKNOWN)
            goto _output_error;
    }
    ...
    DISPLAYLEVEL(3, "test%3d: check DCtx size is reduced after many oversized calls : ", testNb++);
    {
        ...
        for (i = 0; i < nbFrames; i++)
            srcSize +=
                ZSTD_compress((void *)(dst + srcSize), compressedBufferSize - srcSize, CNBuffer, smallFrameSrcSize, 3);
...

    {
        ...
        CHECK_Z(ZSTD_compress_usingCDict_advanced(cctx, compressedBuffer, compressedBufferSize, NULL, 0, cdict,
                                                  params.fParams));
        if (MEM_readLE32(compressedBuffer) != ZSTD_MAGICNUMBER)
            goto _output_error;
        DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : testing dict compression with enableLdm and forceMaxWindow : ", testNb++);
    {
        ...
        for (nbWorkers = 0; nbWorkers < 3; ++nbWorkers)
        {
            ...
            CHECK_Z(cSize);
            CHECK_Z(
                ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dict, CNBuffSize));
        }
    ...
    DISPLAYLEVEL(3, "test%3i : testing dict compression for determinism : ", testNb++);
    {
        ...
        for (level = 1; level <= 5; ++level)
        {
            for (ldmEnabled = ZSTD_ps_enable; ldmEnabled <= ZSTD_ps_disable; ++ldmEnabled)
            {
                ...
                CHECK_Z(cSize);
                CHECK_Z(
                    ZSTD_decompress_usingDict(dctx, decodedBuffer, testSize, compressedBuffer, cSize, dict, testSize));
                ...
                if (cSize != cSize0)
                    goto _output_error;
                if (XXH64(compressedBuffer, cSize, 0) != compressedChecksum0)
                    goto _output_error;
            }
        }
    ...
    {
        size_t const cSize2 =
            ZSTD_compress((char *)compressedBuffer + cSize, compressedBufferSize - cSize,
                          (char const *)CNBuffer + (CNBuffSize / 3), CNBuffSize / 3, -ZSTD_BLOCKSIZE_MAX);
        CHECK_Z(cSize2);
        cSize += cSize2;
    }
    {
    ...
    DISPLAYLEVEL(3, "test%3d : superblock uncompressible data: too many nocompress superblocks : ", testNb++);
    {
...

        {
            ...
            for (read = 0; read < streamCompressThreshold; read += streamCompressDelta)
            {
                ...
                CHECK_Z(ZSTD_compressStream2(cctx, &out, &in, ZSTD_e_continue));
                CHECK_Z(ZSTD_compressStream2(cctx, &out, &in, ZSTD_e_end));
                src += streamCompressDelta;
                srcSize -= streamCompressDelta;
                dst += out.pos;
                dstCapacity -= out.pos;
            }
        }
    ...
    }
    {
        ...
        {
            DISPLAYLEVEL(1, "Superblock too big: %u > %u + %u \n", (U32)superCSize, (U32)normalCSize,
                         (U32)allowedExpansion);
            goto _output_error;
        }
        ZSTD_freeCCtx(cctx);
    }
    ...
    }
    {
        ...
        CHECK_Z(ZSTD_flushStream(cctx,
                                 &output)); /* ensure no possibility to "concatenate" and determine the content size */
    ...
    DISPLAYLEVEL(3, "test%3d : reuse CCtx with expanding block size : ", testNb++);
    {
        ...
        CHECK_Z(ZSTD_compressEnd(cctx, compressedBuffer, compressedBufferSize, CNBuffer,
                                 1)); /* creates a block size of 1 */

        CHECK_Z(
            ZSTD_compressBegin_advanced(cctx, NULL, 0, params, ZSTD_CONTENTSIZE_UNKNOWN)); /* reuse same parameters */
        {
    ...
    DISPLAYLEVEL(3, "test%3d : re-using a CCtx should compress the same : ", testNb++);
    {
        ...
        for (i = 1; i <= 19; i++)
        {
            ...
            size2 = ZSTD_compress2(
                cctx, compressedBuffer, ZSTD_compressBound(sampleSize) - 1, CNBuffer,
    ...
    DISPLAYLEVEL(3, "test%3d : overflow protection with large windowLog : ", testNb++);
    {
        ...
        for (cnb = 0; cnb < nbCompressions; ++cnb)
        {
            ...
            CHECK_Z(ZSTD_compressBegin_advanced(cctx, NULL, 0, params,
                                                ZSTD_CONTENTSIZE_UNKNOWN)); /* reuse same parameters */
    ...
    DISPLAYLEVEL(3, "test%3d : size down context : ", testNb++);
    {
        ...
        {
            size_t const largeCCtxSize =
                ZSTD_sizeof_CCtx(largeCCtx); /* size of context must be measured after compression */
            {
                ...
                CHECK_Z(ZSTD_compressCCtx(smallCCtx, compressedBuffer, compressedBufferSize, CNBuffer, 1, 1));
                {
                    ...
                    DISPLAYLEVEL(5, "(large) %uKB > 32*%uKB (small) : ", (unsigned)(largeCCtxSize >> 10),
                                 (unsigned)(smallCCtxSize >> 10));
                    assert(largeCCtxSize >
                           32 * smallCCtxSize); /* note : "too large" definition is handled within zstd_compress.c .
                                                 * make this test case extreme, so that it doesn't depend on a possibly
                                                 * fluctuating definition */
                }
                ZSTD_freeCCtx(smallCCtx);
            }
            {
                U32 const maxNbAttempts = 1100; /* nb of usages before triggering size down is handled within
                                                 * zstd_compress.c. currently defined as 128x, but could be adjusted in
                                                 * the future. make this test long enough so that it's not too much tied
                                                 * to the current definition within zstd_compress.c */
                unsigned u;
...
#define STATIC_CCTX_LEVEL 4
    DISPLAYLEVEL(3, "test%3i : create static CCtx for level %u : ", testNb++, STATIC_CCTX_LEVEL);
    {
        ...
        }
        {
            ...
            CHECK_VAR(cSize, ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize, CNBuffer,
                                               smallInSize, STATIC_CCTX_LEVEL));
            ...
            {
                size_t const r = ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize, CNBuffer,
            ...
            CHECK_VAR(cSize, ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize,
                                               STATIC_CCTX_LEVEL));
            DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (unsigned)cSize, (double)cSize / CNBuffSize * 100);
            ...
            DISPLAYLEVEL(3, "test%3i : compress small input often enough to trigger context reduce : ", testNb++);
            {
                ...
                {
                    CHECK_Z(ZSTD_compressCCtx(staticCCtx, compressedBuffer, compressedBufferSize,
                                              (char *)CNBuffer + nbc, smallInSize, STATIC_CCTX_LEVEL));
                }
            }
            DISPLAYLEVEL(3, "OK \n")
            ...
            {
                size_t const r = ZSTD_decompressDCtx(staticDCtx, decodedBuffer, CNBuffSize, compressedBuffer, cSize);
                if (r != CNBuffSize)
                    goto _output_error;
            }
            DISPLAYLEVEL(3, "OK \n");
            ...
            DISPLAYLEVEL(3, "test%3i : check decompressed result : ", testNb++);
            if (memcmp(decodedBuffer, CNBuffer, CNBuffSize))
                goto _output_error;
            DISPLAYLEVEL(3, "OK \n");
            ...
            DISPLAYLEVEL(3, "test%3i : test estimation functions with default cctx params : ", testNb++);
            {
                // Test ZSTD_estimateCCtxSize_usingCCtxParams
                {
                    ...
                    {
                        size_t const r =
                            ZSTD_decompressDCtx(staticDCtx, decodedBuffer, CNBuffSize, compressedBuffer, cSize);
                        if (r != CNBuffSize)
                            goto _output_error;
                        if (memcmp(decodedBuffer, CNBuffer, CNBuffSize))
                            goto _output_error;
    ...
    DISPLAYLEVEL(3, "test%3i : create ZSTDMT CCtx : ", testNb++);
    {
        ...
        CHECK_VAR(cSize, ZSTD_compress2(mtctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize));
        DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (unsigned)cSize, (double)cSize / CNBuffSize * 100);
    ...
    DISPLAYLEVEL(3, "test%3i : compress multiple frames : ", testNb++);
    {
        ...
        for (i = 0; i < segs; i++)
        {
            CHECK_NEWV(r, ZSTD_compress((BYTE *)compressedBuffer + off, CNBuffSize - off,
                                        (BYTE *)CNBuffer + segSize * (size_t)i, segSize, 5));
            off += r;
            if (i == segs / 2)
            {
                /* insert skippable frame */
                size_t const skippableSize = ZSTD_writeSkippableFrame(
                    (BYTE *)compressedBuffer + off, compressedBufferSize, skipBuff, skipLen, seed % 15);
    ...
    DISPLAYLEVEL(3, "test%3i : read/write a skippable frame : ", testNb++);
    {
        ...
        skippableSize = ZSTD_writeSkippableFrame(compressedBuffer, compressedBufferSize, skipBuff, skipLen, seed % 15);
        CHECK_Z(skippableSize);
    ...
    DISPLAYLEVEL(3, "test%3i : read/write an empty skippable frame : ", testNb++);
    {
        ...
        unsigned long long receivedSize;
        size_t skippableSize;
        skippableSize = ZSTD_writeSkippableFrame(compressedBuffer, compressedBufferSize, CNBuffer, 0, seed % 15);
    ...
    /* Dictionary and CCtx Duplication tests */
    {
        ...
        CHECKPLUS(
            r, ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, CNBuffer, dictSize),
        ...
        DISPLAYLEVEL(3, "test%3i : compress with duplicated context : ", testNb++);
        {
            ...
            cSize = 0;
            CHECKPLUS(r,
                      ZSTD_compressEnd(ctxDuplicated, compressedBuffer, compressedBufferSize,
...
#if !defined(ZSTD_EXCLUDE_DFAST_BLOCK_COMPRESSOR) && !defined(ZSTD_EXCLUDE_GREEDY_BLOCK_COMPRESSOR) &&  \
    !defined(ZSTD_EXCLUDE_DFAST_BLOCK_COMPRESSOR) && !defined(ZSTD_EXCLUDE_LAZY_BLOCK_COMPRESSOR) &&    \
    !defined(ZSTD_EXCLUDE_LAZY2_BLOCK_COMPRESSOR) && !defined(ZSTD_EXCLUDE_BTLAZY2_BLOCK_COMPRESSOR) && \
    !defined(ZSTD_EXCLUDE_BTOPT_BLOCK_COMPRESSOR) && !defined(ZSTD_EXCLUDE_BTULTRA_BLOCK_COMPRESSOR)
         ...
         */
        if ((int)(compressibility * 100 + 0.1) == FUZ_compressibility_default)
        { /* test only valid with known input */
            ...
            size_t target_nodict_cSize[22 + 1] = {3840, 3770, 3870, 3830, 3770, 3770, 3770, 3770,
                                                  3750, 3750, 3742, 3675, 3674, 3665, 3664, 3663,
                                                  3662, 3661, 3660, 3660, 3660, 3660, 3660};
            size_t const target_wdict_cSize[22 + 1] = {2830, 2896, 2893, 2840, 2950, 2950, 2950, 2925,
                                                       2900, 2892, 2910, 2910, 2910, 2780, 2775, 2765,
                                                       2760, 2755, 2754, 2753, 2753, 2753, 2753};
            int l = 1;
            ...
            for (; l <= maxLevel; l++)
            {
                size_t const nodict_cSize =
                    ZSTD_compress(compressedBuffer, compressedBufferSize, contentStart, contentSize, l);
                if (nodict_cSize > target_nodict_cSize[l])
                {
                    DISPLAYLEVEL(1, "error : compression at level %i worse than expected (%u > %u) \n", l,
                                 (unsigned)nodict_cSize, (unsigned)target_nodict_cSize[l]);
                    goto _output_error;
                }
                DISPLAYLEVEL(4, "level %i : max expected %u >= reached %u \n", l, (unsigned)target_nodict_cSize[l],
                             (unsigned)nodict_cSize);
            }
            for (l = 1; l <= maxLevel; l++)
            {
                size_t const wdict_cSize = ZSTD_compress_usingDict(ctxOrig, compressedBuffer, compressedBufferSize,
                                                                   contentStart, contentSize, dict, flatdictSize, l);
                ...
                DISPLAYLEVEL(4, "level %i with dictionary : max expected %u >= reached %u \n", l,
                             (unsigned)target_wdict_cSize[l], (unsigned)wdict_cSize);
            }
            ...
            for (l = 1; l <= maxLevel; l++)
            {
                ...
                DISPLAYLEVEL(4, "level %i with compress2 : max expected %u >= reached %u \n", l,
                             (unsigned)target_nodict_cSize[l], (unsigned)nodict_cSize);
                ZSTD_freeCCtx(cctx);
            }
            ...
            for (l = 1; l <= maxLevel; l++)
            {
                ...
                wdict_cSize =
                    ZSTD_compress2(ctxOrig, compressedBuffer, compressedBufferSize, contentStart, contentSize);
                if (wdict_cSize > target_wdict_cSize[l])
                {
                    DISPLAYLEVEL(1,
                                 "error : compression with dictionary and compress2 at level %i worse than expected "
                                 "(%u > %u) \n",
                                 l, (unsigned)wdict_cSize, (unsigned)target_wdict_cSize[l]);
                    goto _output_error;
                }
    ...
    /* Dictionary and dictBuilder tests */
    {
        ...
        unsigned char const dictBufferFixed[144] = {
            };
        ...
        }
        {
            size_t const sDictSize =
                ZDICT_trainFromBuffer(dictBuffer, dictBufferCapacity, decodedBuffer, samplesSizes, nbSamples);
        ...
        }
        {
            ...
            dictSize = ZDICT_optimizeTrainFromBuffer_cover(dictBuffer, dictBufferCapacity, CNBuffer, samplesSizes,
        ...
        }
        {
            ...
            dictSize = ZDICT_optimizeTrainFromBuffer_fastCover(dictBuffer, dictBufferCapacity, CNBuffer, samplesSizes,
        ...
        cSize = ZSTD_compress_usingDict(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize, dictBuffer,
                                        dictSize, 4);
        if (ZSTD_isError(cSize))
            goto _output_error;
        DISPLAYLEVEL(3, "OK (%u bytes : %.2f%%)\n", (unsigned)cSize, (double)cSize / CNBuffSize * 100);
        ...
        DISPLAYLEVEL(3, "test%3i : frame built with dictionary should be decompressible : ", testNb++);
        {
            ...
            assert(dctx != NULL);
            CHECKPLUS(r,
                      ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dictBuffer,
                                                dictSize),
        ...
        {
            ZSTD_compressionParameters const cParams = ZSTD_getCParams(1, CNBuffSize, dictSize);
            ZSTD_CDict *const cdict = ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto,
        ...
        DISPLAYLEVEL(3, "test%3i : compress with static CDict : ", testNb++);
        {
            ...
            for (level = 1; level <= maxLevel; ++level)
            {
                    ...
                    goto _output_error;
                {
                    ...
                    cSize = ZSTD_compress_usingCDict(cctx, compressedBuffer, compressedBufferSize, CNBuffer,
        ...
        DISPLAYLEVEL(3, "test%3i : ZSTD_compress_usingCDict_advanced, no contentSize, no dictID : ", testNb++);
        {
            ...
            ZSTD_CDict *const cdict = ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto,
                                                                cParams, ZSTD_defaultCMem);
            assert(cdict != NULL);
            cSize = ZSTD_compress_usingCDict_advanced(cctx, compressedBuffer, compressedBufferSize, CNBuffer,
        ...
        DISPLAYLEVEL(3, "test%3i : ZSTD_compress_advanced, no dictID : ", testNb++);
        {
            ...
            cSize = ZSTD_compress_advanced(cctx, compressedBuffer, compressedBufferSize, CNBuffer, CNBuffSize,
        ...
        DISPLAYLEVEL(3, "test%3d : bufferless api with cdict : ", testNb++);
        {
            ...
            cBlockSize = ZSTD_compressContinue(cctx, (char *)compressedBuffer + cSize, compressedBufferSize - cSize,
                                               CNBuffer, 1000);
            CHECK_Z(cBlockSize);
            cSize += cBlockSize;
            cBlockSize = ZSTD_compressEnd(cctx, (char *)compressedBuffer + cSize, compressedBufferSize - cSize,
                                          (char const *)CNBuffer + 2000, 1000);
            CHECK_Z(cBlockSize);
            cSize += cBlockSize;

            CHECK_Z(ZSTD_decompress_usingDict(dctx, decodedBuffer, CNBuffSize, compressedBuffer, cSize, dictBuffer,
                                              dictSize));
        ...
        DISPLAYLEVEL(3, "test%3i : Building cdict w/ ZSTD_dct_fullDict on a rawContent (must fail) : ", testNb++);
        {
            ...
            ZSTD_CDict *const cdict =
                ZSTD_createCDict_advanced((const char *)dictBuffer + 1, dictSize - 1, ZSTD_dlm_byRef, ZSTD_dct_fullDict,
                                          cParams, ZSTD_defaultCMem);
            if (cdict != NULL)
                goto _output_error;
            ZSTD_freeCDict(cdict);
...

        {
            ...
            DISPLAYLEVEL(
                3, "test%3i : Loading rawContent starting with dict header w/ ZSTD_dct_auto should fail : ", testNb++);
            {
                size_t ret;
                /* Either operation is allowed to fail, but one must fail. */
                ret = ZSTD_CCtx_loadDictionary_advanced(cctx, (const char *)rawDictBuffer, dictSize, ZSTD_dlm_byRef,
                                                        ZSTD_dct_auto);
                if (!ZSTD_isError(ret))
                {
            ...
            DISPLAYLEVEL(3,
                         "test%3i : Loading rawContent starting with dict header w/ ZSTD_dct_rawContent should pass : ",
                         testNb++);
            {
                size_t ret;
                ret = ZSTD_CCtx_loadDictionary_advanced(cctx, (const char *)rawDictBuffer, dictSize, ZSTD_dlm_byRef,
            ...
            DISPLAYLEVEL(3, "test%3i : Testing non-attached CDict with ZSTD_dct_rawContent : ", testNb++);
            {
                ...
                CHECK_Z(ZSTD_CCtx_loadDictionary_advanced(cctx, rawDictBuffer, dictSize, ZSTD_dlm_byRef,
                                                          ZSTD_dct_rawContent));
        ...
        DISPLAYLEVEL(
            3, "test%3i : Loading dictionary before setting parameters is the same as loading after : ", testNb++);
        {
            size_t size1, size2;
            ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
            CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, 7));
        ...
        DISPLAYLEVEL(3, "test%3i : ZSTD_fast attach dictionary with hashLog = 25 and chainLog = 25 : ", testNb++);
        {
            ...
            cdict =
                ZSTD_createCDict_advanced2(dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto, cctxParams, customMem);
        ...
        DISPLAYLEVEL(3, "test%3i : ZSTD_decompressDCtx() with multiple ddicts : ", testNb++);
        {
            ...
            char *dictBufferMulti =
                (char *)malloc(sizeof(char) * dictBufferFixedSize); /* Modifiable copy of fixed full dict buffer */
            ...
            /* Compress a few frames using random CDicts */
            {
                ...
                for (i = 0; i < numFrames; i++)
                {
                    ...
                    ZSTD_CCtx_reset(cctx, ZSTD_reset_session_and_parameters);
                    {
                        CHECK_NEWV(r, ZSTD_compress_usingCDict(cctx, (BYTE *)compressedBuffer + off, CNBuffSize - off,
    ...
    /* COVER dictionary builder tests */
    {
        ...
        memset(&params, 0, sizeof(params));
        params.steps = 4;
        optDictSize = ZDICT_optimizeTrainFromBuffer_cover(dictBuffer, optDictSize, CNBuffer, samplesSizes,
                                                          nbSamples / 4, &params);
    ...
    DISPLAYLEVEL(3, "test%3i : negative compression level : ", testNb++);
    {
        ...
        assert(cctx != NULL);
        {
            size_t const cSize_1pass =
                ZSTD_compress(compressedBuffer, compressedBufferSize, CNBuffer, srcSize, compressionLevel);
            ...
            CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, compressionLevel));
            {
                size_t const compressionResult =
                    ZSTD_compress2(cctx, compressedBuffer, compressedBufferSize, CNBuffer, srcSize);
...

        {
            ...
            CHECK_Z(ZSTD_CCtx_setParameter(cctx, ZSTD_c_windowLog, 18));
            {
                size_t const compressedSize =
                    ZSTD_compress2(cctx, compressedBuffer, ZSTD_compressBound(inputSize), CNBuffer, inputSize);
    ...
    DISPLAYLEVEL(3, "test%3i : Decompression parameter reset test : ", testNb++);
    {
        ...
        int param;
        if (ZSTD_d_experimentalParam3 > maxParam)
            goto _output_error;
    ...
    /* block API tests */
    {
        ...
        CHECK_VAR(cSize,
                  ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize), CNBuffer, blockSize));
        DISPLAYLEVEL(3, "OK \n");
        ...
        CHECK_VAR(cSize, ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize),
                         ...
                         blockSize); /* insert non-compressed block into dctx history */
        {
            ...
            {
                DISPLAYLEVEL(1,
                             "ZSTD_decompressBlock() with _usingDict() and after insertBlock() fails : %u, instead of "
                             "%u expected \n",
                             (unsigned)r, (unsigned)blockSize);
                goto _output_error;
            }
        ...
        DISPLAYLEVEL(3, "test%3i : Block compression with CDict : ", testNb++);
        {
            ...
            CHECK_Z(ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize),
                                       (char *)CNBuffer + dictSize, blockSize));
            ZSTD_freeCDict(cdict);
        }
        DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : ZSTD_compressSequencesAndLiterals : ", testNb++);
    {
        ...
        }
        {
            ...
            {
                DISPLAY(
                    "Error: ZSTD_compressSequencesAndLiterals() did not resized window size to smaller contentSize\n");
                goto _output_error;
            }
        }
        {
            ...
            if (dSize != srcSize)
            {
                DISPLAY("Error: decompression of frame produced by ZSTD_compressSequencesAndLiterals() has different "
                        "size\n");
                goto _output_error;
            }
            if (memcmp(decompressBuffer, src, srcSize))
            {
                DISPLAY("Error: decompression of frame produced by ZSTD_compressSequencesAndLiterals() produces a "
                        "different content (of same size)\n");
                goto _output_error;
            }
        }
...
#define LONGZEROSLENGTH 1000000 /* 1MB of zeros */
    ...
    CHECK_VAR(cSize,
              ZSTD_compress(compressedBuffer, ZSTD_compressBound(LONGZEROSLENGTH), CNBuffer, LONGZEROSLENGTH, 1));
...
#define NB3BYTESSEQMASK (NB3BYTESSEQ - 1)
    /* creates a buffer full of 3-bytes sequences */
    {
        ...
        /* randomly fills CNBuffer with prepared 3-bytes sequences */
        {
            ...
            { /* note : CNBuffer size > _3BYTESTESTLENGTH+3 */
    ...
    CHECK_VAR(cSize,
              ZSTD_compress(compressedBuffer, ZSTD_compressBound(_3BYTESTESTLENGTH), CNBuffer, _3BYTESTESTLENGTH, 19));
    ...
    { /* Train a dictionary on low characters */
        ...
        { /* Compress the data */
            ...
            if (!outputBuffer || !cctx)
                goto _output_error;
            CHECK_Z(
                ZSTD_compress_usingDict(cctx, outputBuffer, outputSize, CNBuffer, inputSize, dictBuffer, dictSize, 1));
            free(outputBuffer);
            ZSTD_freeCCtx(cctx);
        }
    ...
    if (strcmp("No error detected", ZSTD_getErrorName((ZSTD_ErrorCode)(0 - ZSTD_error_no_error))) != 0)
        goto _output_error;
    if (strcmp("No error detected", ZSTD_getErrorString(ZSTD_error_no_error)) != 0)
        goto _output_error;
    if (strcmp("Unspecified error code", ZSTD_getErrorString((ZSTD_ErrorCode)(0 - ZSTD_error_GENERIC))) != 0)
        goto _output_error;
    if (strcmp("Error (generic)", ZSTD_getErrorName((size_t)0 - ZSTD_error_GENERIC)) != 0)
        goto _output_error;
    ...
    DISPLAYLEVEL(3,
                 "test%3i : check compression mem usage monotonicity over levels for estimateCCtxSize() : ", testNb++);
    {
        int level = 1;
        size_t prevSize = 0;
        for (; level < ZSTD_maxCLevel(); ++level)
        {
            ...
            {
                DISPLAYLEVEL(
                    3, "Error! previous cctx size: %u at level: %d is larger than current cctx size: %u at level: %d",
    ...
    DISPLAYLEVEL(3,
                 "test%3i : check estimateCCtxSize() always larger or equal to ZSTD_estimateCCtxSize_usingCParams() : ",
                 testNb++);
    {
        ...
        for (; level <= ZSTD_maxCLevel(); ++level)
        {
            ...
            for (; dictSize <= 256 KB; dictSize += 8 * kSizeIncrement)
            {
                ...
                for (; srcSize < 300 KB; srcSize += kSizeIncrement)
                {
                    ...
                    if (cctxSizeUsingLevel < cctxSizeUsingCParams || ZSTD_isError(cctxSizeUsingCParams) ||
                        ZSTD_isError(cctxSizeUsingLevel))
                    {
                        DISPLAYLEVEL(3, "error! l: %d dict: %u srcSize: %u cctx size cpar: %u, cctx size level: %u\n",
                                     level, (unsigned)dictSize, (unsigned)srcSize, (unsigned)cctxSizeUsingCParams,
                                     (unsigned)cctxSizeUsingLevel);
                        goto _output_error;
                    }
                }
            }
...
static int longUnitTests(U32 const seed, double compressibility)
{
    ...
    DISPLAYLEVEL(3, "longtest%3i : table cleanliness through index reduction : ", testNb++);
    {
        ...
        size_t maxIndex =
            ((3U << 29) + (1U << ZSTD_WINDOWLOG_MAX)); /* ZSTD_CURRENT_MAX from zstd_compress_internal.h */
        ...
        for (dictSize = CNBuffSize; dictSize; dictSize = dictSize >> 3)
        {
            ...
            for (cLevel = 4; cLevel < 13; cLevel++)
            {
                for (i = 0; i < 8; ++i)
                {
                    ...
                    cdict = ZSTD_createCDict_advanced2(dict, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto, cctx_params,
                                                       ZSTD_defaultCMem);
                    CHECK(cdict != NULL);
...
static int fuzzerTests(U32 seed,
                       unsigned nbTests,
                       unsigned startTest,
                       U32 const maxDurationS,
                       double compressibility,
                       int bigTests)
{
    ...
    for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < maxClockSpan); testNb++)
    {
        ...
        /* compression tests */
        {
            int const cLevelPositive =
                ...
                1;
            int const cLevel = ((FUZ_rand(&lseed) & 15) == 3) ? -(int)((FUZ_rand(&lseed) & 7) + 1)
            ...
            assert(cSize > 3);
            {
                ...
                DISPLAYLEVEL(5, "fuzzer t%u: compress into too small buffer of size %u (missing %u bytes) \n", testNb,
                             (unsigned)tooSmallSize, (unsigned)missing);
                {
                    size_t const errorCode =
                        ZSTD_compressCCtx(ctx, dstBuffer, tooSmallSize, sampleBuffer, sampleSize, cLevel);
                ...
                }
                {
                    ...
                    CHECK(endCheck != endMark, "ZSTD_compressCCtx : dst buffer overflow  (check.%08X != %08X.mark)",
                          endCheck, endMark);
                }
            }
        }
        ...
        DISPLAYLEVEL(5, "fuzzer t%u: simple decompression test \n", testNb);
        {
            ...
            CHECK(dSize != sampleSize, "ZSTD_decompress failed (%s) (srcSize : %u ; cSize : %u)",
                  ZSTD_getErrorName(dSize), (unsigned)sampleSize, (unsigned)cSize);
            {
        ...
        DISPLAYLEVEL(5, "fuzzer t%u: decompression of truncated source \n", testNb);
        {
            size_t const missing =
                (FUZ_rand(&lseed) % (cSize - 2)) + 1; /* no problem, as cSize > 4 (frameHeaderSizer) */
        ...
        if (cSize > 6)
        {
            ...
            DISPLAYLEVEL(5, "fuzzer t%u: decompress noisy source \n", testNb);
            {
                ...
                memcpy(dstBuffer + sampleSize, &endMark, 4);
                {
                    ...
                    /* result *may* be an unlikely success, but even then, it must strictly respect dst buffer
                     * boundaries */
        ...
        DISPLAYLEVEL(5, "fuzzer t%u: Bufferless streaming compression test \n", testNb);
        {
            ...
            int const cLevel =
                (int)(FUZ_rand(&lseed) % ((U32)ZSTD_maxCLevel() - (MAX(testLog, dictLog) / (U32)cLevelLimiter))) + 1;
            ...
            DISPLAYLEVEL(6, "fuzzer t%u: Compressing up to <=%u bytes at level %i with dictionary size %u \n", testNb,
                         (unsigned)maxTestSize, cLevel, (unsigned)dictSize);
            ...
            else
            {
                ...
                ZSTD_frameParameters const fPar = {
                    FUZ_rand(&lseed) & 1 /* contentSizeFlag */, !(FUZ_rand(&lseed) & 3) /* contentChecksumFlag*/,
