...
static const char *g_stratName[ZSTD_STRATEGY_MAX + 1] = {
    "(none)       ", "ZSTD_fast    ", "ZSTD_dfast   ", "ZSTD_greedy  ", "ZSTD_lazy    ",
...
static const U32 mintable[NUM_PARAMS] = {ZSTD_WINDOWLOG_MIN, ZSTD_CHAINLOG_MIN, ZSTD_HASHLOG_MIN,
                                         ZSTD_SEARCHLOG_MIN, ZSTD_MINMATCH_MIN, ZSTD_TARGETLENGTH_MIN,
                                         ZSTD_STRATEGY_MIN,  FADT_MIN};

/* maximum value of parameters */
static const U32 maxtable[NUM_PARAMS] = {ZSTD_WINDOWLOG_MAX, ZSTD_CHAINLOG_MAX, ZSTD_HASHLOG_MAX,
                                         ZSTD_SEARCHLOG_MAX, ZSTD_MINMATCH_MAX, ZSTD_TARGETLENGTH_MAX,
                                         ZSTD_STRATEGY_MAX,  FADT_MAX};

/* # of values parameters can take on */
static const U32 rangetable[NUM_PARAMS] = {WLOG_RANGE, CLOG_RANGE, HLOG_RANGE, SLOG_RANGE,
                                           MML_RANGE,  TLEN_RANGE, STRT_RANGE, FADT_RANGE};

/* ZSTD_cctxSetParameter() index to set */
static const ZSTD_cParameter cctxSetParamTable[NUM_PARAMS] = {
    ZSTD_c_windowLog, ZSTD_c_chainLog,     ZSTD_c_hashLog,  ZSTD_c_searchLog,
    ZSTD_c_minMatch,  ZSTD_c_targetLength, ZSTD_c_strategy, ZSTD_c_forceAttachDict};

/* names of parameters */
static const char *g_paramNames[NUM_PARAMS] = {"windowLog", "chainLog",     "hashLog",  "searchLog",
                                               "minMatch",  "targetLength", "strategy", "forceAttachDict"};

/* shortened names of parameters */
static const char *g_shortParamNames[NUM_PARAMS] = {"wlog", "clog", "hlog", "slog", "mml", "tlen", "strat", "fadt"};
...
static void paramVariation(paramValues_t *ptr, memoTable_t *mtAll, const U32 nbChanges)
{
    ...
    while (!validated)
    {
        ...
        for (i = 0; i < nbChanges; i++)
        {
            ...
            paramVaryOnce(mtAll[p.vals[strt_ind]].varArray[changeID >> 1], (int)((changeID & 1) << 1) - 1, &p);
        }
...
static double resultScore(const BMK_benchResult_t res, const size_t srcSize, const constraint_t target)
{
    ...
    if (target.cSpeed)
    {
        cs = (double)res.cSpeed / (double)target.cSpeed;
    }
    if (target.dSpeed)
    {
        ds = (double)res.dSpeed / (double)target.dSpeed;
    }
    if (target.cMem != (U32)-1)
    {
        cm = (double)target.cMem / (double)res.cMem;
    }
...
static int compareResultLT(const BMK_benchResult_t result1,
                           ...
                           size_t srcSize)
{
    ...
    return feasible(result2, target) || (!feasible(result1, target) && (resultScore(result1, srcSize, target) <
...
static void optimizerAdjustInput(paramValues_t *pc, const size_t maxBlockSize)
{
    ...
    for (v = 0; v < NUM_PARAMS; v++)
    {
        if (pc->vals[v] != PARAM_UNSET)
        {
            ...
            {
                pc->vals[v] = newval;
                DISPLAY("Warning: parameter %s not in valid range, adjusting to ", g_paramNames[v]);
    ...
    return (ZSTD_estimateCStreamSize_usingCParams(pvalsToCParams(paramValues)) >
           ...
           || (paramValues.vals[clog_ind] >
               (paramValues.vals[wlog_ind] +
                (paramValues.vals[strt_ind] > ZSTD_btlazy2)))                /* chainLog larger than windowLog*/
...
static void BMK_paramValues_into_commandLine(FILE *f, const paramValues_t params)
{
    varInds_t v;
    int first = 1;
    fprintf(f, "--zstd=");
...
static void BMK_displayOneResult(FILE *f, winnerInfo_t res, const size_t srcSize)
{
    ...
    {
        double const ratio = res.result.cSize ? (double)srcSize / (double)res.result.cSize : 0;
...
static void BMK_printWinner(FILE *f,
                            const int cLevel,
                            ...
                            const size_t srcSize)
{
    ...
    if (TIMED)
    {
        ...
        fprintf(f, "%1lu:%2lu:%05.2f - ", (unsigned long)minutes / 60, (unsigned long)minutes % 60,
...
static void BMK_printWinnerOpt(FILE *f,
                               const U32 cLevel,
                               const BMK_benchResult_t result,
                               const paramValues_t params,
                               ...
                               const size_t srcSize)
{
    ...
    if (g_optmode && g_optimizer && (DEBUG || g_displayLevel == 3))
    {
        ...
        fprintf(f, "================================\n");
        fprintf(f, "Level Bounds: R: > %.3f AND C: < %.1f MB/s \n\n", (double)srcSize / (double)g_lvltarget.cSize,
 ...
 */
static void BMK_print_cLevelEntry(FILE *f,
                                  const int cLevel,
                                  paramValues_t params,
                                  const BMK_benchResult_t result,
                                  const size_t srcSize)
 ...
 * this function cannot error.
 */
static void BMK_print_cLevelTable(FILE *f, const winnerInfo_t *winners, const size_t srcSize)
{
    int cLevel;
    ...
    for (cLevel = 0; cLevel <= NB_LEVELS_TRACKED; cLevel++)
        BMK_print_cLevelEntry(f, cLevel, winners[cLevel].params, winners[cLevel].result, srcSize);
}
 ...
 * this function cannot error.
 */
static void BMK_saveAndPrint_cLevelTable(FILE *const f, const winnerInfo_t *winners, const size_t srcSize)
{
    fseek(f, 0, SEEK_SET);
...
static size_t
local_defaultCompress(const void *srcBuffer, size_t srcSize, void *dstBuffer, size_t dstSize, void *addArgs)
{
...
/* additional argument is just the context */
static size_t
local_defaultDecompress(const void *srcBuffer, size_t srcSize, void *dstBuffer, size_t dstSize, void *addArgs)
{
...
static int createBuffersFromMemory(buffers_t *buff, void *srcBuffer, const size_t nbFiles, const size_t *fileSizes)
{
...
static int createBuffers(buffers_t *buff, const char *const *const fileNamesTable, size_t nbFiles)
{
    size_t pos = 0;
    size_t n;
...
sanitizeVarArray(varInds_t *varNew, const size_t varLength, const varInds_t *varArray, const ZSTD_strategy strat)
{
    ...
    {
        if (!((varArray[i] == clog_ind && strat == ZSTD_fast) || (varArray[i] == slog_ind && strat == ZSTD_fast) ||
...
static memoTable_t *createMemoTableArray(const paramValues_t p,
...
/* The purpose is so that sufficient information is returned so that a decompression call to benchMemInvertible is
 * possible */
/* BMK_benchMemAdvanced(srcBuffer,srcSize, dstBuffer, dstSize, fileSizes, nbFiles, 0, &cParams, dictBuffer, dictSize,
 * ctx, dctx, 0, "File", &adv); */
...
static BMK_benchOutcome_t BMK_benchMemInvertible(buffers_t buf,
                                                 contexts_t ctx,
                                                 int cLevel,
                                                 const paramValues_t *comprParams,
                                                 BMK_mode_t mode,
...
static int BMK_benchParam(BMK_benchResult_t *resultPtr, buffers_t buf, contexts_t ctx, paramValues_t cParams)
{
    BMK_benchOutcome_t const outcome = BMK_benchMemInvertible(buf, ctx, BASE_CLEVEL, &cParams, BMK_both, 3);
...
static int allBench(BMK_benchResult_t *resultPtr,
                    ...
                    const constraint_t target,
                    BMK_benchResult_t *winnerResult,
                    int feas)
{
...
static int benchMemo(BMK_benchResult_t *resultPtr,
                     ...
                     const constraint_t target,
                     BMK_benchResult_t *winnerResult,
                     memoTable_t *const memoTableArray,
                     const int feas)
{
    ...
    /* establish speed objectives (relative to level 1) */
    {
        ...
        for (l = 2; l <= NB_LEVELS_TRACKED; l++)
        {
            ...
            g_level_constraint[l].windowLog_max =
                (l < 20) ? 23 : l + 5; /* only --ultra levels >= 20 can use windowlog > 23 */
            g_level_constraint[l].strategy_max = ZSTD_STRATEGY_MAX;
        }
    }
}

static int BMK_seed(winnerInfo_t *winners, const paramValues_t params, const buffers_t buf, const contexts_t ctx)
{
    ...
    for (cLevel = 1; cLevel <= NB_LEVELS_TRACKED; cLevel++)
    {
        ...
        if ((double)testResult.cSize <= ((double)winners[cLevel].result.cSize * (1. + (0.02 / cLevel))))
        {
            ...
            size_t W_CMemUsed =
                ((size_t)1 << params.vals[wlog_ind]) + ZSTD_estimateCCtxSize_usingCParams(pvalsToCParams(params));
            size_t O_CMemUsed = ((size_t)1 << winners[cLevel].params.vals[wlog_ind]) +
            ...
            double O_CSpeed_note =
                O_ratioNote * (double)(30 + 10 * cLevel) + log((double)winners[cLevel].result.cSpeed);
            ...
            if (W_DMemUsed_note < O_DMemUsed_note)
            {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3,
                        "Decompression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ %4.1f MB   : not enough for level %i\n",
                        W_ratio, (double)(W_DMemUsed) / 1024 / 1024, O_ratio, (double)(O_DMemUsed) / 1024 / 1024,
                        cLevel);
                continue;
            }
            if (W_CMemUsed_note < O_CMemUsed_note)
            {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        3, "Compression Memory : %5.3f @ %4.1f MB  vs  %5.3f @ %4.1f MB   : not enough for level %i\n",
                        W_ratio, (double)(W_CMemUsed) / 1024 / 1024, O_ratio, (double)(O_CMemUsed) / 1024 / 1024,
                        cLevel);
                continue;
            }
            if (W_CSpeed_note < O_CSpeed_note)
            {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(
                        ...
                        (double)winners[cLevel].result.cSpeed / MB_UNIT, cLevel);
                continue;
            }
            if (W_DSpeed_note < O_DSpeed_note)
            {
                ...
                if (W_ratio > O_ratio)
                    DISPLAYLEVEL(3,
                                 "Decompression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ %4.1f MB/s   : not enough for "
                                 "level %i\n",
                                 W_ratio, (double)testResult.dSpeed / MB_UNIT, O_ratio,
                                 (double)winners[cLevel].result.dSpeed / MB_UNIT, cLevel);
                continue;
            }
...
static void playAround(FILE *f, winnerInfo_t *winners, paramValues_t p, const buffers_t buf, const contexts_t ctx)
{
    int nbVariations = 0;
    UTIL_time_t const clockStart = UTIL_getTime();
    ...
    while (UTIL_clockSpanMicro(clockStart) < g_maxVariationTime)
    {
        ...
        do
        {
            ...
            for (i = 0; i < 4; i++)
            {
                paramVaryOnce(FUZ_rand(&g_rand) % (strt_ind + 1), ((FUZ_rand(&g_rand) & 1) << 1) - 1, &p);
            }
        } while (!paramValid(p));
...
static void BMK_selectRandomStart(FILE *f, winnerInfo_t *winners, const buffers_t buf, const contexts_t ctx)
{
    U32 const id = FUZ_rand(&g_rand) % (NB_LEVELS_TRACKED + 1);
    if ((id == 0) || (winners[id].params.vals[wlog_ind] == 0))
    {
        /* use some random entry */
        paramValues_t const p =
...
static int benchOnce(const buffers_t buf, const contexts_t ctx, const int cLevel)
{
    BMK_benchResult_t testResult;
    g_params =
        adjustParams(overwriteParams(cParamsToPVals(ZSTD_getCParams(cLevel, buf.maxBlockSize, ctx.dictSize)), g_params),
...
static int benchFiles(const char **fileNamesTable, int nbFiles, const char *dictFileName, int cLevel)
{
    buffers_t buf;
    contexts_t ctx;
    int ret = 0;
        ...
        while (better)
        {
            ...
            for (i = 0; i < varLen; i++)
            {
                for (offset = -1; offset <= 1; offset += 2)
                {
                    ...
                    {
                        int res;
                        res = benchMemo(&candidateInfo.result, buf, ctx, sanitizeParams(candidateInfo.params), target,
            ...
            { /* infeas -> feas -> stop */
                ...
                feas = 1;
                better = 1;
                winnerInfo = bestFeasible1; /* note with change, bestFeasible may not necessarily be feasible, but if
                                               one has been benchmarked, it will be. */
...
/* flexible parameters: iterations of failed climbing (or if we do non-random, maybe this is when everything is close to
   visited) weight more on visit for bad results, less on good results/more on later results / ones with more failures.
   allocate memoTable here.
 */
static winnerInfo_t optimizeFixedStrategy(const buffers_t buf,
                                          const contexts_t ctx,
                                          const constraint_t target,
                                          paramValues_t paramTarget,
                                          const ZSTD_strategy strat,
                                          memoTable_t *memoTableArray,
                                          const int tries)
{
    int i = 0;
                           ...
                           const U32 memoTableLog)
{
    ...
    if (target.cSpeed != 0)
    {
        DISPLAYLEVEL(2, " - limit compression speed %u MB/s", (unsigned)(target.cSpeed >> 20));
    }
    if (target.dSpeed != 0)
    {
        DISPLAYLEVEL(2, " - limit decompression speed %u MB/s", (unsigned)(target.dSpeed >> 20));
    }
    if (target.cMem != (U32)-1)
    {
...

    {
        ...
        /* find best solution from default params */
        {
            ...
            if (paramTarget.vals[strt_ind] == PARAM_UNSET)
            {
                ...
                {
                    int ec;
                    CParams = overwriteParams(cParamsToPVals(ZSTD_getCParams(i, buf.maxBlockSize, ctx.dictSize)),
                                              paramTarget);
        ...
        /* start 'real' optimization */
        {
            ...
            {
                winner =
                    optimizeFixedStrategy(buf, ctx, target, paramBase, paramTarget.vals[strt_ind], allMT, g_maxTries);
            }
        }
...
#define PARSE_SUB_ARGS(stringLong, stringShort, variable)                                      \
    {                                                                                          \
        if (longCommandWArg(&argument, stringLong) || longCommandWArg(&argument, stringShort)) \
        {                                                                                      \
