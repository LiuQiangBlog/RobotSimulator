...
static void freeBuffer(buffer_s b)
{
    free(b.address);
}
...
static buffer_s read_file(const char *path)
{
    ...
    if (!ptr)
        ERR_OUT("failed to allocate memory to hold %s \n", path);

    size_t const read = fread(ptr, 1, size, f);
    if (read != size)
        ERR_OUT("error while reading file %s \n", path);
...
static void write_file(const char *path, const u8 *ptr, size_t size)
{
    ...
    while (written < size)
    {
        ...
        if (ferror(f))
            ERR_OUT("error while writing file %s\n", path);
    }
...
int main(int argc, char **argv)
{
    ...
    if (!output)
        ERR_OUT("failed to allocate memory \n");
    ...
    size_t const decompressed_size =
        ZSTD_decompress_with_dict(output, out_capacity, input.address, input.size, parsed_dict);
