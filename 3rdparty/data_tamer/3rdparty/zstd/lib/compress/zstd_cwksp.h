...
MEM_STATIC void ZSTD_cwksp_assert_internal_consistency(ZSTD_cwksp *ws)
{
    ...
    {
        intptr_t const offset =
            __msan_test_shadow(ws->initOnceStart, (U8 *)ZSTD_cwksp_initialAllocStart(ws) - (U8 *)ws->initOnceStart);
        (void)offset;
...
MEM_STATIC void *ZSTD_cwksp_reserve_internal_buffer_space(ZSTD_cwksp *ws, size_t const bytes)
{
    ...
    DEBUGLOG(5, "cwksp: reserving [0x%p]:%zd bytes; %zd bytes remaining", alloc, bytes,
...
{
    void *const ptr =
        ZSTD_cwksp_reserve_internal(ws, ZSTD_cwksp_align(bytes, ZSTD_CWKSP_ALIGNMENT_BYTES), ZSTD_cwksp_alloc_aligned);
    assert(((size_t)ptr & (ZSTD_CWKSP_ALIGNMENT_BYTES - 1)) == 0);
    return ptr;
}
...
MEM_STATIC void *ZSTD_cwksp_reserve_table(ZSTD_cwksp *ws, size_t bytes)
{
    ...
    DEBUGLOG(5, "cwksp: reserving %p table %zd bytes, %zd bytes remaining", alloc, bytes,
             ZSTD_cwksp_available_space(ws) - bytes);
...
MEM_STATIC void *ZSTD_cwksp_reserve_object(ZSTD_cwksp *ws, size_t bytes)
{
    ...
    DEBUGLOG(4, "cwksp: reserving %p object %zd bytes (rounded to %zd), %zd bytes remaining", alloc, bytes,
             roundedBytes, ZSTD_cwksp_available_space(ws) - roundedBytes);
