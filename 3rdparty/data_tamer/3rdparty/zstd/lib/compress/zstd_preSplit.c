...
static U64 abs64(S64 s64)
{
    return (U64)((s64 < 0) ? -s64 : s64);
}
...
static U64 fpDistance(const Fingerprint *fp1, const Fingerprint *fp2, unsigned hashLog)
{
    ...
    {
        distance += abs64((S64)fp1->events[n] * (S64)fp2->nbEvents - (S64)fp2->events[n] * (S64)fp1->nbEvents);
    }
    return distance;
}
 ...
 */
static int compareFingerprints(const Fingerprint *ref, const Fingerprint *newfp, int penalty, unsigned hashLog)
{
...
static size_t
ZSTD_splitBlock_byChunks(const void *blockStart, size_t blockSize, int level, void *workspace, size_t wkspSize)
{
    static const RecordEvents_f records_fs[] = {FP_RECORD(43), FP_RECORD(11), FP_RECORD(5), FP_RECORD(1)};
    ...
    assert(wkspSize >= sizeof(FPStats));
    (void)wkspSize;
 ...
 */
static size_t ZSTD_splitBlock_fromBorders(const void *blockStart, size_t blockSize, void *workspace, size_t wkspSize)
{
