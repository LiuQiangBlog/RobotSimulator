...
static size_t
ZSTD_ldm_gear_feed(ldmRollingHashState_t *state, BYTE const *data, size_t size, size_t *splits, unsigned *numSplits)
{
    size_t n;
    U64 hash, mask;
...
static ldmEntry_t *ZSTD_ldm_getBucket(const ldmState_t *ldmState, size_t hash, U32 const bucketSizeLog)
{
...
static void
ZSTD_ldm_insertEntry(ldmState_t *ldmState, size_t const hash, const ldmEntry_t entry, U32 const bucketSizeLog)
{
...
static size_t
ZSTD_ldm_countBackwardsMatch(const BYTE *pIn, const BYTE *pAnchor, const BYTE *pMatch, const BYTE *pMatchBase)
{
                                                     ...
                                                     const BYTE *pExtDictEnd)
{
    ...
    DEBUGLOG(7, "ZSTD_ldm_countBackwardsMatch_2segments: found 2-parts backwards match (length in prefix==%zu)",
             matchLength);
...
static ZSTD_ALLOW_POINTER_OVERFLOW_ATTR size_t ZSTD_ldm_generateSequences_internal(ldmState_t *ldmState,
            ...
            for (cur = bucket; cur < bucket + entsPerBucket; cur++)
            {
                ...
                if (extDict)
                {
                    ...
                    curBackwardMatchLength =
                        ZSTD_ldm_countBackwardsMatch_2segments(split, anchor, pMatch, lowMatchPtr, dictStart, dictEnd);
                }
                else
                { /* !extDict */
                                  ...
                                  size_t srcSize)
{
    ...
    for (chunk = 0; chunk < nbChunks && sequences->size < sequences->capacity; ++chunk)
    {
        ...
        if (ZSTD_window_needOverflowCorrection(ldmState->window, 0, maxDist, ldmState->loadedDictEnd, chunkStart,
                                               chunkEnd))
        {
                              ...
                              size_t srcSize)
{
    ...
    while (rawSeqStore->pos < rawSeqStore->size && ip < iend)
    {
        ...
        DEBUGLOG(5, "pos %u : calling block compressor on segment of size %u", (unsigned)(ip - istart),
                 sequence.litLength);
        {
            int i;
            ...
            /* Store the sequence */
            ZSTD_storeSeq(seqStore, newLitLength, ip - newLitLength, iend, OFFSET_TO_OFFBASE(sequence.offset),
