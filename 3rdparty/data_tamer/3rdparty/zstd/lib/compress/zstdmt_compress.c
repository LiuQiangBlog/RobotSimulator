...
static int ZSTDMT_serialState_reset(SerialState *serialState,
...
static void
ZSTDMT_serialState_genSequences(SerialState *serialState, RawSeqStore_t *seqStore, Range src, unsigned jobID)
{
    /* Wait for our turn */
    ...
    if (serialState->nextJobID == jobID)
    {
        ...
        if (serialState->params.ldmParams.enableLdm == ZSTD_ps_enable)
        {
            ...
            error = ZSTD_ldm_generateSequences(&serialState->ldmState, seqStore, &serialState->params.ldmParams,
                                               src.start, src.size);
...
static void ZSTDMT_serialState_applySequences(const SerialState *serialState, /* just for an assert() check */
                                              ZSTD_CCtx *jobCCtx,
...
static void ZSTDMT_serialState_ensureFinished(SerialState *serialState, unsigned jobID, size_t cSize)
{
    ZSTD_PTHREAD_MUTEX_LOCK(&serialState->mutex);
    if (serialState->nextJobID <= jobID)
    {
...
static void ZSTDMT_compressionJob(void *jobDescription)
{
    ...
    else
    {
        ...
        DEBUGLOG(6, "ZSTDMT_compressionJob: job %u: loading prefix of size %zu", job->jobID, job->prefix.size);
        {
            size_t const initError = ZSTD_compressBegin_advanced_internal(
                cctx, job->prefix.start, job->prefix.size, ZSTD_dct_rawContent, ZSTD_dtlm_fast, NULL, /*cdict*/
    ...
    /* compress the entire job by smaller chunks, for better granularity */
    {
        ...
        for (chunkNb = 1; chunkNb < nbChunks; chunkNb++)
        {
            ...
            DEBUGLOG(5, "ZSTDMT_compressionJob: compress new block : cSize==%u bytes (total: %u)", (U32)cSize,
                     (U32)job->cSize);
...
void ZSTDMT_updateCParams_whileCompressing(ZSTDMT_CCtx *mtctx, const ZSTD_CCtx_params *cctxParams)
{
    ...
    mtctx->params.compressionLevel = compressionLevel;
    {
        ZSTD_compressionParameters cParams =
            ZSTD_getCParamsFromCCtxParams(cctxParams, ZSTD_CONTENTSIZE_UNKNOWN, 0, ZSTD_cpm_noAttachDict);
...
ZSTD_frameProgression ZSTDMT_getFrameProgression(ZSTDMT_CCtx *mtctx)
{
    ...
    fps.nbActiveWorkers = 0;
    {
        ...
        DEBUGLOG(6, "ZSTDMT_getFrameProgression: jobs: from %u to <%u (jobReady:%u)", mtctx->doneJobID, lastJobNb,
                 mtctx->jobReady);
...
{
    DEBUGLOG(4, "ZSTDMT_initCStream_internal (pledgedSrcSize=%u, nbWorkers=%u, cctxPool=%u)", (U32)pledgedSrcSize,
             params.nbWorkers, mtctx->cctxPool->totalCCtx);
    ...
    ZSTDMT_setBufferSize(mtctx->bufPool, ZSTD_compressBound(mtctx->targetSectionSize));
    {
        ...
        size_t const windowSize =
            mtctx->params.ldmParams.enableLdm == ZSTD_ps_enable ? (1U << mtctx->params.cParams.windowLog) : 0;
    ...
    if (dict)
    {
        ...
        else
        {
            ...
            mtctx->cdictLocal =
                ZSTD_createCDict_advanced(dict, dictSize, ZSTD_dlm_byRef, dictContentType, params.cParams, mtctx->cMem);
            mtctx->cdict = mtctx->cdictLocal;
            if (mtctx->cdictLocal == NULL)
                return ERROR(memory_allocation);
        }
...
static size_t ZSTDMT_createCompressionJob(ZSTDMT_CCtx *mtctx, size_t srcSize, ZSTD_EndDirective endOp)
{
    ...
    DEBUGLOG(5, "ZSTDMT_createCompressionJob: posting job %u : %u bytes  (end:%u, jobNb == %u (mod:%u))",
             mtctx->nextJobID, (U32)mtctx->jobs[jobID].src.size, mtctx->jobs[jobID].lastJob, mtctx->nextJobID, jobID);
...
ZSTDMT_flushProduced(ZSTDMT_CCtx *mtctx, ZSTD_outBuffer *output, unsigned blockToFlush, ZSTD_EndDirective end)
{
    ...
    if (blockToFlush && (mtctx->doneJobID < mtctx->nextJobID))
    {
        ...
        { /* nothing to flush */
            ...
            DEBUGLOG(5, "waiting for something to flush from job %u (currently flushed: %u bytes)", mtctx->doneJobID,
    ...
    /* try to flush something */
    {
        ...
        if (ZSTD_isError(cSize))
        {
            DEBUGLOG(5, "ZSTDMT_flushProduced: job %u : compression error detected : %s", mtctx->doneJobID,
        ...
        { /* compression is ongoing or completed */
            ...
            if (toFlush > 0)
            {
                ZSTD_memcpy((char *)output->dst + output->pos,
                            (const char *)mtctx->jobs[wJobID].dstBuff.start + mtctx->jobs[wJobID].dstFlushed, toFlush);
            }
            output->pos += toFlush;
...
size_t
ZSTDMT_compressStream_generic(ZSTDMT_CCtx *mtctx, ZSTD_outBuffer *output, ZSTD_inBuffer *input, ZSTD_EndDirective endOp)
{
    ...
    /* fill input buffer */
    if ((!mtctx->jobReady) && (input->size > input->pos))
    { /* support NULL input */
        if (mtctx->inBuff.buffer.start == NULL)
        {
            ...
            if (!ZSTDMT_tryGetInputRange(mtctx))
            {
                ...
                DEBUGLOG(5, "ZSTDMT_tryGetInputRange completed successfully : mtctx->inBuff.buffer.start = %p",
