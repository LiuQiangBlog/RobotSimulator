...
#define ZBUFF_DEPRECATED(message) ZSTDLIB_API
    ...
    size_t ZBUFF_compressContinue(ZBUFF_CCtx *cctx,
                                  void *dst,
     ...
     *  Note that it may not consume the entire input, in which case it's up to the caller to present again remaining
     * data. The content of `dst` will be overwritten (up to *dstCapacityPtr) at each call, so save its content if it
     ...
     *  Similar to ZBUFF_compressFlush(), it may not be able to output the entire internal buffer content if
     * *dstCapacityPtr is too small. In which case, call again ZBUFF_compressFlush() to complete the flush.
     *  @return : nb of bytes still present into internal buffer (0 if it's empty)
     *            or an error code, which can be tested using ZBUFF_isError().
     *
     ...
     * stages (better latency) output : ZBUFF_recommendedCOutSize==ZSTD_compressBound(128 KB) + 3 + 3 : ensures it's
     * always possible to write/flush/end a full block. Skip some buffering. By using both, it ensures that input will
     * be entirely consumed, and output will always contain the result, reducing intermediate buffering.
     * **************************************************/
     ...
     *            >1 when more data is expected, with value being a suggested next input size (it's just a hint, which
     * helps latency), or an error code, which can be tested using ZBUFF_isError().
     *
