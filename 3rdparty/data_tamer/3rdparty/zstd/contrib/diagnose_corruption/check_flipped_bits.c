...
static void print_summary(stuff_t *stuff)
{
    ...
    for (error_code = 0; error_code < ZSTD_error_maxCode; error_code++)
    {
        ...
        if (count)
        {
            fprintf(stderr, "%9d failed decompressions with message: %s\n", count, ZSTD_getErrorString(error_code));
        }
    }
}
...
static ZSTD_DDict *readDictByID(stuff_t *stuff, int32_t dict_id, char **buf, size_t *size)
{
    ...
    else
    {
        ...
        size_t dict_file_name_alloc_size =
            dir_name_len + 1 /* '/' */ + 10 /* max int32_t len */ + strlen(".zstd-dict") + 1 /* '\0' */;
        ...
        snprintf(dict_file_name, dict_file_name_alloc_size, "%s%s%u.zstd-dict", stuff->dict_file_dir_name,
                 dir_needs_separator ? "/" : "", dict_id);
