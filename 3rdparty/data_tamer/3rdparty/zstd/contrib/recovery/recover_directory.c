...
static ZstdFrames readFile(char const *fileName)
{
    ...
    {
        size_t const frameSize = ZSTD_findFrameCompressedSize(data + index, fileSize - index);
        CHECK(!ZSTD_isError(frameSize), "Bad zstd frame: %s", ZSTD_getErrorName(frameSize));
...
int main(int argc, char **argv)
{
    ...
    if (frames.frames <= 1)
    {
        fprintf(stderr, "%s only has %u zstd frame. Simply use `zstd -d` to decompress it.\n", zstdFile,
    ...
    {
        size_t const frameSize = ZSTD_findFrameCompressedSize(frames.data + index, frames.size - index);
