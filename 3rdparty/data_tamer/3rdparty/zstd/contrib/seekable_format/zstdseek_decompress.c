 ...
 * (https://pubs.opengroup.org/onlinepubs/7908799/xsh/unistd.h.html). note : there is no simple way to know in advance
 * if <unistd.h> is present or not on target system, Posix specification mandates its presence and its content, but
 * target system must respect this spec. It's necessary to _not_ #include <unistd.h> whenever target OS is not unix-like
...
size_t ZSTD_seekTable_getFrameCompressedSize(const ZSTD_seekTable *st, unsigned frameIndex)
{
    ...
    return st->entries[frameIndex + 1].cOffset - st->entries[frameIndex].cOffset;
}
...
size_t ZSTD_seekable_decompress(ZSTD_seekable *zs, void *dst, size_t len, unsigned long long offset)
{
    ...
    do
    {
        ...
        if (targetFrame != zs->curFrame || offset < zs->decompressedOffset)
        {
            ...
            CHECK_IO(zs->src.seek(zs->src.opaque, (long long)zs->seekTable.entries[targetFrame].cOffset, SEEK_SET));
        ...
        while (zs->decompressedOffset < offset + len)
        {
            ...
            if (zs->decompressedOffset < offset)
            {
                ...
                outTmp = (ZSTD_outBuffer){zs->outBuff,
                                          (size_t)(MIN(SEEKABLE_BUFF_SIZE, offset - zs->decompressedOffset)), 0};
            }
            else
            {
