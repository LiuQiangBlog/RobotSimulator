...
size_t
ZSTD_seekable_initCStream(ZSTD_seekable_CStream *zcs, int compressionLevel, int checksumFlag, unsigned maxFrameSize)
{
    zcs->framelog.size = 0;
    zcs->frameCSize = 0;
    zcs->frameDSize = 0;
...
size_t ZSTD_seekable_logFrame(ZSTD_frameLog *fl, unsigned compressedSize, unsigned decompressedSize, unsigned checksum)
{
    ...
    if (fl->size == fl->capacity)
    {
        ...
        framelogEntry_t *const newEntries =
            (framelogEntry_t *)realloc(fl->entries, sizeof(framelogEntry_t) * newCapacity);
...
size_t ZSTD_seekable_endFrame(ZSTD_seekable_CStream *zcs, ZSTD_outBuffer *output)
{
    ...
    ret = ZSTD_seekable_logFrame(&zcs->framelog, zcs->frameCSize, zcs->frameDSize,
