...
static int
loadFiles(void *buffer, size_t bufferSize, size_t *fileSizes, const char *const *fileNamesTable, unsigned nbFiles)
{
...
static buffer_collection_t createBufferCollection_fromSliceCollection(slice_collection_t sc)
{
    size_t const bufferSize = sliceCollection_totalCapacity(sc);
...
static cdict_collection_t createCDictCollection(const void *dictBuffer,
                                                size_t dictSize,
                                                size_t nbCDict,
                                                ...
                                                ZSTD_CCtx_params *cctxParams)
{
    ...
    if (cdicts == NULL)
        return kNullCDictCollection;
    for (size_t dictNb = 0; dictNb < nbCDict; dictNb++)
    {
        cdicts[dictNb] = ZSTD_createCDict_advanced2(dictBuffer, dictSize, DICT_LOAD_METHOD, dictContentType, cctxParams,
                                                    ZSTD_defaultCMem);
               ...
               int cLevel)
{
    ...
    for (size_t blockNb = 0; blockNb < nbBlocks; blockNb++)
    {
        ...
        {
            cBlockSize =
                ZSTD_compressCCtx(cctx, dstBlockBuffers.slicePtrs[blockNb], dstBlockBuffers.capacities[blockNb],
                                  srcBlockBuffers.slicePtrs[blockNb], srcBlockBuffers.capacities[blockNb], cLevel);
        }
        else
        {
            cBlockSize = ZSTD_compress_usingCDict(
                cctx, dstBlockBuffers.slicePtrs[blockNb], dstBlockBuffers.capacities[blockNb],
                srcBlockBuffers.slicePtrs[blockNb], srcBlockBuffers.capacities[blockNb], cdict);
        }
...
size_t compress(const void *src, size_t srcSize, void *dst, size_t dstCapacity, void *payload)
{
    ...
    ZSTD_compress2(ci->cctx, dst, srcSize, src, srcSize);

    ci->dictNb = ci->dictNb + 1;
    if (ci->dictNb >= ci->nbDicts)
        ci->dictNb = 0;
...
double aggregateData(double *data, size_t size, metricAggregatePref_e metricAggregatePref)
{
                    ...
                    metricAggregatePref_e metricAggregatePref)
{
    ...
    BMK_benchParams_t const bp = {.benchFn = benchCompression ? compress : decompress,
    ...
    double speedAggregated = aggregateData(speedPerRound, roundNb + 1, metricAggregatePref);
    if (metricAggregatePref == fastest)
    ...
    if (!csvFile)
    {
        ...
        fprintf(csvFile, "%s\n", exeName);
        /* Print table headers */
        fprintf(csvFile, "Compression/Decompression,Level,nbDicts,dictAttachPref,metricAggregatePref,Speed\n");
    }
    else
    {
        fclose(csvFile);
    ...
    if (benchCompression)
    {
        ZSTD_CCtxParams_getParameter(cctxParams, ZSTD_c_compressionLevel, &cLevel);
        ZSTD_CCtxParams_getParameter(cctxParams, ZSTD_c_forceAttachDict, &dictAttachPref);
    }
    fprintf(csvFile, "%s,%d,%ld,%d,%d,%.1f\n", benchCompression ? "Compression" : "Decompression", cLevel,
            benchCompression ? ci.nbDicts : di.nbDicts, dictAttachPref, metricAggregatePref, speedAggregated);
    fclose(csvFile);
    free(csvFileName);
          ...
          metricAggregatePref_e metricAggregatePref)
{
    ...
    buffer_t const dictBuffer =
        createDictionaryBuffer(dictionary, srcs.buffer.ptr, srcSlices.capacities, srcSlices.nbSlices, DICTSIZE);
    CONTROL(dictBuffer.ptr != NULL);

    ZSTD_CDict *const cdict = ZSTD_createCDict_advanced2(dictBuffer.ptr, dictBuffer.size, DICT_LOAD_METHOD,
                                                         dictContentType, cctxParams, ZSTD_defaultCMem);
    CONTROL(cdict != NULL);
    ...
    cdict_collection_t const cdictionaries =
        createCDictCollection(dictBuffer.ptr, dictBuffer.size, nbDicts, dictContentType, cctxParams);
    ...
    if (benchCompression)
    {
        ...
        result = benchMem(dstSlices, resultCollection.slices, ddictionaries, cdictionaries, nbRounds, benchCompression,
                          exeName, cctxParams, metricAggregatePref);
    ...
    else
    {
        ...
        result = benchMem(resultCollection.slices, dstSlices, ddictionaries, cdictionaries, nbRounds, benchCompression,
                          exeName, NULL, metricAggregatePref);
...
int main(int argc, const char **argv)
{
    ...
    for (int argNb = 1; argNb < argc; argNb++)
    {
        ...
        if (longCommandWArg(&argument, "-p"))
        {
            metricAggregatePref = (int)readU32FromChar(&argument);
            continue;
        }
        ...
        if (longCommandWArg(&argument, "--dict-content-type="))
        {
            dictContentType = (int)readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--dict-attach-pref="))
        {
            dictAttachPref = (int)readU32FromChar(&argument);
            continue;
        }
        if (longCommandWArg(&argument, "--prefetch-cdict-tables="))
        {
            prefetchCDictTables = (int)readU32FromChar(&argument);
            continue;
        }
    ...
    int result =
        bench(filenameTable->fileNames, (unsigned)filenameTable->tableSize, dictionary, blockSize, cLevel, nbDicts,
              nbBlocks, nbRounds, benchCompression, dictContentType, cctxParams, exeName, metricAggregatePref);
