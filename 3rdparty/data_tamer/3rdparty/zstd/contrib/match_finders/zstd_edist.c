    ...
    for (iterations = 1;; iterations++)
    {
        ...
        for (diag = forwardMax; diag >= forwardMin; diag -= 2)
        {
            ...
            for (dictIdx = dictIdx0, srcIdx = dictIdx0 - diag;
                 dictIdx < dictHigh && srcIdx < srcHigh && dict[dictIdx] == src[srcIdx]; dictIdx++, srcIdx++)
                continue;
        ...
        for (diag = backwardMax; diag >= backwardMin; diag -= 2)
        {
            ...
            for (dictIdx = dictIdx0, srcIdx = dictIdx0 - diag;
                 dictLow < dictIdx && srcLow < srcIdx && dict[dictIdx - 1] == src[srcIdx - 1]; dictIdx--, srcIdx--)
                continue;
        ...
        {
            {
                ...
                for (diag = forwardMax; diag >= forwardMin; diag -= 2)
                {
                    ...
                    {
                        if (v > best && dictLow + ZSTD_EDIST_SNAKE_THRESH <= dictIdx && dictIdx <= dictHigh &&
...

            {
                ...
                for (diag = backwardMax; diag >= backwardMin; diag -= 2)
                {
                    ...
                    if (v > 12 * (iterations + (diagDiag < 0 ? -diagDiag : diagDiag)))
                    {
                        if (v > best && dictLow < dictIdx && dictIdx <= dictHigh - ZSTD_EDIST_SNAKE_THRESH &&
...
static void ZSTD_eDist_insertMatch(ZSTD_eDist_state *state, S32 const dictIdx, S32 const srcIdx)
{
...
static int
ZSTD_eDist_compare(ZSTD_eDist_state *state, S32 dictLow, S32 dictHigh, S32 srcLow, S32 srcHigh, int useHeuristics)
{
    const BYTE *const dict = state->dict;
    const BYTE *const src = state->src;
    ...
    else
    {
        ...
        partition.dictMid = 0;
        partition.srcMid = 0;
        ZSTD_eDist_diag(state, &partition, dictLow, dictHigh, srcLow, srcHigh, useHeuristics);
        if (ZSTD_eDist_compare(state, dictLow, partition.dictMid, srcLow, partition.srcMid, partition.lowUseHeuristics))
            return 1;
        if (ZSTD_eDist_compare(state, partition.dictMid, dictHigh, partition.srcMid, srcHigh,
                               partition.highUseHeuristics))
            return 1;
    }
...
static void ZSTD_eDist_combineMatches(ZSTD_eDist_state *state)
{
    ...
    ZSTD_eDist_match *combinedMatches = ZSTD_malloc(state->nbMatches * sizeof(ZSTD_eDist_match), ZSTD_defaultCMem);
    ...
    for (i = 1; i < state->nbMatches; i++)
    {
        ...
        ZSTD_eDist_match const combinedMatch = combinedMatches[nbCombinedMatches - 1];
        ...
        else
        {
            ...
            memcpy(combinedMatches + nbCombinedMatches, state->matches + i, sizeof(ZSTD_eDist_match));
            nbCombinedMatches++;
        }
    }
...
static size_t ZSTD_eDist_convertMatchesToSequences(ZSTD_Sequence *sequences, ZSTD_eDist_state *state)
{
    ...
    for (i = 0; i < nbMatches; i++)
    {
        ...
        U32 const litLength = !i ? match.srcIdx : match.srcIdx - (matches[i - 1].srcIdx + matches[i - 1].matchLength);
...
static size_t ZSTD_eDist_hamingDist(const BYTE *const a, const BYTE *const b, size_t n)
{
    size_t i;
    size_t dist = 0;
    for (i = 0; i < n; i++)
...
static size_t ZSTD_eDist_levenshteinDist(const BYTE *const s, size_t const sn, const BYTE *const t, size_t const tn)
{
    size_t a, b, c;
    ...
    if (s[sn - 1] == t[tn - 1])
        return ZSTD_eDist_levenshteinDist(s, sn - 1, t, tn - 1);
...
static void ZSTD_eDist_validateMatches(ZSTD_eDist_match *matches,
                                       size_t const nbMatches,
                                       const BYTE *const dict,
