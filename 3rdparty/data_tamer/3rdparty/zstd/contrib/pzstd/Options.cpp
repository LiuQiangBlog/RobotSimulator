...
Options::Options()
    : numThreads(defaultNumThreads()), maxWindowLog(23), compressionLevel(kDefaultCompressionLevel), decompress(false),
      overwrite(false), keepSource(true), writeMode(WriteMode::Auto), checksum(true), verbosity(2)
{
}
...
Options::Status Options::parse(int argc, const char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        while (!finished && *options != 0)
        {
            ...
            case 'p':
            {
                ...
                if (*optionArgument != 0)
                {
                    std::fprintf(stderr, "Option -p expects a number, but %u%s provided\n", numThreads, optionArgument);
                    return Status::Failure;
                }
                break;
            }
    ...
    if (recursive)
    {
        FileNamesTable *const files =
            UTIL_createExpandedFNT(localInputFiles.data(), localInputFiles.size(), followLinks);
        if (files == nullptr)
        {
