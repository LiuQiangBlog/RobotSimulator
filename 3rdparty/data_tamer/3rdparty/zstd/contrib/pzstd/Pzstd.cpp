                                    ...
                                    SharedState &state)
{
    ...
    std::uint64_t bytesWritten;
    {
        ...
        if (!options.decompress)
        {
            ...
            readExecutor.add(
                ...
                {
                    bytesRead = asyncCompressChunks(state, outs, executor, inputFd, inputSize, options.numThreads,
        ...
        else
        {
            ...
            readExecutor.add(
                [&state, &outs, &executor, inputFd, &bytesRead]
                {
    ...
    if (!state.errorHolder.hasError())
    {
        ...
        if (!options.decompress)
        {
            ...
            state.log(kLogInfo, "%-20s :%6.2f%%   (%6" PRIu64 " => %6" PRIu64 " bytes, %s)\n", inputFileName.c_str(),
                      ratio * 100, bytesRead, bytesWritten, outputFileName.c_str());
        }
        else
        {
...
static FILE *openOutputFile(const Options &options, const std::string &outputFile, SharedState &state)
{
    ...
    if (!options.overwrite && outputFile != nullOutput)
    {
        ...
        if (outputFd != nullptr)
        {
            ...
            state.log(kLogInfo, "pzstd: %s already exists; do you wish to overwrite (y/n) ? ", outputFile.c_str());
            int c = getchar();
            if (c != 'y' && c != 'Y')
            {
 ...
 */
static FileStatus
readData(BufferWorkQueue &queue, size_t chunkSize, size_t size, FILE *fd, std::uint64_t *totalBytesRead)
{
    Buffer buffer(size);
    while (!buffer.empty())
    {
...
{
    auto &errorHolder = state.errorHolder;
    auto outsFinishGuard = makeScopeGuard(
        [&outs]
        {
            outs.finish();
        });
    auto lineClearGuard = makeScopeGuard(
        [&state]
        {
    ...
    while (outs.pop(out))
    {
        auto outFinishGuard = makeScopeGuard(
            [&out]
            {
                out->finish();
            });
        ...
        while (out->pop(buffer) && !errorHolder.hasError())
        {
            ...
            state.log.update(kLogInfo, "Written: %u MB   ", static_cast<std::uint32_t>(bytesWritten >> 20));
        }
    }
    return bytesWritten;
}
