...
IWritable::IWritable() noexcept : crc_(internal::CRC32_INIT) {}
...
StreamWriter::StreamWriter(std::ostream &stream) : stream_(stream), size_(0) {}
...
LZ4Writer::LZ4Writer(CompressionLevel compressionLevel, uint64_t chunkSize) : compressionLevel_(compressionLevel)
{
...
void LZ4Writer::end()
{
    ...
    const auto dstSize = LZ4F_compressFrame(compressedBuffer_.data(), dstCapacity, uncompressedBuffer_.data(),
...
ZStdWriter::ZStdWriter(CompressionLevel compressionLevel, uint64_t chunkSize)
{
    ...
    ZSTD_CCtx_setParameter(zstdContext_, ZSTD_c_compressionLevel, internal::ZStdCompressionLevel(compressionLevel));
    uncompressedBuffer_.reserve(chunkSize);
}
...
void ZStdWriter::end()
{
    ...
    if (ZSTD_isError(dstSize))
    {
        ...
        std::cerr << "ZSTD_compress2 failed: " << ZSTD_getErrorName(dstSize) << " (" << ZSTD_getErrorString(errCode)
                  << ")\n";
        std::abort();
    }
...
void McapWriter::close()
{
    ...
    if (!options_.noSummary)
    {
        ...
        if (!options_.noSummaryOffsets)
        {
            ...
            {
                write(fileOutput, SummaryOffset{OpCode::Channel, channelStart, statisticsStart - channelStart});
            }
            if (!options_.noStatistics)
            {
                write(fileOutput,
                      SummaryOffset{OpCode::Statistics, statisticsStart, chunkIndexStart - statisticsStart});
            }
            if (!options_.noChunkIndex && !chunkIndex_.empty())
            {
                write(fileOutput,
                      SummaryOffset{OpCode::ChunkIndex, chunkIndexStart, attachmentIndexStart - chunkIndexStart});
            }
...
Status McapWriter::write(const Message &message)
{
    ...
    if (channelMessageCounts.find(message.channelId) == channelMessageCounts.end())
    {
        ...
        if ((channel.schemaId != 0) && (writtenSchemas_.find(channel.schemaId) == writtenSchemas_.end()))
        {
...
Status McapWriter::write(Attachment &attachment)
{
    ...
    if (!options_.noAttachmentCRC)
    {
        ...
        crc = internal::crc32Update(crc, reinterpret_cast<const std::byte *>(attachment.name.data()), sizePrefix);
        sizePrefix = uint32_t(attachment.mediaType.size());
        crc = internal::crc32Update(crc, reinterpret_cast<const std::byte *>(&sizePrefix), 4);
        crc = internal::crc32Update(crc, reinterpret_cast<const std::byte *>(attachment.mediaType.data()), sizePrefix);
        crc = internal::crc32Update(crc, reinterpret_cast<const std::byte *>(&attachment.dataSize), 8);
        crc = internal::crc32Update(crc, reinterpret_cast<const std::byte *>(attachment.data), attachment.dataSize);
...
void McapWriter::writeChunk(IWritable &output, IChunkWriter &chunkData)
{
    ...
    write(output, Chunk{currentChunkStart_, currentChunkEnd_, uncompressedSize, uncompressedCrc, compressionStr,
...
uint64_t McapWriter::write(IWritable &output, const Attachment &attachment)
{
    const uint64_t recordSize =
        4 + attachment.name.size() + 8 + 8 + 4 + attachment.mediaType.size() + 8 + attachment.dataSize + 4;
...
{
    const uint64_t recordSize = 8 + 8 + 8 + 4 + 4 + chunk.compression.size() + 8 + chunk.compressedSize;
