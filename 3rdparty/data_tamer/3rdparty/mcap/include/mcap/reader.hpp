...
public:
     ...
     */
    static Status
    DecompressAll(const std::byte *data, uint64_t compressedSize, uint64_t uncompressedSize, ByteArray *output);
    ZStdReader() = default;
...
public:
     ...
     */
    LinearMessageView
    readMessages(const ProblemCallback &onProblem, Timestamp startTime = 0, Timestamp endTime = MaxTime);
...
{
public:
    IndexedMessageReader(McapReader &reader,
...
struct MCAP_PUBLIC LinearMessageView
{
    ...
    LinearMessageView(McapReader &mcapReader,
