...
inline Status ParseKeyValueMap(const std::byte *data, uint64_t maxSize, KeyValueMap *output)
{
    ...
    while (pos < sizeInBytes)
    {
        ...
        {
            const auto msg =
                StrCat("cannot read key-value map value for key \"", key, "\" at pos ", pos, ": ", status.message);
            return Status{StatusCode::InvalidRecord, msg};
        }
        pos += 4 + value.size();
...
{
    return internal::ToHex(data[0]) + internal::ToHex(data[1]) + internal::ToHex(data[2]) + internal::ToHex(data[3]) +
           internal::ToHex(data[4]) + internal::ToHex(data[5]) + internal::ToHex(data[6]) + internal::ToHex(data[7]);
}
