...
FileReader::FileReader(std::FILE *file) : file_(file), size_(0), position_(0)
{
    assert(file_);
...
FileStreamReader::FileStreamReader(std::ifstream &stream) : stream_(stream), position_(0)
{
...
LZ4Reader::LZ4Reader()
{
    const LZ4F_errorCode_t err = LZ4F_createDecompressionContext((LZ4F_dctx **)&decompressionContext_, LZ4F_VERSION);
    if (LZ4F_isError(err))
    {
        const auto msg = internal::StrCat("failed to create lz4 decompression context: ", LZ4F_getErrorName(err));
...
Status
LZ4Reader::decompressAll(const std::byte *data, uint64_t compressedSize, uint64_t uncompressedSize, ByteArray *output)
{
    if (!decompressionContext_)
    {
        return status_;
    ...
    const auto status =
        LZ4F_decompress((LZ4F_dctx *)decompressionContext_, output->data(), &dstSize, data, &srcSize, nullptr);
    if (status != 0)
    {
        if (LZ4F_isError(status))
        {
            const auto msg =
                internal::StrCat("lz4 decompression of ", compressedSize, " bytes into ", uncompressedSize,
                                 " output bytes failed with error ", (int)status, " (", LZ4F_getErrorName(status), ")");
            result = Status{StatusCode::DecompressionFailed, msg};
        }
        else
        {
            const auto msg = internal::StrCat("lz4 decompression of ", compressedSize, " bytes into ", uncompressedSize,
...
Status
ZStdReader::DecompressAll(const std::byte *data, uint64_t compressedSize, uint64_t uncompressedSize, ByteArray *output)
{
    auto result = Status();
    ...
    if (status != uncompressedSize)
    {
        ...
        {
            const auto msg = internal::StrCat("zstd decompression of ", compressedSize, " bytes into ",
                                              uncompressedSize, " output bytes only produced ", status, " bytes");
            result = Status{StatusCode::DecompressionSizeMismatch, msg};
        }
        output->clear();
    }
...
Status McapReader::open(IReadable &reader)
{
    ...
    {
        const auto msg = internal::StrCat("invalid magic bytes in Header: 0x", internal::MagicToHex(data));
    ...
    {
        const auto msg =
            internal::StrCat("invalid opcode, expected Header: 0x", internal::ToHex(uint8_t(record.opcode)));
...
Status McapReader::readSummarySection_(IReadable &reader)
{
    ...
    const ByteOffset summaryStart = footer.summaryStart != 0 ? footer.summaryStart : fileSize - internal::FooterLength;
    ...
    {
        const auto msg =
            internal::StrCat("summary_offset_start ", summaryOffsetStart, " < summary_start ", summaryStart);
    ...
    typedReader.onChunkIndex = [&](const ChunkIndex chunkIndex, ByteOffset)
    {
        ...
        if (std::binary_search(chunkIndexes_.begin(), chunkIndexes_.end(), chunkIndex, CompareChunkIndexes))
        {
            return;
        }
...
Status McapReader::readSummaryFromScan_(IReadable &reader)
{
    ...
    typedReader.onChunk = [&](const Chunk &chunk, ByteOffset fileOffset)
    {
        ...
        chunkIndex.chunkLength = 9 + 8 + 8 + 8 + 4 + 4 + chunk.compression.size() + 8 + chunk.compressedSize;
...
LinearMessageView McapReader::readMessages(const ProblemCallback &onProblem, Timestamp startTime, Timestamp endTime)
{
...
LinearMessageView McapReader::readMessages(const ProblemCallback &onProblem, const ReadMessageOptions &options)
{
...
std::pair<ByteOffset, ByteOffset> McapReader::byteRange(Timestamp startTime, Timestamp endTime) const
{
    ...
    chunkRanges_.visit_overlapping(startTime, endTime,
                                   [&](const auto &interval)
                                   {
...
Status McapReader::ReadRecord(IReadable &reader, uint64_t offset, Record *record)
{
    ...
    if (maxSize < 9)
    {
        const auto msg = internal::StrCat("cannot read record at offset ", offset, ", ", maxSize, " bytes remaining");
        return Status{StatusCode::InvalidFile, msg};
    }
    ...
    {
        const auto msg =
            internal::StrCat("record type 0x", internal::ToHex(uint8_t(record->opcode)), " at offset ", offset,
                             " has length ", record->dataSize, " but only ", maxSize, " bytes remaining");
    ...
    {
        const auto msg = internal::StrCat("attempted to read ", record->dataSize, " bytes for record type 0x",
...
Status McapReader::ReadFooter(IReadable &reader, uint64_t offset, Footer *footer)
{
    ...
    {
        const auto msg = internal::StrCat("invalid magic bytes in Footer: 0x",
    ...
    {
        const auto msg = internal::StrCat("invalid opcode, expected Footer: 0x", internal::ToHex(data[0]));
        return Status{StatusCode::InvalidFile, msg};
    }
...
Status McapReader::ParseHeader(const Record &record, Header *header)
{
    ...
    if (auto status = internal::ParseString(record.data, record.dataSize, &header->profile); !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseSchema(const Record &record, Schema *schema)
{
    ...
    offset += 2;
    // name
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &schema->name);
        !status.ok())
    {
        return status;
    }
    ...
    // encoding
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &schema->encoding);
        !status.ok())
    {
        return status;
    }
    ...
    // data
    if (auto status = internal::ParseByteArray(record.data + offset, record.dataSize - offset, &schema->data);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseChannel(const Record &record, Channel *channel)
{
    ...
    offset += 2;
    // topic
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &channel->topic);
        !status.ok())
    {
        return status;
    }
    ...
    // message_encoding
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &channel->messageEncoding);
        !status.ok())
    {
        return status;
    }
    ...
    // metadata
    if (auto status = internal::ParseKeyValueMap(record.data + offset, record.dataSize - offset, &channel->metadata);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseChunk(const Record &record, Chunk *chunk)
{
    ...
    // compression
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &chunk->compression);
        !status.ok())
    {
        return status;
    }
    ...
    // compressed_size
    if (auto status = internal::ParseUint64(record.data + offset, record.dataSize - offset, &chunk->compressedSize);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseChunkIndex(const Record &record, ChunkIndex *chunkIndex)
{
    ...
    if (messageIndexOffsetsSize % 10 != 0 || messageIndexOffsetsSize > record.dataSize - PreambleSize)
    {
        const auto msg = internal::StrCat("invalid ChunkIndex.message_index_offsets length:", messageIndexOffsetsSize);
        return Status{StatusCode::InvalidRecord, msg};
    }
    ...
    if (auto status =
            internal::ParseUint64(record.data + offset, record.dataSize - offset, &chunkIndex->messageIndexLength);
        !status.ok())
    {
        return status;
    }
    offset += 8;
    // compression
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &chunkIndex->compression);
        !status.ok())
    {
        return status;
    }
    ...
    // compressed_size
    if (auto status =
            internal::ParseUint64(record.data + offset, record.dataSize - offset, &chunkIndex->compressedSize);
        !status.ok())
    {
        return status;
    }
    ...
    // uncompressed_size
    if (auto status =
            internal::ParseUint64(record.data + offset, record.dataSize - offset, &chunkIndex->uncompressedSize);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseAttachment(const Record &record, Attachment *attachment)
{
    ...
    uint32_t offset = 0;
    // log_time
    if (auto status = internal::ParseUint64(record.data + offset, record.dataSize - offset, &attachment->logTime);
        !status.ok())
    {
        return status;
    }
    offset += 8;
    // create_time
    if (auto status = internal::ParseUint64(record.data + offset, record.dataSize - offset, &attachment->createTime);
        !status.ok())
    {
        return status;
    }
    offset += 8;
    // name
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &attachment->name);
        !status.ok())
    {
        return status;
    }
    ...
    // media_type
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &attachment->mediaType);
        !status.ok())
    {
        return status;
    }
    ...
    // data_size
    if (auto status = internal::ParseUint64(record.data + offset, record.dataSize - offset, &attachment->dataSize);
        !status.ok())
    {
        return status;
    }
    ...
    // crc
    if (auto status = internal::ParseUint32(record.data + offset, record.dataSize - offset, &attachment->crc);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseAttachmentIndex(const Record &record, AttachmentIndex *attachmentIndex)
{
    ...
    // name
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &attachmentIndex->name);
        !status.ok())
    {
        return status;
    }
    ...
    // media_type
    if (auto status =
            internal::ParseString(record.data + offset, record.dataSize - offset, &attachmentIndex->mediaType);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseStatistics(const Record &record, Statistics *statistics)
{
    ...
    const uint32_t channelMessageCountsSize = internal::ParseUint32(record.data + 8 + 2 + 4 + 4 + 4 + 4 + 8 + 8);
    if (channelMessageCountsSize % 10 != 0 || channelMessageCountsSize > record.dataSize - PreambleSize)
    {
        const auto msg = internal::StrCat("invalid Statistics.channelMessageCounts length:", channelMessageCountsSize);
        return Status{StatusCode::InvalidRecord, msg};
    }
...
Status McapReader::ParseMetadata(const Record &record, Metadata *metadata)
{
    ...
    // name
    if (auto status = internal::ParseString(record.data, record.dataSize, &metadata->name); !status.ok())
    {
        return status;
    }
    uint64_t offset = 4 + metadata->name.size();
    // metadata
    if (auto status = internal::ParseKeyValueMap(record.data + offset, record.dataSize - offset, &metadata->metadata);
        !status.ok())
    {
        return status;
    }
...
Status McapReader::ParseMetadataIndex(const Record &record, MetadataIndex *metadataIndex)
{
    ...
    uint64_t offset = 8 + 8;
    if (auto status = internal::ParseString(record.data + offset, record.dataSize - offset, &metadataIndex->name);
        !status.ok())
    {
        return status;
    }
...
RecordReader::RecordReader(IReadable &dataSource, ByteOffset startOffset, ByteOffset endOffset)
    : offset(startOffset), endOffset(endOffset), dataSource_(&dataSource), status_(StatusCode::Success), curRecord_{}
{
}
...
TypedChunkReader::TypedChunkReader() : reader_{uncompressedReader_, 0, 0}, status_{StatusCode::Success} {}
...
bool TypedChunkReader::next()
{
    ...
    case OpCode::DataEnd:
    {
        ...
        const auto msg = internal::StrCat("record type ", uint8_t(record.opcode), " cannot appear in Chunk");
...
TypedRecordReader::TypedRecordReader(IReadable &dataSource, ByteOffset startOffset, ByteOffset endOffset)
    : reader_(dataSource, startOffset, std::min(endOffset, dataSource.size())), status_(StatusCode::Success),
      parsingChunk_(false)
{
    ...
    case OpCode::Chunk:
    {
        if (onMessage || onChunk || onSchema || onChannel)
        {
            ...
            if (onMessage || onSchema || onChannel)
            {
                ...
                {
                    const auto msg = internal::StrCat("unrecognized compression \"", chunk.compression, "\"");
...
LinearMessageView::LinearMessageView(McapReader &mcapReader, const ProblemCallback &onProblem)
    : mcapReader_(mcapReader), dataStart_(0), dataEnd_(0), onProblem_(onProblem)
{
}

LinearMessageView::LinearMessageView(McapReader &mcapReader,
                                     ByteOffset dataStart,
                                     ByteOffset dataEnd,
                                     Timestamp startTime,
                                     Timestamp endTime,
                                     const ProblemCallback &onProblem)
    : mcapReader_(mcapReader), dataStart_(dataStart), dataEnd_(dataEnd), readMessageOptions_(startTime, endTime),
...
LinearMessageView::LinearMessageView(McapReader &mcapReader,
                                     ...
                                     ByteOffset dataEnd,
                                     const ProblemCallback &onProblem)
    : mcapReader_(mcapReader), dataStart_(dataStart), dataEnd_(dataEnd), readMessageOptions_(options),
      onProblem_(onProblem)
{
}
...
LinearMessageView::Iterator::Iterator(LinearMessageView &view) : impl_(std::make_unique<Impl>(view))
{
    if (!impl_->has_value())
    {
        impl_ = nullptr;
...
LinearMessageView::Iterator::Impl::Impl(LinearMessageView &view) : view_(view)
{
    ...
    if (readMessageOptions.readOrder == ReadMessageOptions::ReadOrder::FileOrder)
    {
        ...
        recordReader_->onChannel = [this](const ChannelPtr channel, ByteOffset, std::optional<ByteOffset>)
        {
            view_.mcapReader_.channels_.insert_or_assign(channel->id, channel);
        };
        recordReader_->onMessage =
            [this](const Message &message, ByteOffset messageStartOffset, std::optional<ByteOffset> chunkStartOffset)
        {
            RecordOffset offset;
...
void LinearMessageView::Iterator::Impl::onMessage(const Message &message, RecordOffset offset)
{
    ...
    if (!maybeChannel)
    {
        view_.onProblem_(Status{StatusCode::InvalidChannelId,
...
IndexedMessageReader::IndexedMessageReader(McapReader &reader,
                                           const ReadMessageOptions &options,
                                           const std::function<void(const Message &, RecordOffset)> onMessage)
    : mcapReader_(reader), recordReader_(*mcapReader_.dataSource(), 0, 0), options_(options), onMessage_(onMessage),
      queue_(options_.readOrder == ReadMessageOptions::ReadOrder::ReverseLogTimeOrder)
{
    ...
    {
        status_ =
            Status(StatusCode::NoMessageIndexesAvailable, "cannot read MCAP in time order with no message indexes");
        return;
    }
...
void IndexedMessageReader::decompressChunk(const Chunk &chunk, IndexedMessageReader::ChunkSlot &slot)
{
    ...
    {
        status_ =
            Status(StatusCode::UnsupportedCompression, internal::StrCat("unhandled compression: ", chunk.compression));
    }
}
...
bool IndexedMessageReader::next()
{
    while (queue_.len() != 0)
    {
        ...
        if (std::holds_alternative<internal::DecompressChunkJob>(nextItem))
        {
            ...
            for (auto record = recordReader_.next(); record != std::nullopt; record = recordReader_.next())
            {
                switch (record->opcode)
                {
                case OpCode::Chunk:
