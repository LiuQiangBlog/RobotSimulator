                          ...
                          int clevel)
{
    ...
    int const selected_clevel =
        proposed_clevel < 0 ? -proposed_clevel : proposed_clevel; /* if level < 0, it becomes an acceleration value */
    compressFn compress = selected_clevel >= LZ4HC_CLEVEL_MIN ? LZ4_compress_HC : LZ4_compress_fast;
    int const cSize = compress((const char *)srcBuff, (char *)compressedBuff, (int)srcSize, (int)compressedBuffCapacity,
                               selected_clevel);
    CONTROL_MSG(cSize == 0, "Compression error !");

    {
        int const dSize =
            LZ4_decompress_safe((const char *)compressedBuff, (char *)resultBuff, cSize, (int)resultBuffCapacity);
    ...
    assert(resultBuffCapacity >= srcSize);
    {
        ...
        CONTROL_MSG(errorPos != srcSize, "Silent decoding corruption, at pos %u !!!", (unsigned)errorPos);
    }
}
...
static void roundTripCheck(const void *srcBuff, size_t srcSize, int clevel)
{
    ...
    roundTripTest(rBuff, cBuffSize, cBuff, cBuffSize, srcBuff, srcSize, clevel);
