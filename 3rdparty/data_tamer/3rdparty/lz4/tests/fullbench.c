...
extern "C"
{
    ...
    extern int LZ4_decompress_safe_forceExtDict(const char *in,
                                                char *out,
                                                int inSize,
                                                int outSize,
                                                const void *dict,
...
extern "C"
{
    ...
    extern int LZ4_decompress_safe_partial_forceExtDict(const char *in,
                                                        char *out,
                                                        int inSize,
    ...
    while (fileIdx < nbFiles)
    {
        ...
        /* Bench */
        {
            ...
            for (cAlgNb = 0; (cAlgNb <= (int)NB_COMPRESSION_ALGORITHMS) && (g_compressionTest); cAlgNb++)
            {
                ...
                if ((g_compressionAlgo != ALL_COMPRESSORS) && (g_compressionAlgo != cAlgNb))
                    continue;
                ...
                for (loopNb = 1; loopNb <= g_nbIterations; loopNb++)
                {
                    ...
                    while (BMK_GetClockSpan(clockTime) < loopDuration)
                    {
                        ...
                        for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
                        {
                            chunkP[chunkNb].compressedSize = compressionFunction(
                                chunkP[chunkNb].origBuffer, chunkP[chunkNb].compressedBuffer, chunkP[chunkNb].origSize);
                    ...
                    cSize = 0;
                    for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
                        cSize += (size_t)chunkP[chunkNb].compressedSize;
                ...
                if (ratio < 100.)
                    DISPLAY("%2i-%-34.34s :%10i ->%9i (%5.2f%%),%7.1f MB/s\n", cAlgNb, compressorName, (int)benchedSize,
                            (int)cSize, ratio, (double)benchedSize / bestTime / 1000000);
                else
                    DISPLAY("%2i-%-34.34s :%10i ->%9i (%5.1f%%),%7.1f MB/s\n", cAlgNb, compressorName, (int)benchedSize,
                            (int)cSize, ratio, (double)benchedSize / bestTime / 100000);
            }
            ...
            for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
            {
                chunkP[chunkNb].compressedSize =
                    LZ4_compress_default(chunkP[chunkNb].origBuffer, chunkP[chunkNb].compressedBuffer,
            ...
            for (dAlgNb = 0; (dAlgNb <= (int)NB_DECOMPRESSION_ALGORITHMS) && g_decompressionTest; dAlgNb++)
            {
                ...
                if ((g_decompressionAlgo != ALL_DECOMPRESSORS) && (g_decompressionAlgo != dAlgNb))
                    continue;
                ...
                if (decDescArray[dAlgNb].frameFormat)
                {
                    ...
                    size_t const fcsize =
                        LZ4F_compressFrame(compressed_buff, (size_t)compressedBuffSize, orig_buff, benchedSize, NULL);
                ...
                for (loopNb = 1; loopNb <= g_nbIterations; loopNb++)
                {
                    ...
                    while (BMK_GetClockSpan(clockTime) < loopDuration)
                    {
                        for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
                        {
                            int const decodedSize =
                                decompressionFunction(chunkP[chunkNb].compressedBuffer, chunkP[chunkNb].origBuffer,
                    ...
                    if (checkResult && (crcOriginal != crcDecoded))
                    {
                        DISPLAY("\n!!! WARNING !!! %14s : Invalid Checksum : %x != %x\n", inFileName,
                                (unsigned)crcOriginal, (unsigned)crcDecoded);
                        exit(1);
                    }
                }
