...
#define COMPRESSIBLE_NOISE_LENGTH (2 MB)
    ...
    DISPLAYLEVEL(3, "Decompression test : \n");
    {
        ...
        DISPLAYLEVEL(3, "Reusing decompression context \n");
        {
            ...
            if (decResult != 0)
                goto _output_error; /* should finish now */
            op += oSize;
            if (op > oend)
            {
        ...
        memcpy((char *)compressedBuffer + 16, (const char *)decodedBuffer + 32, 32); /* insert noise */
        {
            ...
            size_t const decompressError =
                LZ4F_decompress(dCtx, decodedBuffer, &dbSize, compressedBuffer, &cbSize, NULL);
            if (!LZ4F_isError(decompressError))
                goto _output_error;
    ...
    DISPLAYLEVEL(3, "Decompression test : \n");
    {
        ...
        while (ip < iend)
        {
            ...
            size_t oSize = (size_t)(oend - op);
            if (iSize > (size_t)(iend - ip))
                iSize = (size_t)(iend - ip);
            CHECK(LZ4F_decompress(dCtx, op, &oSize, ip, &iSize, NULL));
            op += oSize;
            ip += iSize;
        }
    ...
    /* dictID tests */
    {
        ...
        if (prefs.frameInfo.dictID != dictID)
            goto _output_error;
    ...
    /* Raw Dictionary compression test */
    {
        ...
        DISPLAYLEVEL(3, "LZ4F_decompress_usingDict: ");
        {
            ...
            CHECK(LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION));
            CHECK(LZ4F_decompress_usingDict(dctx, decodedBuffer, &decodedSize, compressedBuffer, &compressedSize,
                                            CNBuffer, dictSize, NULL));
            if (compressedSize != cSizeWithDict)
                goto _output_error;
            if (decodedSize != srcSize)
                goto _output_error;
            {
    ...
    /* Digested Dictionary (cdict) compression test */
    {
        ...
        CHECK_V(cSizeNoDict,
                LZ4F_compressFrame_usingCDict(cctx, compressedBuffer, dstCapacity, CNBuffer, srcSize, NULL, NULL));
        ...
        CHECK_V(cSizeWithDict,
                LZ4F_compressFrame_usingCDict(cctx, compressedBuffer, dstCapacity, CNBuffer, srcSize, cdict, NULL));
        ...
        DISPLAYLEVEL(3, "LZ4F_compressFrame_usingCDict, with dict, negative level : ");
        {
            ...
            CHECK_V(cSizeLevelMax, LZ4F_compressFrame_usingCDict(cctx, compressedBuffer, dstCapacity, CNBuffer,
                                                                 dictSize, cdict, &cParams));
        ...
        DISPLAYLEVEL(3, "LZ4F_compressFrame_usingCDict, multiple linked blocks : ");
        {
            ...
            CHECK_V(cSizeContiguous, LZ4F_compressFrame_usingCDict(cctx, compressedBuffer, outCapacity, CNBuffer,
            ...
            DISPLAYLEVEL(3, "LZ4F_decompress_usingDict on multiple linked blocks : ");
            {
                ...
                CHECK(LZ4F_decompress_usingDict(dctx, decodedBuffer, &decodedSize, compressedBuffer, &compressedSize,
                                                CNBuffer, dictSize, NULL));
                if (compressedSize != cSizeContiguous)
                    goto _output_error;
        ...
        DISPLAYLEVEL(3, "LZ4F_compressFrame_usingCDict, multiple independent blocks : ");
        {
            ...
            cParams.frameInfo.blockSizeID = LZ4F_max64KB;
            CHECK_V(cSizeIndep, LZ4F_compressFrame_usingCDict(cctx, compressedBuffer, outCapacity, CNBuffer, inSize,
                                                              cdict, &cParams));
                                        ...
                                        int findErrorPos)
{
    ...
    while (ip < iend)
    {
        ...
        if (o_scenario == o_overwrite)
            dOptions.stableDst = 0; /* overwrite mode */
        ...
        {
            if (op == oend)
                return LZ4F_ERROR_GENERIC; /* can theoretically happen with bogus data */
            op++;                          /* create a gap between consecutive output */
        }
        if (o_scenario == o_overwrite)
            op = (BYTE *)dst; /* overwrite destination */
    ...
    if (totalOut)
    { /* otherwise, it's a skippable frame */
...
size_t test_lz4f_decompression(const void *cSrc,
                               ...
                               int findErrorPos)
{
    o_scenario_e const o_scenario =
        (o_scenario_e)(FUZ_rand(randState) % 3); /* 0 : contiguous; 1 : non-contiguous; 2 : dst overwritten */
    /* tighten dst buffer conditions */
    size_t const dstCapacity = (o_scenario == o_noncontiguous) ? (decompressedSize * 2) + 128 : decompressedSize;
    size_t result;
    void *const dstBuffer = malloc(dstCapacity);
    assert(dstBuffer != NULL);

    result = test_lz4f_decompression_wBuffers(cSrc, cSize, dstBuffer, dstCapacity, o_scenario, srcRef, decompressedSize,
                                              crcOrig, randState, dCtx, seed, testNb, findErrorPos);
...
int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest, double compressibility, U32 duration_s)
{
    ...
    decodedBuffer =
        calloc(1, CNBufferLength); /* calloc avoids decodedBuffer being considered "garbage" by scan-build */
    ...
    for (; (testNb < nbTests) || (clockDuration > FUZ_GetClockSpan(startClock)); testNb++)
    {
        ...
        { /* single pass compression (simple) */
            ...
            while (ip < iend)
            {
                    ...
                    && (FUZ_rand(&randState) & 15) == 1)
                {
                    ...
                    size_t const flushedSize =
                        LZ4F_uncompressedUpdate(cCtx, op, (size_t)(oend - op), ip, uSize, &cOptions);
        ...
        test_lz4f_decompression(compressedBuffer, cSize, srcStart, srcSize, crcOrig, &randState, dCtx, seed, testNb,
                                0 /*don't search error Pos*/);
