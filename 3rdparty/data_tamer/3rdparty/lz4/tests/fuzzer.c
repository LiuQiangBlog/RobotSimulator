...
static void FUZ_fillCompressibleNoiseBuffer(void *buffer, size_t bufferSize, double proba, U32 *seed)
{
    ...
    while (pos < bufferSize)
    {
        ...
        else
        {
            ...
            size_t const d = MIN(pos + length, bufferSize);
            while (pos < d)
                BBuffer[pos++] = (BYTE)(FUZ_rand(seed) >> 5);
        }
    }
}
...
_endOfTests:
{
    ...
    if (!highAddress)
        DISPLAY("high address not possible \n");
    else
...
{
    void *const lowBuff = mmap((void *)(0x1000), size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
#define FUZ_DISPLAYTEST(...)                          \
    ...
    if (LZ4_initStream(&LZ4dictBody, sizeof(LZ4dictBody)) == NULL)
        abort();
    {
    ...
    for (cycleNb = startCycle; (cycleNb < nbCycles) || (FUZ_GetClockSpan(clockStart) < clockDuration); cycleNb++)
    {
        ...
        /* Compression tests */
        if (((FUZ_rand(&randState) & 63) == 2) && ((size_t)blockSize < labSize))
        {
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_HC_destSize()");
        {
            ...
            DISPLAYLEVEL(5, "LZ4_compress_HC_destSize(%i): destSize : %7i/%7i; content%7i/%7i ", compressionLevel,
                         cSize, targetSize, srcSize, blockSize);
            LZ4_freeHC(ctx);
        ...
        HCcompressedSize =
            LZ4_compress_HC(block, compressedBuffer, blockSize, (int)compressedBufferSize, compressionLevel);
        ...
        {
            int const r =
                LZ4_compress_fast_extState(stateLZ4, block, compressedBuffer, blockSize, (int)compressedBufferSize, 8);
            ...
            FUZ_DISPLAYTEST("test LZ4_compress_destSize_extState() with too small dest buffer (must succeed, compress "
                            "less than full input)");
            {
                ...
                FUZ_CHECKTEST(inputSize >= blockSize,
                              "LZ4_compress_destSize_extState() should consume less than full input");
            }
        }
...
#pragma warning(push)
#pragma warning( \
    disable : 6385) /* lz4\tests\fuzzer.c(497): warning C6385: Reading invalid data from 'compressedBuffer'. */
#endif
...
#pragma warning(pop)
            ...
            if (blockSize > 10)
            {
                decodedBuffer[blockSize - 10] = 0;
                {
                    ...
                    FUZ_CHECKTEST(
                        r >= 0, "LZ4_decompress_safe should have failed, due to Output Size being 10 bytes too short");
                }
            ...
            FUZ_DISPLAYTEST("decompress noisy source ");
            {
                ...
                {
                    int const decompressResult =
                        LZ4_decompress_safe(cBuffer_exact, decodedBuffer, compressedSize, blockSize);
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            FUZ_CHECKTEST(r >= 0,
                          "LZ4_decompress_safe should have failed, due to input size being one byte too short "
                          "(blockSize=%i, result=%i, compressedSize=%i)",
        ...
        FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial");
        {
            ...
            int const decResult =
                LZ4_decompress_safe_partial(compressedBuffer, decodedBuffer, inCSize, targetSize, blockSize);
        ...
        FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial_usingDict using no dict");
        {
            ...
            int const inCSize = (int)((size_t)compressedSize + extraneousInBytes);
            char const sentinel = decodedBuffer[targetSize] = block[targetSize] ^ 0x5A;
            int const decResult = LZ4_decompress_safe_partial_usingDict(compressedBuffer, decodedBuffer, inCSize,
                                                                        targetSize, blockSize, NULL, 0);
            FUZ_CHECKTEST(decResult < 0,
                          "LZ4_decompress_safe_partial_usingDict failed despite valid input data (error:%i)",
                          decResult);
            FUZ_CHECKTEST(
                decResult != targetSize,
                "LZ4_decompress_safe_partial_usingDict did not regenerated required amount of data (%i < %i <= %i)",
                decResult, targetSize, blockSize);
            FUZ_CHECKTEST(
                decodedBuffer[targetSize] != sentinel,
                "LZ4_decompress_safe_partial_usingDict overwrite beyond requested size (though %i <= %i <= %i)",
                decResult, targetSize, blockSize);
            FUZ_CHECKTEST(memcmp(block, decodedBuffer, (size_t)targetSize),
                          "LZ4_decompress_safe_partial_usingDict: corruption detected in regenerated data");
        }
        ...
        FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial_usingDict() using prefix as dict");
        {
            ...
            char const sentinel = decodedBuffer[targetSize] = block[targetSize] ^ 0x5A;
            int const decResult = LZ4_decompress_safe_partial_usingDict(compressedBuffer, decodedBuffer, inCSize,
                                                                        targetSize, blockSize, decodedBuffer, dictSize);
            FUZ_CHECKTEST(decResult < 0,
                          "LZ4_decompress_safe_partial_usingDict failed despite valid input data (error:%i)",
                          decResult);
            FUZ_CHECKTEST(
        ...
        FUZ_DISPLAYTEST("test LZ4_decompress_safe_partial_usingDict() using external dict");
        {
            ...
            char const sentinel = decodedBuffer[targetSize] = block[targetSize] ^ 0x5A;
            int const decResult = LZ4_decompress_safe_partial_usingDict(compressedBuffer, decodedBuffer, inCSize,
                                                                        targetSize, blockSize, dict, dictSize);
            FUZ_CHECKTEST(decResult < 0,
                          "LZ4_decompress_safe_partial_usingDict failed despite valid input data (error:%i)",
                          decResult);
            FUZ_CHECKTEST(
        ...
        FUZ_DISPLAYTEST("test LZ4_compress_default() with output buffer a bit too short");
        {
            ...
            if (missingBytes >= compressedSize)
                missingBytes = compressedSize - 1;
            missingBytes += !missingBytes; /* avoid special case missingBytes==0 */
            compressedBuffer[compressedSize - missingBytes] = 0;
            {
                int const cSize =
                    LZ4_compress_default(block, compressedBuffer, blockSize, compressedSize - missingBytes);
        ...
        {
            int missingBytes = (FUZ_rand(&randState) % 0x3F) + 1;
            if (missingBytes >= HCcompressedSize)
                missingBytes = HCcompressedSize - 1;
            missingBytes += !missingBytes; /* avoid special case missingBytes==0 */
        ...
        if (dict < (char *)CNBuffer)
            dict = (char *)CNBuffer;
        LZ4_loadDict(&LZ4dictBody, dict, dictSize);
        blockContinueCompressedSize =
            LZ4_compress_fast_continue(&LZ4dictBody, block, compressedBuffer, blockSize, (int)compressedBufferSize, 1);
        FUZ_CHECKTEST(blockContinueCompressedSize == 0, "LZ4_compress_fast_continue failed");
        ...
        FUZ_CHECKTEST(ret > 0,
                      "LZ4_compress_fast_continue using ExtDict should fail : one missing byte for output buffer : %i "
                      "written, %i buffer",
        ...
        DISPLAYLEVEL(5, " compress %i bytes from buffer(%p) into dst(%p) using dict(%p) of size %i \n", blockSize,
                     (const void *)block, (void *)decodedBuffer, (const void *)dict, dictSize);
        LZ4_loadDict(&LZ4dictBody, dict, dictSize);
        ...
        DISPLAYLEVEL(5, " decoding %i bytes from buffer(%p) using dict(%p) of size %i \n", blockSize,
                     (void *)decodedBuffer, (const void *)dict, dictSize);
        decodedBuffer[blockSize] = 0;
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            if (blockSize > missingBytes)
            {
                ...
                FUZ_CHECKTEST(ret >= 0,
                              ...
                              missingBytes);
                FUZ_CHECKTEST(
                    decodedBuffer[blockSize - missingBytes],
        ...
        /* Compress using external dictionary stream */
        {
            ...
            FUZ_CHECKTEST(blockContinueCompressedSize != expectedSize,
                          "LZ4_compress_fast_continue using extDictCtx produced different-sized output (%d expected vs "
                          "%d actual)",
            ...
            ret = LZ4_compress_fast_continue(&LZ4_stream, block, compressedBuffer, blockSize,
                                             blockContinueCompressedSize - 1, 1);
            FUZ_CHECKTEST(ret > 0,
                          "LZ4_compress_fast_continue using extDictCtx should fail : one missing byte for output "
                          "buffer : %i written, %i buffer",
                          ret, blockContinueCompressedSize);
            ...
            FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                          "LZ4_compress_limitedOutput_compressed size is different (%i != %i)", ret,
                          blockContinueCompressedSize);
            FUZ_CHECKTEST(ret <= 0, "LZ4_compress_fast_continue using extDictCtx with re-used context should work : "
                                    "enough size available within output buffer");
            FUZ_CHECKTEST(ret != expectedSize,
                          "LZ4_compress_fast_continue using extDictCtx produced different-sized output");
            FUZ_CHECKTEST(XXH32(compressedBuffer, (size_t)ret, 0) != expectedCrc,
                          "LZ4_compress_fast_continue using extDictCtx produced different output");
        ...
        blockContinueCompressedSize =
            LZ4_compress_HC_continue(LZ4dictHC, block, compressedBuffer, blockSize, (int)compressedBufferSize);
        ...
        FUZ_CHECKTEST(ret > 0,
                      "LZ4_compress_HC_continue using ExtDict should fail : one missing byte for output buffer "
                      "(expected %i, but result=%i)",
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            blockContinueCompressedSize =
                LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize, (int)compressedBufferSize);
            ...
            LZ4_attach_HC_dictionary(LZ4_streamHC, LZ4dictHC);
            ret =
                LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize, blockContinueCompressedSize);
            FUZ_CHECKTEST(ret != blockContinueCompressedSize,
            ...
            ret =
                LZ4_compress_HC_continue(LZ4_streamHC, block, compressedBuffer, blockSize, blockContinueCompressedSize);
            FUZ_CHECKTEST(ret != blockContinueCompressedSize,
                          "LZ4_compress_HC_continue using ExtDictCtx and fast reset size is different (%i != %i)", ret,
                          blockContinueCompressedSize);
            FUZ_CHECKTEST(ret <= 0, "LZ4_compress_HC_continue using ExtDictCtx and fast reset should work : enough "
                                    "size available within output buffer");
            FUZ_CHECKTEST(LZ4_streamHC->internal_donotuse.dirty, "Context should be clean");
        ...
        FUZ_DISPLAYTEST();
        {
            ...
            LZ4_setCompressionLevel(LZ4dictHC, compressionLevel);
            blockContinueCompressedSize =
                LZ4_compress_HC_continue_destSize(LZ4dictHC, block, compressedBuffer, &consumedSize, availableSpace);
...
static void FUZ_unitTests(int compressionLevel)
{
    ...
    DISPLAYLEVEL(3, "LZ4_initStream with multiple valid alignments : ");
    {
        ...
        DISPLAYLEVEL(4, "state1(%p) state2(%p) state3(%p) LZ4_stream_t size(0x%x): ", (void *)&(shc->state1),
                     (void *)&(shc->state2), (void *)&(shc->state3), (unsigned)sizeof(LZ4_stream_t));
    ...
    /* LZ4 streaming tests */
    {
        ...
        DISPLAYLEVEL(3, "saveDict (right after init) : ");
        {
            ...
            FUZ_CHECKTEST(LZ4_saveDict(ctx, NULL, 0) != 0, "LZ4_saveDict() can't save anything into (NULL,0)");
        ...
        /* ring buffer test */
        {
            ...
            while (iNext + messageSize < testCompressedSize)
            {
                ...
                if (rNext + messageSize > ringBufferSize)
                    rNext = 0;
    ...
    DISPLAYLEVEL(3, "LZ4_initStreamHC with multiple valid alignments : ");
    {
        ...
        assert(shc != NULL);
        memset(shc, 0, sizeof(*shc));
        DISPLAYLEVEL(4, "hc1(%p) hc2(%p) hc3(%p) size(0x%x): ", (void *)&(shc->hc1), (void *)&(shc->hc2),
                     (void *)&(shc->hc3), (unsigned)sizeof(LZ4_streamHC_t));
    ...
    /* LZ4 HC streaming tests */
    {
        ...
        DISPLAYLEVEL(3, "Simple HC round-trip : ");
        {
            ...
            result =
                LZ4_compress_HC_continue(&sHC, testInput, testCompressed, testCompressedSize, testCompressedSize - 1);
        ...
        DISPLAYLEVEL(3, "saveDictHC test #926 : ");
        {
            ...
            /* Check access violation with asan */
            FUZ_CHECKTEST(LZ4_saveDictHC(ctx, NULL, 0) != 0, "LZ4_saveDictHC() can't save anything into (NULL,0)");
        ...
        DISPLAYLEVEL(3, "Long sequence HC_destSize test : ");
        {
            ...
            result =
                LZ4_compress_HC_destSize(&sHC, (const char *)block, (char *)dstBlock, &srcSize, (int)targetSize, 3);
            DISPLAYLEVEL(4, "cSize=%i; readSize=%i; ", result, srcSize);
            FUZ_CHECKTEST(result != 4116,
            ...
            FUZ_CHECKTEST(srcSize < 1045000,
                          "LZ4_compress_HC_destSize() doesn't compress enough"
                          " (%i -> %i , expected > %i)",
        ...
        /* multiple HC compression test with dictionary */
        {
            ...
            FUZ_CHECKTEST(result1 == 0,
                          ...
                          result1);
            FUZ_CHECKTEST(sHC.internal_donotuse.dirty, "Context should be clean");
            result2 = LZ4_compress_HC_continue(&sHC, testInput + 2 * (size_t)segSize, testCompressed + result1, segSize,
                                               segSize - 1);
        ...
        /* remote dictionary HC compression test */
        {
            ...
            result =
                LZ4_decompress_safe_usingDict(testCompressed, testVerify, result, testCompressedSize, testInput, 32 KB);
            FUZ_CHECKTEST(
        ...
        /* multiple HC compression with ext. dictionary */
        {
            ...
            while (segStart + segSize < testInputSize)
            {
                ...
                XXH64_update(&crcNewState, dst, segSize);
                {
                    ...
                    if (crcOrig != crcNew)
                        FUZ_findDiff(dst, testInput + segStart);
...
#define BSIZE1 (dBufferSize - (maxMessageSize - 1))
            ...
            /* first block */
            messageSize = BSIZE1; /* note : we cheat a bit here, in theory no message should be > maxMessageSize. We
                                     just want to fill the decoding ring buffer once. */
            ...
            result =
                LZ4_decompress_fast_continue(&decodeStateFast, testCompressed, ringBufferFast + dNext, messageSize);
            ...
            while (totalMessageSize < 9 MB)
            {
                ...
                result = LZ4_decompress_safe_continue(
                    &decodeStateSafe, testCompressed, ringBufferSafe + dNext, compressedSize,
                    dBufferSize - dNext); /* works without knowing messageSize, under assumption that messageSize <=
                                             maxMessageSize */
                ...
                if (dNext + maxMessageSize > dBufferSize)
                    dNext = 0;
            }
        } /* Ring buffer test : Non synchronized decoder */
    }
