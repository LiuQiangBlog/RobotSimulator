    ...
    *offsetsEnd++ = sizeof(kTestMagic);
    ...
    for (;;)
    {
        ...
        LZ4_loadDict(lz4Stream, (const char *)dict, dictSize);
        {
            ...
            const int cmpBytes = LZ4_compress_fast_continue(lz4Stream, inpBuf, cmpBuf, inpBytes, sizeof(cmpBuf), 1);
            if (cmpBytes <= 0)
            {
                exit(1);
            }
...
void test_decompress(FILE *outFp, FILE *inpFp, void *dict, int dictSize, int offset, int length)
{
    ...
    for (; currentBlock < endBlock; ++currentBlock)
    {
        ...
        {
            const int decBytes = LZ4_decompress_safe_continue(lz4StreamDecode, cmpBuf, decBuf, cmpBytes, BLOCK_BYTES);
            if (decBytes <= 0)
            {
                exit(5);
            }
