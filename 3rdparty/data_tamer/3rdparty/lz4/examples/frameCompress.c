...
static const LZ4F_preferences_t kPrefs = {
    {LZ4F_max256KB, LZ4F_blockLinked, LZ4F_noContentChecksum, LZ4F_frame, 0 /* unknown content size */,
                                               ...
                                               long uncOffset)
{
    ...
    for (;;)
    {
        ...
        if (bytesToOffset <= 0 && uncOffset >= 0 && f_unc)
        {
            ...
            count_in += readSize;
            compressedSize = LZ4F_uncompressedUpdate(ctx, outBuff, outCapacity, inBuff, readSize, NULL);
        }
        else
        {
            ...
            count_in += readSize;
            compressedSize = LZ4F_compressUpdate(ctx, outBuff, outCapacity, inBuff, readSize, NULL);
        }
...
static compressResult_t compress_file(FILE *f_in, FILE *f_out, FILE *f_unc, int uncOffset)
{
    assert(f_in != NULL);
    assert(f_out != NULL);
    ...
    size_t const outbufCapacity =
        LZ4F_compressBound(IN_CHUNK_SIZE, &kPrefs); /* large enough for any input <= IN_CHUNK_SIZE */
    ...
    {
        result =
            compress_file_internal(f_in, f_out, ctx, src, IN_CHUNK_SIZE, outbuff, outbufCapacity, f_unc, uncOffset);
    }
    else
    {
...
static int decompress_file_allocDst(FILE *f_in, FILE *f_out, LZ4F_dctx *dctx, void *src, size_t srcCapacity)
{
    ...
    int const decompressionResult = decompress_file_internal(f_in, f_out, dctx, src, srcCapacity,
                                                             readSize - consumedSize, consumedSize, dst, dstCapacity);
...
int main(int argc, char **argv)
{
    ...
    /* compress */
    {
        ...
        compressResult_t const ret = compress_file(inpFp, outFp, uncFp, uncOffset);
        ...
        printf("%s: %zu → %zu bytes, %.1f%%\n", inpFilename, (size_t)ret.size_in,
    ...
    /* verify */
    {
        ...
        int const cmp = compareFiles(inpFp, decFp, uncFp, uncOffset);
