...
static void state_decompress(state_t *state, char const *src, int srcSize)
{
    ...
    int const dSize = LZ4_decompress_safe_continue(state->dstream, src, dst, srcSize, dstCapacity);
    FUZZ_ASSERT(dSize >= 0);
...
static void state_prefixRoundTrip(state_t *state)
{
    while (state->data.pos != state->data.size)
    {
        ...
        int const cSize = LZ4_compress_fast_continue(state->cstream, src, dst, srcSize, dstCapacity, 0);
        FUZZ_ASSERT(cSize > 0);
...
static void state_prefixHCRoundTrip(state_t *state)
{
    while (state->data.pos != state->data.size)
    {
        ...
        int const srcSize = FUZZ_rand32(&state->seed, 0, srcRemaining);
        int const dstCapacity = state->compressed.size - state->compressed.pos;
        int const cSize = LZ4_compress_HC_continue(state->cstreamHC, src, dst, srcSize, dstCapacity);
        FUZZ_ASSERT(cSize > 0);
        state->data.pos += srcSize;
        state->compressed.pos += cSize;
        state_decompress(state, dst, cSize);
...
static void state_loadDictHCRoundTrip(state_t *state)
{
    ...
    LZ4_setStreamDecode(state->dstream, dict, dictSize);
    state_randomRoundTrip(state, state_prefixHCRoundTrip, state_extDictHCRoundTrip);
}
...
round_trip_t roundTrips[] = {
    &state_prefixRoundTrip,   &state_extDictRoundTrip,   &state_loadDictRoundTrip,   &state_attachDictRoundTrip,
    &state_prefixHCRoundTrip, &state_extDictHCRoundTrip, &state_loadDictHCRoundTrip, &state_attachDictHCRoundTrip,
};
