...
#define DISPLAYUPDATE(l, ...)                                                   \
    if (g_displayLevel >= l)                                                    \
    {                                                                           \
        if ((TIME_clockSpan_ns(g_time) > refreshRate) || (g_displayLevel >= 4)) \
        {                                                                       \
            g_time = TIME_getTime();                                            \
            DISPLAY(__VA_ARGS__);                                               \
            if (g_displayLevel >= 4)                                            \
...
static void LZ4IO_finalTimeDisplay(TIME_t timeStart, clock_t cpuStart, unsigned long long size)
{
...
#endif
    {
        ...
        DISPLAYLEVEL(3, "Done in %.2f s ==> %.2f MiB/s  (cpu load : %.0f%%)\n", seconds,
                     (double)size / seconds / 1024. / 1024., (cpuLoad_s / seconds) * 100.);
    }
}
...
LZ4IO_prefs_t *LZ4IO_defaultPreferences(void)
{
    ...
    if (!prefs)
        END_PROCESS(11, "Can't even allocate LZ4IO preferences");
    prefs->passThrough = 0;
    prefs->overwrite = 1;
    prefs->testMode = 0;
...
static void LZ4IO_checkWriteOrder(void *arg)
{
    ...
    free(wjd); /* because wjd is pod */
    {
        ...
        DISPLAYUPDATE(2, "\rRead : %u MiB   ==> %.2f%%   ", (unsigned)(processedSize >> 20),
                      (double)wr->totalCSize / (double)processedSize * 100.);
    }
}

typedef size_t (*compress_f)(const void *parameters,
                             void *dst,
                             size_t dstCapacity,
                             const void *src,
                             size_t srcSize,
...
static void LZ4IO_compressChunk(void *arg)
{
        ...
        END_PROCESS(33, "Allocation error : can't allocate output buffer to compress new chunk");
    {
        ...
        size_t const cSize =
            cjd->compress(cjd->compressParameters, out_buff, outCapacity, inBuff, cjd->inSize, cjd->prefixSize);
...
static size_t LZ4IO_compressBlockLegacy_fast(const void *params,
                                             void *dst,
                                             size_t dstCapacity,
                                             const void *src,
                                             size_t srcSize,
                                             size_t prefixSize)
{
    ...
    int const cSize = LZ4_compress_fast((const char *)src, (char *)dst + LZ4IO_LEGACY_BLOCK_HEADER_SIZE, (int)srcSize,
                                        (int)dstCapacity, acceleration);
    if (cSize < 0)
...
static size_t LZ4IO_compressBlockLegacy_HC(const void *params,
                                           void *dst,
                                           size_t dstCapacity,
                                           const void *src,
                                           size_t srcSize,
                                           size_t prefixSize)
{
    ...
    int const cSize = LZ4_compress_HC((const char *)src, (char *)dst + LZ4IO_LEGACY_BLOCK_HEADER_SIZE, (int)srcSize,
                                      (int)dstCapacity, clevel);
    if (cSize < 0)
...
{
    int clResult = 0;
    compress_f const compressionFunction =
        (compressionlevel < 3) ? LZ4IO_compressBlockLegacy_fast : LZ4IO_compressBlockLegacy_HC;
...
_cfl_clean:
    ...
    if (finput)
        fclose(finput);
    if (foutput && !LZ4IO_isStdout(output_filename))
        fclose(foutput); /* do not close stdout */
                                           ...
                                           const LZ4IO_prefs_t *prefs)
{
    ...
    if (dstFileName == NULL)
        return ifntSize; /* not enough memory */
    ...
    for (i = 0; i < ifntSize; i++)
    {
        ...
        {
            missed_files +=
                LZ4IO_compressLegacy_internal(&processed, inFileNamesTable[i], stdoutmark, compressionLevel, prefs);
        ...
        missed_files +=
            LZ4IO_compressLegacy_internal(&processed, inFileNamesTable[i], dstFileName, compressionLevel, prefs);
        totalProcessed += processed;
    }
...
static void LZ4IO_freeCResources(cRess_t ress)
{
...

    {
        ...
        if (LZ4F_isError(errorCode))
            END_PROCESS(35, "Error : can't free LZ4F context resource : %s", LZ4F_getErrorName(errorCode));
    }
}
...
static void *LZ4IO_createDict(size_t *dictSize, const char *dictFilename)
{
    ...
    *dictSize = dictLen;
    ...
    else
    {
        ...
        if (!dictBuf)
            END_PROCESS(28, "Allocation error : not enough memory");
...
static LZ4F_CDict *LZ4IO_createCDict(const LZ4IO_prefs_t *io_prefs)
{
    ...
    LZ4F_CDict *cdict;
    if (!io_prefs->useDictionary)
        return NULL;
    dictionaryBuffer = LZ4IO_createDict(&dictionarySize, io_prefs->dictionaryFilename);
    if (!dictionaryBuffer)
        END_PROCESS(29, "Dictionary error : could not create dictionary");
                                       ...
                                       size_t prefixSize)
{
    ...
    if (prefixSize)
    {
        size_t const whr = LZ4F_compressBegin_usingDict(cctx, dst, dstCapacity, (const char *)src - prefixSize,
                                                        prefixSize, cfcp->prefs);
        if (LZ4F_isError(whr))
                                      ...
                                      const LZ4IO_prefs_t *const io_prefs)
{
    ...
    if (readSize < chunkSize)
    {
        ...
        size_t const cSize =
            LZ4F_compressFrame_usingCDict(ctx, dstBuffer, dstBufferSize, srcBuffer, readSize, ress->cdict, &prefs);
        if (LZ4F_isError(cSize))
            END_PROCESS(41, "Compression failed : %s", LZ4F_getErrorName(cSize));
        compressedfilesize = cSize;
        DISPLAYUPDATE(2, "\rRead : %u MiB   ==> %.2f%%   ", (unsigned)(filesize >> 20),
                      (double)compressedfilesize / (double)(filesize + !filesize) * 100); /* avoid division by zero */
    ...
    fclose(srcFile);
    if (!LZ4IO_isStdout(dstFileName))
        fclose(dstFile); /* do not close stdout */

    /* Copy owner, file permissions and modification time */
    {
        stat_t statbuf;
        if (!LZ4IO_isStdin(srcFileName) && !LZ4IO_isStdout(dstFileName) && !LZ4IO_isDevNull(dstFileName) &&
                                      ...
                                      const LZ4IO_prefs_t *const io_prefs)
{
    ...
    if (ferror(srcFile))
        END_PROCESS(40, "Error reading %s ", srcFileName);
    filesize += readSize;
    ...
    /* multiple-blocks file */
    {
        ...
        if (ferror(srcFile))
            END_PROCESS(47, "Error reading %s ", srcFileName);
...
static int LZ4IO_compressFilename_extRess(unsigned long long *inStreamSize,
                                          cRess_t *ress,
                                          const char *srcFileName,
                                          const char *dstFileName,
                                          int compressionLevel,
...
{
    if (LZ4IO_MULTITHREAD)
        return LZ4IO_compressFilename_extRess_MT(inStreamSize, ress, srcFileName, dstFileName, compressionLevel,
                                                 io_prefs);
                           ...
                           const LZ4IO_prefs_t *prefs)
{
    ...
    int const result =
        LZ4IO_compressFilename_extRess(&processed, &ress, srcFileName, dstFileName, compressionLevel, prefs);
                                    ...
                                    const LZ4IO_prefs_t *prefs)
{
    ...
    for (i = 0; i < ifntSize; i++)
    {
        ...
        if (LZ4IO_isStdout(suffix))
        {
            missed_files += LZ4IO_compressFilename_extRess(&processed, &ress, inFileNamesTable[i], stdoutmark,
        ...
        missed_files += LZ4IO_compressFilename_extRess(&processed, &ress, inFileNamesTable[i], dstFileName,
                                                       compressionLevel, prefs);
        totalProcessed += processed;
    }
...
static unsigned
LZ4IO_fwriteSparse(FILE *file, const void *buffer, size_t bufferSize, int sparseFileSupport, unsigned storedSkips)
{
    ...
    { /* normal write */
        ...
        if (sizeCheck != bufferSize)
            END_PROCESS(70, "Write error : cannot write decoded block");
        return 0;
    }
    ...
    if (storedSkips > 1 GB)
    {
        ...
        if (seekResult != 0)
            END_PROCESS(71, "1 GB skip error (sparse file support)");
        storedSkips -= 1 GB;
    }
    ...
    while (ptrT < bufferTEnd)
    {
        ...
        { /* not all 0s */
            errno = 0;
            {
                ...
                if (seekResult)
                    END_PROCESS(72, "Sparse skip error(%d): %s ; try --no-sparse", (int)errno, strerror(errno));
            }
            storedSkips = 0;
            seg0SizeT -= nb0T;
            ptrT += nb0T;
            {
                ...
                if (sizeCheck != seg0SizeT)
                    END_PROCESS(73, "Write error : cannot write decoded block");
            }
        }
        ptrT += seg0SizeT;
    }
    ...
    { /* size not multiple of sizeT : implies end of block */
        ...
        if (restPtr != restEnd)
        {
            int const seekResult = UTIL_fseek(file, storedSkips, SEEK_CUR);
            if (seekResult)
                END_PROCESS(74, "Sparse skip error ; try --no-sparse");
            storedSkips = 0;
            {
                size_t const sizeCheck = fwrite(restPtr, 1, (size_t)(restEnd - restPtr), file);
                if (sizeCheck != (size_t)(restEnd - restPtr))
                    END_PROCESS(75, "Write error : cannot write decoded end of block");
            }
        }
    }
...
static void LZ4IO_writeDecodedChunk(void *arg)
{
    ...
    *ctw->storedSkips =
        LZ4IO_fwriteSparse(ctw->f, ctw->buffer, ctw->size, ctw->sparseEnable, *ctw->storedSkips); /* success or die */
...
static void LZ4IO_decompressBlockLegacy(void *arg)
{
    ...
    decodedSize =
        LZ4_decompress_safe((const char *)lbi->inBuffer, (char *)lbi->outBuffer, (int)lbi->inSize, LEGACY_BLOCKSIZE);
    if (decodedSize < 0)
        END_PROCESS(64, "Decoding Failed ! Corrupted input detected !");
...
#define NB_BUFFSETS 4 /* 1 being read, 1 being processed, 1 being written, 1 being queued */
    ...
    if (ferror(finput))
        END_PROCESS(65, "Read error : ferror");
...
static unsigned long long LZ4IO_decodeLegacyStream(FILE *finput, FILE *foutput, const LZ4IO_prefs_t *prefs)
{
    ...
    if (!in_buff || !out_buff)
        END_PROCESS(61, "Allocation error : not enough memory");
...
static void LZ4IO_loadDDict(dRess_t *ress, const LZ4IO_prefs_t *const prefs)
{
    ...
    if (!ress->dictBuffer)
        END_PROCESS(25, "Dictionary error : could not create dictionary");
}
...
static dRess_t LZ4IO_createDResources(const LZ4IO_prefs_t *const prefs)
{
    ...
    if (LZ4F_isError(errorCode))
        END_PROCESS(60, "Can't create LZ4F context : %s", LZ4F_getErrorName(errorCode));
...
static void LZ4IO_freeDResources(dRess_t ress)
{
    ...
    if (LZ4F_isError(errorCode))
        END_PROCESS(69, "Error : can't free LZ4F context resource : %s", LZ4F_getErrorName(errorCode));
    free(ress.srcBuffer);
    free(ress.dstBuffer);
...
static void LZ4IO_writeDecodedLZ4FChunk(void *arg)
{
    ...
    /* note: works because only 1 thread */
    *ctw->storedSkips = LZ4IO_fwriteSparse(ctw->fOut, ctw->bufOut.ptr, ctw->bufOut.size, ctw->sparseEnable,
                                           *ctw->storedSkips); /* success or die */
...
static void LZ4IO_decompressLZ4FChunk(void *arg)
{
    ...
    { /* still to read */
        ...
        b.size = b.capacity;
        {
            size_t nextToLoad = LZ4F_decompress_usingDict(lz4fc->dctx, b.ptr, &b.size, inPtr + pos, &remainingInSize,
                                                          lz4fc->dictBuffer, lz4fc->dictBufferSize, NULL);
...
LZ4IO_decompressLZ4F(dRess_t ress, FILE *const srcFile, FILE *const dstFile, const LZ4IO_prefs_t *const prefs)
{
    unsigned long long filesize = 0;
    ...
    const LZ4F_decompressOptions_t *const dOptPtr =
        ((prefs->blockChecksum == 0) && (prefs->streamChecksum == 0)) ? &dOpt_skipCrc : NULL;
    TPool *const tPool = TPool_create(1, 1);
    TPool *const wPool = TPool_create(1, 1);
...
#define NB_BUFFSETS 4 /* 1 being read, 1 being processed, 1 being written, 1 being queued */
    ...
    /* Init feed with magic number (already consumed from FILE* sFile) */
    {
        ...
        nextToLoad =
            LZ4F_decompress_usingDict(ress.dCtx, ress.dstBuffer, &outSize, ress.srcBuffer, &inSize, ress.dictBuffer,
                                      ress.dictBufferSize, dOptPtr); /* set it once, it's enough */
        if (LZ4F_isError(nextToLoad))
    ...
    for (bSetNb = 0;; bSetNb = (bSetNb + 1) % NB_BUFFSETS)
    {
        ...
        if (readSize < INBUFF_SIZE)
            break; /* likely reached end of stream */
    }
    assert(feof(srcFile));
...
LZ4IO_decompressLZ4F(dRess_t ress, FILE *const srcFile, FILE *const dstFile, const LZ4IO_prefs_t *const prefs)
{
    ...
    for (; nextToLoad;)
    {
        ...
        { /* still to read, or still to flush */
            ...
            nextToLoad =
                LZ4F_decompress_usingDict(ress.dCtx, ress.dstBuffer, &decodedBytes, (char *)(ress.srcBuffer) + pos,
                                          &remaining, ress.dictBuffer, ress.dictBufferSize, NULL);
            if (LZ4F_isError(nextToLoad))
...
static unsigned long long
LZ4IO_passThrough(FILE *finput, FILE *foutput, unsigned char MNstore[MAGICNUMBER_SIZE], int sparseFileSupport)
{
...
static int LZ4IO_decompressSrcFile(unsigned long long *outGenSize,
                                   dRess_t ress,
                                   const char *input_filename,
                                   const char *output_filename,
                                   const LZ4IO_prefs_t *const prefs)
{
    ...
    { /* endless loop, see break condition */
        unsigned long long const decodedSize = selectDecoder(ress, finput, foutput, prefs);
...
static int LZ4IO_decompressDstFile(unsigned long long *outGenSize,
                                   dRess_t ress,
                                   const char *input_filename,
                                   const char *output_filename,
                                   const LZ4IO_prefs_t *const prefs)
{
    ...
    if (!LZ4IO_isStdin(input_filename) && UTIL_getFileStat(input_filename, &statbuf))
        stat_result = 1;
    ...
    /* Copy owner, file permissions and modification time */
    if (stat_result != 0 && !LZ4IO_isStdout(output_filename) && !LZ4IO_isDevNull(output_filename))
    {
                                      ...
                                      const LZ4IO_prefs_t *prefs)
{
    ...
    if (outFileName == NULL)
        END_PROCESS(70, "Memory allocation error");
...
#define LZ4IO_INIT_CFILEINFO                         \
    {                                                \
        NULL, 0ULL, 0, LZ4IO_INIT_FRAMEINFO, 1, 1, 1 \
    }
                                               ...
                                               const LZ4F_contentChecksum_t contentChecksumFlag)
{
    ...
    for (;;)
    {
        ...
        totalBlocksSize += LZ4F_BLOCK_HEADER_SIZE;
        {
            ...
            if (UTIL_fseek(finput, (long)nextBlock, SEEK_CUR) != 0)
                return 0;
        }
    }
    return totalBlocksSize;
...
static unsigned long long LZ4IO_skipLegacyBlocksData(FILE *finput)
{
    ...
    for (;;)
    {
        ...
        }
        {
            ...
            if (nextCBlockSize == LEGACY_MAGICNUMBER || nextCBlockSize == LZ4IO_MAGICNUMBER ||
...
LZ4IO_getCompressedFileInfo(LZ4IO_cFileInfo_t *cfinfo, const char *input_filename, int displayNow)
{
    ...
    while (!feof(finput))
    {
        ...
        {
        case LZ4IO_MAGICNUMBER:
            if (cfinfo->frameSummary.frameType != lz4Frame)
                cfinfo->eqFrameTypes = 0;
            /* Get frame info */
            {
                const size_t readBytes =
                    fread(buffer + MAGICNUMBER_SIZE, 1, LZ4F_HEADER_SIZE_MIN - MAGICNUMBER_SIZE, finput);
                if (!readBytes || ferror(finput))
                    END_PROCESS(71, "Error reading %s", input_filename);
            }
            {
                ...
                if (hSize > (LZ4F_HEADER_SIZE_MIN + MAGICNUMBER_SIZE))
                {
                    ...
                    const size_t readBytes =
                        fread(buffer + LZ4F_HEADER_SIZE_MIN, 1, hSize - LZ4F_HEADER_SIZE_MIN, finput);
                    if (!readBytes || ferror(finput))
                        END_PROCESS(72, "Error reading %s", input_filename);
                }
                /* Create decompression context */
                {
                    ...
                    if (LZ4F_isError(LZ4F_createDecompressionContext(&dctx, LZ4F_VERSION)))
                        break;
                    {
                        unsigned const frameInfoError =
                            LZ4F_isError(LZ4F_getFrameInfo(dctx, &frameInfo.lz4FrameInfo, buffer, &hSize));
                        ...
                        {
                            const unsigned long long totalBlocksSize =
                                LZ4IO_skipBlocksData(finput, frameInfo.lz4FrameInfo.blockChecksumFlag,
                                                     frameInfo.lz4FrameInfo.contentChecksumFlag);
                            if (totalBlocksSize)
                            {
                                char bTypeBuffer[5];
                                LZ4IO_blockTypeID(frameInfo.lz4FrameInfo.blockSizeID, frameInfo.lz4FrameInfo.blockMode,
                                                  bTypeBuffer);
                                if (displayNow)
                                    DISPLAYOUT("    %6llu %14s %5s %8s", cfinfo->frameCount + 1,
                                               LZ4IO_frameTypeNames[frameInfo.frameType], bTypeBuffer,
                                ...
                                if (frameInfo.lz4FrameInfo.contentSize)
                                {
                                    double const ratio = (double)(totalBlocksSize + hSize) /
                                                         (double)frameInfo.lz4FrameInfo.contentSize * 100;
                                    if (displayNow)
                                        DISPLAYOUT(" %20llu %20llu %9.2f%%\n", totalBlocksSize + hSize,
                                                   frameInfo.lz4FrameInfo.contentSize, ratio);
        ...
        case LEGACY_MAGICNUMBER:
            ...
            if (cfinfo->frameSummary.frameType != legacyFrame && cfinfo->frameCount != 0)
                cfinfo->eqFrameTypes = 0;
            cfinfo->eqBlockTypes = 0;
            cfinfo->allContentSize = 0;
            {
                ...
                {
                    if (displayNow)
                        DISPLAYOUT("    %6llu %14s %5s %8s %20llu %20s %9s\n", cfinfo->frameCount + 1,
                                   LZ4IO_frameTypeNames[frameInfo.frameType], "-", "-", totalBlocksSize + 4, "-", "-");
                    result = LZ4IO_LZ4F_OK;
                }
            }
            break;
        case LZ4IO_SKIPPABLE0:
            ...
            }
            {
                ...
                if (errorNb != 0)
                    END_PROCESS(43, "Stream error : cannot skip skippable area");
                if (displayNow)
                    DISPLAYOUT("    %6llu %14s %5s %8s %20u %20s %9s\n", cfinfo->frameCount + 1, "SkippableFrame", "-",
                               "-", size + 8, "-", "-");
...
int LZ4IO_displayCompressedFilesInfo(const char **inFileNames, size_t ifnIdx)
{
    ...
    if (g_displayLevel < 3)
    {
        DISPLAYOUT("%10s %14s %5s %11s %13s %8s   %s\n", "Frames", "Type", "Block", "Compressed", "Uncompressed",
                   "Ratio", "Filename");
    }
    for (; idx < ifnIdx; idx++)
    {
        ...
        if (g_displayLevel >= 3)
        {
            ...
            DISPLAYOUT("    %6s %14s %5s %8s %20s %20s %9s\n", "Frame", "Type", "Block", "Checksum", "Compressed",
                       "Uncompressed", "Ratio");
        }
        {
            LZ4IO_infoResult const op_result =
                LZ4IO_getCompressedFileInfo(&cfinfo, inFileNames[idx], g_displayLevel >= 3);
        ...
        if (g_displayLevel < 3)
        {
            /* Display summary */
            char buffers[3][10];
            DISPLAYOUT("%10llu %14s %5s %11s %13s ", cfinfo.frameCount,
                       ...
                       cfinfo.allContentSize
                           ? LZ4IO_toHuman((long double)cfinfo.frameSummary.lz4FrameInfo.contentSize, buffers[2])
                           : "-");
            if (cfinfo.allContentSize)
            {
                double const ratio =
                    (double)cfinfo.fileSize / (double)cfinfo.frameSummary.lz4FrameInfo.contentSize * 100;
