...
static U32 g_displayLevel = 2; /* 0 : no display;   1: errors;   2 : + result + interaction + warnings;   3 : +
...
struct compressionParameters
{
    ...
    int (*blockFunction)(const struct compressionParameters *pThis,
                         const char *src,
                         char *dst,
                         int srcSize,
                         int dstSize);
...
static void LZ4_compressResetNoStream(const struct compressionParameters *cparams)
{
    (void)cparams;
}

static void LZ4_compressResetStream(const struct compressionParameters *cparams)
{
    LZ4_resetStream_fast(cparams->LZ4_stream);
    LZ4_attach_dictionary(cparams->LZ4_stream, cparams->LZ4_dictStream);
}

static void LZ4_compressResetStreamHC(const struct compressionParameters *cparams)
{
...
static int LZ4_compressBlockNoStream(const struct compressionParameters *cparams,
                                     const char *src,
                                     char *dst,
                                     int srcSize,
                                     int dstSize)
...
static int LZ4_compressBlockNoStreamHC(const struct compressionParameters *cparams,
                                       const char *src,
                                       char *dst,
                                       int srcSize,
                                       int dstSize)
...
static int LZ4_compressBlockStream(const struct compressionParameters *cparams,
                                   const char *src,
                                   char *dst,
                                   int srcSize,
                                   int dstSize)
...
static int LZ4_compressBlockStreamHC(const struct compressionParameters *cparams,
                                     const char *src,
                                     char *dst,
                                     int srcSize,
                                     int dstSize)
...
static void LZ4_compressCleanupNoStream(const struct compressionParameters *cparams)
{
    (void)cparams;
}

static void LZ4_compressCleanupStream(const struct compressionParameters *cparams)
{
    LZ4_freeStream(cparams->LZ4_stream);
    LZ4_freeStream(cparams->LZ4_dictStream);
}

static void LZ4_compressCleanupStreamHC(const struct compressionParameters *cparams)
{
...
static void
LZ4_buildCompressionParameters(struct compressionParameters *pParams, int cLevel, const char *dictBuf, int dictSize)
{
...
typedef int (
    *DecFunction_f)(const char *src, char *dst, int srcSize, int dstCapacity, const char *dictStart, int dictSize);

static LZ4F_dctx *g_dctx = NULL;

static int
LZ4F_decompress_binding(const char *src, char *dst, int srcSize, int dstCapacity, const char *dictStart, int dictSize)
{
    ...
    decStatus = LZ4F_decompress(g_dctx, dst, &dstSize, src, &readSize, &dOpt);
    ...
    return -1;
    (void)dictStart;
    (void)dictSize; /* not compatible with dictionary yet */
}
...
static int BMK_benchMem(const void *srcBuffer,
                        size_t srcSize,
                        ...
                        int dictSize)
{
    size_t const blockSize =
        (g_blockSize >= 32 && !g_decodeOnly ? g_blockSize : srcSize) + (!srcSize) /* avoid div by 0 */;
    U32 const maxNbBlocks = (U32)((srcSize + (blockSize - 1)) / blockSize) + nbFiles;
    blockParam_t *const blockTable = (blockParam_t *)malloc(maxNbBlocks * sizeof(blockParam_t));
    size_t const maxCompressedSize =
        (size_t)LZ4_compressBound((int)srcSize) + (maxNbBlocks * 1024); /* add some room for safety */
...
#define NB_MARKS 4
        ...
        while (!cCompleted || !dCompleted)
        {
            ...
            { /* still some time to do compression tests */
                ...
                for (nbLoops = 0; nbLoops < nbCompressionLoops; nbLoops++)
                {
                    ...
                    for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                    {
                        size_t const rSize = (size_t)compP.blockFunction(
                            &compP, blockTable[blockNb].srcPtr, blockTable[blockNb].cPtr,
                ...
                OUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (%5.3f),%6.1f MB/s\r", marks[markNb], displayName,
                         (U32)totalRSize, (U32)cSize, ratio, ((double)totalRSize / (double)fastestC) * 1000);
                fflush(NULL);
            }
            (void)fastestD;
            ...
            if (!dCompleted)
            {
                const DecFunction_f decFunction =
                    g_decodeOnly ? LZ4F_decompress_binding : LZ4_decompress_safe_usingDict;
                const char *const decString = g_decodeOnly ? "LZ4F_decompress" : "LZ4_decompress_safe_usingDict";
                TIME_t const timeStart = TIME_getTime();
                U32 nbLoops;
                ...
                for (nbLoops = 0; nbLoops < nbDecodeLoops; nbLoops++)
                {
                    ...
                    for (blockNb = 0; blockNb < nbBlocks; blockNb++)
                    {
                        ...
                        size_t const resCapa = (blockTable[blockNb].srcSize < inMaxSize)
                                                   ...
                                                   : INT_MAX;
                        int const regenSize =
                            decFunction(blockTable[blockNb].cPtr, blockTable[blockNb].resPtr,
                                        (int)blockTable[blockNb].cSize, (int)resCapa, dictBuf, dictSize);
                        if (regenSize < 0)
                        {
                            DISPLAY("%s() failed on block %u of size %u \n", decString, blockNb,
                                    (unsigned)blockTable[blockNb].srcSize);
                            if (g_decodeOnly)
            ...
            if (g_decodeOnly)
            {
                unsigned u;
                totalRSize = 0;
                for (u = 0; u < nbBlocks; u++)
                    totalRSize += blockTable[u].resSize;
            }
            markNb = (markNb + 1) % NB_MARKS;
            ratio = (double)totalRSize / (double)cSize;
            OUTLEVEL(2, "%2s-%-17.17s :%10u ->%10u (%5.3f),%6.1f MB/s, %6.1f MB/s\r", marks[markNb], displayName,
                     (U32)totalRSize, (U32)cSize, ratio, ((double)totalRSize / (double)fastestC) * 1000,
        ...
        if (g_displayLevel == 1)
        {
            ...
            DISPLAYOUT("-%-3i%11i (%5.3f) %6.2f MB/s %6.1f MB/s  %s ", cLevel, (int)cSize, ratio, cSpeed, dSpeed,
                       displayName);
...
static int BMK_benchCLevel(void *srcBuffer,
                           size_t benchedSize,
                           const char *displayName,
                           int cLevel,
                           int cLevelLast,
                           ...
                           int dictSize)
{
    ...
    {
        benchError |= BMK_benchMem(srcBuffer, benchedSize, displayName, l, fileSizes, nbFiles, dictBuf, dictSize);
    }
    return benchError;
}
...
static void
BMK_loadFiles(void *buffer, size_t bufferSize, size_t *fileSizes, const char **fileNamesTable, unsigned nbFiles)
{
    ...
    for (n = 0; n < nbFiles; n++)
    {
        ...
        if (f == NULL)
            END_PROCESS(10, "impossible to open file %s", fileNamesTable[n]);
        ...
        }
        {
            ...
            if (readSize != (size_t)fileSize)
                END_PROCESS(11, "could not read %s", fileNamesTable[n]);
            pos += readSize;
        }
    ...
    if (totalSize == 0)
        END_PROCESS(12, "no data to bench");
}

static int BMK_benchFileTable(const char **fileNamesTable,
                              unsigned nbFiles,
                              int cLevel,
                              int cLevelLast,
                              const char *dictBuf,
                              int dictSize)
{
    ...
    if (!fileSizes)
        END_PROCESS(12, "not enough memory for fileSizes");

    /* Memory allocation & restrictions */
    benchedSize = BMK_findMaxMem(totalSizeToLoad * 3) / 3;
    if (benchedSize == 0)
        END_PROCESS(12, "not enough memory");
    ...
    snprintf(mfName, sizeof(mfName), " %u files", nbFiles);
    {
        ...
        benchError = BMK_benchCLevel(srcBuffer, benchedSize, displayName, cLevel, cLevelLast, fileSizes, nbFiles,
                                     dictBuf, dictSize);
    }
...
static int BMK_syntheticTest(int cLevel, int cLevelLast, const char *dictBuf, int dictSize)
{
    int benchError = 0;
    ...
    /* Memory allocation */
    if (!srcBuffer)
        END_PROCESS(21, "not enough memory");
    ...
    /* Bench */
    benchError =
        BMK_benchCLevel(srcBuffer, benchedSize, "Lorem ipsum", cLevel, cLevelLast, &benchedSize, 1, dictBuf, dictSize);
...
static int BMK_benchFilesSeparately(const char **fileNamesTable,
                                    unsigned nbFiles,
                                    int cLevel,
                                    int cLevelLast,
                                    const char *dictBuf,
                                    int dictSize)
{
    int benchError = 0;
    unsigned fileNb;
    if (cLevel > LZ4HC_CLEVEL_MAX)
        cLevel = LZ4HC_CLEVEL_MAX;
    if (cLevelLast > LZ4HC_CLEVEL_MAX)
...
int BMK_benchFiles(const char **fileNamesTable, unsigned nbFiles, int cLevel, int cLevelLast, const char *dictFileName)
{
    ...
    if (dictFileName)
    {
        ...
        if (!dictBuf)
            END_PROCESS(25, "Allocation error : not enough memory");
