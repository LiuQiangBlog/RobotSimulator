...
static unsigned displayLevel =
    2; /* 0 : no display ; 1: errors only ; 2 : downgradable normal ; 3 : non-downgradable normal; 4 : + information */
...
static operationMode_e determineOpMode(const char *inputFilename)
{
    ...
    if (!strcmp(inputFilename + extStart, LZ4_EXTENSION))
...
{
    int argNb, cLevel = init_cLevel(), cLevelLast = -10000, legacy_format = 0, forceStdout = 0, forceOverwrite = 0,
               main_pause = 0, multiple_inputs = 0, all_arguments_are_files = 0, operationResult = 0;
    ...
    if (exeNameMatch(exeName, UNLZ4))
    {
        mode = om_decompress;
    }
    if (exeNameMatch(exeName, LZ4_LEGACY))
    {
        g_lz4c_legacy_commands = 1;
    }
    ...
    for (argNb = 1; argNb < argCount; argNb++)
    {
        ...
        if (!all_arguments_are_files && argument[0] == '-')
        {
            ...
            if (argument[1] == '-')
            {
                ...
                if ((!strcmp(argument, "--decompress")) || (!strcmp(argument, "--uncompress")))
                {
                    if (mode != om_bench)
                        mode = om_decompress;
                ...
                if ((!strcmp(argument, "--stdout")) || (!strcmp(argument, "--to-stdout")))
                {
                    forceStdout = 1;
                    output_filename = stdoutmark;
                    continue;
                ...
                if (!strcmp(argument, "--no-crc"))
                {
                    LZ4IO_setStreamChecksumMode(prefs, 0);
                    LZ4IO_setBlockChecksumMode(prefs, 0);
                    BMK_skipChecksums(1);
                    continue;
                }
        ...
        if (!output_filename)
        {
            ...
            if (!strcmp(output_filename, nullOutput))
        ...
        DISPLAYLEVEL(1, "%s : %s won't be used ! Do you want multiple input files (-m) ? \n",
                     forceOverwrite ? "Warning" : "Error", argument);
        if (!forceOverwrite)
            exit(1);
    }
    ...
    if (!strcmp(output_filename, stdoutmark) && (displayLevel == 2))
        displayLevel = 1;
    if ((multiple_inputs) && (displayLevel == 2))
        displayLevel = 1;
    ...
    if (mode == om_decompress)
    {
        if (multiple_inputs)
        {
            ...
            if (!strcmp(output_filename, stdoutmark))
                dec_extension = stdoutmark;
            if (!strcmp(output_filename, nulmark))
                dec_extension = nulmark;
    ...
    { /* compression is default action */
        ...
        else
        {
            if (multiple_inputs)
            {
                ...
                assert(ifnIdx <= INT_MAX);
                operationResult =
                    LZ4IO_compressMultipleFilenames(inFileNames, (int)ifnIdx, comp_extension, cLevel, prefs);
            }
            else
            {
