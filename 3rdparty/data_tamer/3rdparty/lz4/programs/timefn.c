...
TIME_t TIME_getTime(void)
{
    ...
    {
        TIME_t r;
        r.t = mach_absolute_time() * (Duration_ns)rate.numer / (Duration_ns)rate.denom;
        return r;
    }
}
...
TIME_t TIME_getTime(void)
{
    ...
    {
        TIME_t r;
        r.t = (Duration_ns)time.tv_sec * 1000000000ULL + (Duration_ns)time.tv_nsec;
        return r;
    }
}
...
#elif (defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 201112L) /* C11 */) && defined(TIME_UTC) && \
    !defined(__ANDROID__)
