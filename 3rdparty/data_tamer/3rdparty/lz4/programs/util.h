...
#define UTIL_sleep(s) sleep(s)
    ...
    (PLATFORM_POSIX_VERSION >= 200112L) /* nanosleep requires POSIX.1-2001 */
...
typedef struct stat stat_t;
    ...
    UTIL_STATIC int UTIL_isRegFD(int fd)
    {
        stat_t statbuf;
#ifdef _WIN32
        /* Windows runtime library always open file descriptors 0, 1 and 2 in text mode, therefore we can't use them for
         * binary I/O */
        if (fd < 3)
            return 0;
#endif
    ...
    UTIL_STATIC U64 UTIL_getOpenFileSize(FILE *file)
    {
        ...
        if (r || !UTIL_STAT_MODE_ISREG(statbuf.st_mode))
            return 0; /* No good... */
        ...
        do
        {
            ...
            if (cFile.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
            {
                ...
                nbFiles += UTIL_prepareFileList(
                    path, bufStart, pos, bufEnd); /* Recursively call "UTIL_prepareFileList" with the new path. */
                if (*bufStart == NULL)
                {
                    free(path);
                    FindClose(hFile);
...
UTIL_STATIC int UTIL_prepareFileList(const char *dirName, char **bufStart, size_t *pos, char **bufEnd)
{
    ...
    /*
     * UTIL_createFileList - takes a list of files and directories (params: inputNames, inputNamesNb), scans
     * directories, and returns a new list of files (params: return value, allocatedBuffer, allocatedNamesNb). After
     * finishing usage of the list the structures should be freed with UTIL_freeFileList(params: return value,
