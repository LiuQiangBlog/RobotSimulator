...
{
    std::string filepath =
        "/home/liuqiang/CLionProjects/RobotControlAlgorithms/3rdparty/data_tamer/examples/test_sample.mcap";
    ...
    {
        std::string schema_text(reinterpret_cast<const char *>(mcap_schema->data.data()), mcap_schema->data.size());
    ...
    auto IncrementCounter = [&](const std::string &series_name, MessageCount &message_counts)
    {
    ...
    for (const auto &msg : reader.readMessages())
    {
        ...
        // one after the other.
        DataTamerParser::BufferSpan msg_buffer = {reinterpret_cast<const uint8_t *>(msg.message.data),
