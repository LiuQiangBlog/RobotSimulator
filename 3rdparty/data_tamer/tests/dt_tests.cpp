...
TEST(DataTamerBasic, Disable)
{
    ...
    size_t expected_size = sizeof(v1) + sizeof(v2) + sizeof(v3) + sizeof(v4) + sizeof(v5) + 3 * sizeof(double) +
...
TEST(DataTamerBasic, VectorWithChangingSize)
{
    ...
    channel->takeSnapshot();
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    ASSERT_EQ(sink->latest_snapshot.payload.size(), vect.size() * sizeof(float) + sizeof(uint32_t));
