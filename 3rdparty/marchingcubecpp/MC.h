...
*/
static void mc_internalComputeEdge(mcVec3i *slab_inds,
                                   mcMesh &mesh,
                                   MC_FLOAT va,
                                   MC_FLOAT vb,
                                   int axis,
    ...
    for (muint z = 0; z < nz - 1; z++)
    {
        for (muint y = 0; y < ny - 1; y++)
        {
            for (muint x = 0; x < nx - 1; x++)
            {
                ...
                const int config_n = ((vs[0] < 0) << 0) | ((vs[1] < 0) << 1) | ((vs[2] < 0) << 2) | ((vs[3] < 0) << 3) |
                ...
                {
                    mc_internalAccumulateNormal(outputMesh, outputMesh.indices[indexBase + i * 3 + 0],
