...
void TestPerformances()
{
    ...
    std::cout << "Average time = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() / count
              << "[ms]" << std::endl;
}
...
void TestSimpleMC()
{
    ...
    {
        out << "f " << mesh.indices.at(i) + 1 << "//" << mesh.indices.at(i) + 1 << " " << mesh.indices.at(i + 1) + 1
