...
public:
    ...
    std::unordered_set<std::string> all_channels, mcap_all_channels;
    std::unordered_map<int, std::deque<std::string>> tab_selected_channels, mcap_tab_selected_channels;
        ...
        if (ImGui::Begin("##WelcomeWindow", nullptr, flags))
        {
            ...
            if (ImGui::Button("ViewOfflineData", ImVec2(buttonWidth, buttonHeight)))
            {
                ...
                auto f = pfd::open_file("Choose files to read", pfd::path::home(),
                                        {"Mcap Files (.mcap .MCAP)", "*.mcap *.MCAP", "All Files", "*"},
                                        pfd::opt::multiselect);
                auto res = f.result();
                if (res.empty())
                {
                    CLOG_ERROR << "No any file is selected.";
                }
                else
                {
        ...
        if (ImGui::Begin("##ViewerOfMcapData", &openViewerOfMcapData, flags))
        {
            ...
            if (ImGui::BeginTabBar("TabBarOfViewerOfMcapData", tab_bar_flags))
            {
                ...
                for (int n = 0; n < active_tabs.Size;)
                {
                    ...
                    {
                        h.mcap_tab_selected_channels[tab_id] = {};
                    }
                    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(10, 12));
                    ImGui::PushStyleVar(ImGuiStyleVar_TabBorderSize, 1.0f);     // 增大边框厚度
                    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(4, 4));
                    ...
                    if (ImGui::BeginTabItem(tab_title, &tab_open))
                    {
                        ...
                        if (tab_open)
                        {
                            ...
                            {
                                popup_menu_open = true;
                                auto &selected_channels = h.mcap_tab_selected_channels[tab_id];
                                static char filter_text[128] = "";
                                ImGui::Text("Filter Channels:");
                                ImGui::PushItemWidth(300);
                                ImGui::InputText("##FilterCondition", filter_text, IM_ARRAYSIZE(filter_text),
